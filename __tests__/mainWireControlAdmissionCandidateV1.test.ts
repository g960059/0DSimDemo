...
import type { createMainWireControlAdmissionCandidateV1 as CandidateFactory } from "@/studio/integrations/mainWireIntegratedV3/MainWireControlAdmissionCandidateHostV1";
...
import type { MainWireStaticCaseSessionV1 as CandidateSessionInstance } from "@/engine/vnext/MainWireControlAdmissionCandidateSessionV1";
import { MainWireStaticCaseSessionV1 as OriginalSession } from "@/engine/vnext/MainWireStaticCaseSessionV1";
...
import { loadStudioControlAdmissionCandidateV1 } from "@/studio/composition/StudioControlAdmissionCandidateV1";
...
let candidateFactory: typeof CandidateFactory;
type Session = CandidateSessionInstance;
let Session: typeof import("@/engine/vnext/MainWireControlAdmissionCandidateSessionV1").MainWireStaticCaseSessionV1;
beforeAll(async () => {
  const module = await loadControlAdmissionCandidateModuleV1();
  candidateFactory = module.createCircleHeartExactModelReleaseV1;
  Session = module.ControlCandidateSessionV1;
});
type Release = ReturnType<typeof candidateFactory>;
const tier = hotPathIntegrityTierV1();
beforeEach(() => selectHotPathIntegrityTierV1("hot-path-lean"));
afterEach(() => { selectHotPathIntegrityTierV1(tier); vi.restoreAllMocks(); });
...
async function compiled(path: string): Promise<Release> {
  ...
  return (module.createCircleHeartExactModelReleaseV1 as typeof candidateFactory)();
}
...
describe("local control-admission candidate (never the registered model)", () => {
  it.each(CURRENT_MODEL_PRESETS_V1)("keeps pressure-volume sample continuation exact for $title", async preset => {
    ...
    const restore = () => Session.restoreWithStepRecovery(seed.checkpoint!.payload, f.anatomyId as never,
  ...
  it("corrects the reproduced 6000→4200 failure without time subdivision and resumes exactly", async () => {
    ...
    let original = await OriginalSession.restore(seed.checkpoint.payload, anatomy, f.hemodynamicResearchInputs, 1, mechanism);
    ...
    expect(() => {
      for (let tick = 1; tick <= 250; tick++) original.advanceToPresentationTimeWithSelectedOutputProjectionV1(timeAt(tick), []);
    }).toThrow(/typed ordinary coupled solve failed: maximum-iterations/);
    expect(original.currentAcceptedState().acceptedTimeSec - start).toBeLessThan(.2);
    const session = await Session.restore(saved, anatomy, inputs, 1, mechanism);
    const executable = await compiled("data/model-candidates/control-admission-v1/artifact.mjs.txt");
  ...
  it("runs the corrected large-TBV trajectory for 60 seconds without adaptive recovery", async () => {
    const seed = bundle.baseline.capture, f = seed.fixture;
    let old = await OriginalSession.restore(seed.checkpoint.payload, f.anatomyId as never, f.hemodynamicResearchInputs, 1, f.mechanismResearchInputs as never);
  ...
  it("has a separate exact identity and inherits every current Surface/analysis pin", async () => {
    const candidate = candidateFactory(), current = currentFactory();
    expect(candidate.manifest).toEqual(metadata.manifest);
    expect(candidate.manifest.modelId).not.toBe(current.manifest.modelId);
    for (const key of ["equations", "checkpointCodec", "fixtureSchema", "primitiveControlCatalog", "primitiveSignalCatalog", "modelMetricCatalog", "capabilities"] as const)
      expect(candidate.manifest[key]).toEqual(current.manifest[key]);
    expect(candidate.manifest.solver).toMatchObject(current.manifest.solver);
    expect(candidate.manifest.solver).toHaveProperty("nonconvergenceRecovery");
    const composition = await loadStudioControlAdmissionCandidateV1();
    ...
    const artifact = await readFile("data/model-candidates/control-admission-v1/artifact.mjs.txt");
    const manifest = Buffer.from(canonicalJsonStringify(candidate.manifest)), lengths = Buffer.alloc(8);
  ...
  it("launches only its own continued captures and artifact-bound prepared curves", async () => {
    ...
    for (const preset of composition.presets!) {
      ...
      expect(preset.capture.checkpoint!.acceptedTimeSec).toBeCloseTo(previous.capture.checkpoint!.acceptedTimeSec + 10, 10);
      await candidateFactory().executables.captureAdapter.validateCapture({ model, capture: preset.capture });
  ...
  it("probes exactly one short live batch without adopting lookahead or mutating siblings", async () => {
    const release = candidateFactory(), adapter = release.executables.simulationAdapter;
  ...
  it("copies a TBV admission trial by rebase with exactly the same continuation as checkpoint restore", async () => {
    for (const preset of CURRENT_MODEL_PRESETS_V1) {
      ...
      const source = await Session.restoreWithStepRecovery(preset.capture.checkpoint!.payload, f.anatomyId as never,
      ...
      const restored = await Session.restoreWithStepRecovery(adoptedBefore, f.anatomyId as never, inputs, 1, f.mechanismResearchInputs as never);
  ...
  for (const preset of presets) for (const ticks of (preset.presetId.includes("as-") ? [0] : [0, 83, 251])) for (const delta of [-5, 5]) {
    it(`${preset.presetId}, phase +${ticks * 2} ms, TBV ${delta}: independent candidate/admitted normal paths and restored continuation agree`, async () => {
      const releases = [candidateFactory(), await compiled("data/model-candidates/control-admission-v1/artifact.mjs.txt"),
        await compiled("data/model-releases/standard73/artifact.mjs.txt")];
      ...
      try {
        ...
        for (let index = 0; index < 32; index++) {
          ...
          expect(normalize(batches[2]!)).toEqual(normalize(batches[0]!));
        }
        const captures = await Promise.all(releases.map(release => capture(release, fixture)));
        expect(captures[1]).toEqual(captures[0]); expect(captures[2]).toEqual(captures[0]);
        const restored = candidateFactory();
        try {
  ...
  it("keeps the protected 6000→4200 continuation clock and exact candidate restore", async () => {
    const releases = [candidateFactory(), await compiled("data/model-releases/standard73/artifact.mjs.txt")];
    try {
      for (const release of releases) {
        const adapter = release.executables.simulationAdapter;
      ...
      // A new inverse policy is not a bit-equivalent artifact revision. This
      // protected path remains numerically close, while its own restore is exact.
      for (const key of Object.keys(admitted)) {
        ...
        if (typeof value === "number" && typeof previous === "number") expect(value).toBeCloseTo(previous, 8);
      ...
      const captured = await capture(releases[0]!, fixture), resumed = candidateFactory();
      try {
  ...
  it("executes the inherited pressure-crossing method only for the pinned candidate artifact, with an isolated source", async () => {
    const release = candidateFactory(), adapter = release.executables.simulationAdapter;
    const seed = bundle.baseline.capture;
