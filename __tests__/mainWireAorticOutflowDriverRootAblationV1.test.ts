...
describe("main-wire aortic outflow driver/root ablation V1", () => {
  ...
  it("separates sample-level shoulders from prominent independent peaks", () => {
    ...
    expect(peaks.map((peak) => peak.sampleIndex)).toEqual([1, 3, 5]);
    expect(peaks[0]!.prominence).toBeCloseTo(0.005, 12);
    expect(peaks[0]!.prominenceFractionOfGlobalMaximum)
      .toBeLessThan(0.01);
    expect(peaks[1]!.prominenceFractionOfGlobalMaximum).toBeCloseTo(1, 12);
    expect(peaks[2]!.prominence).toBeCloseTo(3, 12);
    expect(peaks.filter((peak) =>
      peak.prominenceFractionOfGlobalMaximum >= 0.01)).toHaveLength(2);
    expect(measureMainWireLocalMaximumProminencesV1(
      [0, 10, 10, 10, 0],
      0.5,
    )).toEqual([{
      sampleIndex: 2,
      value: 10,
      prominence: 10,
      prominenceFractionOfGlobalMaximum: 1,
    }]);
    expect(() => measureMainWireLocalMaximumProminencesV1(
      [0, Number.NaN, 1],
      0,
    )).toThrow("local-maximum prominence signal must be finite");
  });
  ...
  it("defines a fixed loss-placement damping bracket at the limiting stiff-load contexts", () => {
    expect(
      MAIN_WIRE_AORTIC_OUTFLOW_CHARACTERISTIC_RESISTANCE_DAMPING_PLACEMENT_PROFILE_IDS_V1,
    ).toEqual([
      "all-Ao-SA-resistance-upstream-of-root-compliance",
    ...
    for (const contextId of
      ...
      expect(context.complianceProfileId)
        .toBe("arterial-stiffness-eight-thirds");
      expect(Object.isFrozen(context)).toBe(true);
    }
    expect(MAIN_WIRE_AORTIC_OUTFLOW_CHARACTERISTIC_RESISTANCE_DAMPING_CLAIM_V1)
      .toMatchObject({
        sourceAoSaLinearResistanceSumPreservedAcrossPlacements: true,
        allUpstreamPlacementLeavesZeroDirectAoSaSeriesResistance: true,
        Land2017PlacementLeavesPositiveDirectAoSaSeriesResistance: true,
        aorticValveAreaOrOpeningLawChanged: false,
        rootInertanceChanged: false,
  ...
  it("seals the ET-relaxation candidate as a bounded effective-rate calibration", () => {
    ...
    expect(selected).toMatchObject({
      ...
      kineticParameterScaleFromSourceByParameter: { kws: 0.4, nTm: 0.8 },
      selectionStage: "bounded-ET-relaxation-balance-after-load-envelope",
    ...
    expect(selectedMaterial.landEquationParameters.values.kws)
      .toBeCloseTo(
        sourceMapped.landEquationParameters.values.kws * 0.4,
        14,
      );
    ...
    const selectedExitMaterial =
      resolveMainWireVentricularLandStrongBridgeDeactivationExitWallMaterialV1(
        exact.strongBridgeDeactivationExitProfileId,
        exact.sourceVelocityDistortionProfileId,
        exact.twitchRetentionCandidateId,
        exact.trefForceLoadProfileId,
        exact.sarcomereReferenceProfileId,
        exact.kuwProfileId,
      );
    ...
    expect(MAIN_WIRE_AORTIC_OUTFLOW_PHYSIOLOGY_CANDIDATE_V9_CLAIM)
      .toMatchObject({
        ...
        ventricularPerformanceMetricsEvaluated: [
          "ICT",
          "IVRT",
          "Tei-index",
          "LV-positive-dP-dt",
        ...
        fullSourceAoSaResistanceReinterpretedAsProximalCharacteristicImpedance:
          false,
        arterialCharacteristicImpedanceMatchedToLand2017Source: true,
        selectedProximalCharacteristicResistanceMmHgSecPerMl: 0.035,
        selectedResidualAoSaResistanceMmHgSecPerMl: 0.0115088,
        totalAoSaLinearResistancePreservedExactly: true,
        residualAoSaResistanceRestoresDirectInertialBranchDamping: true,
        allResistanceUpstreamHighStiffnessExchangeModeObserved: true,
        characteristicResistancePlacementContinuousOptimizerApplied: false,
        characteristicResistancePlacementChangesTotalLinearResistance: false,
        characteristicResistancePlacementAddsState: false,
