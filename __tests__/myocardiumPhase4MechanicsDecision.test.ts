...
describe("myocardium Phase 4A mechanics decision dossier validation", () => {
  it("passes the current repository artifacts and boundary scans", () => {
    ...
    expect(
      input.integrationFiles.some(
        (file) => file.path === "engine/myocardium/protocols/tissueHomogenizationReadiness.ts",
      ),
    ).toBe(false);
    expect(
      input.integrationFiles.some(
    ...
    expect(
      input.integrationFiles.some(
        (file) => file.path === "engine/myocardium/protocols/selectedMechanicsCalibrationReadiness.ts",
      ),
    ).toBe(false);
    expect(
      input.integrationFiles.some(
    ...
    expect(
      input.integrationFiles.some(
        (file) => file.path === "engine/myocardium/protocols/landNewMyocardiumLowPreloadCheck.ts",
      ),
    ).toBe(false);
    expect(
      input.integrationFiles.some(
