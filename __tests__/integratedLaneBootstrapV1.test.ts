...
describe("integrated lane numerical bootstrap V1", () => {
  it("owns the frozen regular-sinus, coronary, and HMII 9000 configuration", async () => {
    ...
    const configuration = bootstrap.rhythm.configuration;
    ...
    expect(configuration.ventricularCalciumDeposit
      .electricalToCalciumDelaySec).toBe(0.0625);
    expect(bootstrap.rhythm.state).toMatchObject({
      revision: 0,
      acceptedTimeSec: 0,
    ...
    expect(bootstrap.coronaryStepInput.coronaryPrior).toBe(
      MAIN_WIRE_PROVISIONAL_NORMAL_ADULT_CORONARY_PRIOR_V2,
    );
    expect(bootstrap.coronaryStepInput.coronaryDisease).toBe(
      NORMAL_CORONARY_DISEASE_INPUT_V2,
    );
    expect(bootstrap.coronaryStepInput.collapseHydraulics).toBe(
      MAIN_WIRE_PROVISIONAL_NORMAL_ADULT_CORONARY_COLLAPSE_V2,
    );
    expect(bootstrap.coronaryStepInput).toMatchObject({
      impMechanism: "cep-shortening-induced",
      shorteningImpPrior:
        NORMAL_ADULT_CORONARY_SHORTENING_IMP_GAIN_PRIOR_V2,
    });
    expect(bootstrap.coronaryInitializeInput.autoregulationWindow).toEqual({
      durationSec: 1,
      interpretation: "periodic-sinus-cycle-aligned",
    });
    expect(bootstrap.coronaryInitializeInput).not.toHaveProperty(
      "coronaryAutoregulationDrive",
    );

    expect(bootstrap.config.lvad).toMatchObject({
      enabled: true,
      speedRpm: 9_000,
    ...
    expect(bootstrap.config.iabp.enabled).toBe(false);
    expect(bootstrap.profile.inertanceByDevice.LVAD).toEqual(
      HEARTMATE_II_LITERATURE_CIRCUIT_INERTANCE_V1,
    );
    for (const deviceId of ["IMPELLA", "VA_ECMO", "VV_ECMO"] as const) {
      expect(Object.values(bootstrap.profile.inertanceByDevice[deviceId])
        .filter((value) => typeof value === "number")).toEqual([0, 0, 0, 0]);
    }
    expect(bootstrap.cold.acceptedState).toMatchObject({
      revision: 0,
      acceptedTimeSec: 0,
    });
  });
  ...
  it("preserves the periodic fixture identity while sharing its numeric rhythm construction", async () => {
    const lane = await createMainWireIntegratedLaneBootstrapV1();
    const periodic =
      createMainWireIntegratedModelRegularSinusAllOffFixtureV3();
  ...
  it("T7/T11 — computes and verifies every disclaimer-bearing profile digest", async () => {
    const { profile } = await createMainWireIntegratedLaneBootstrapV1();
  ...
  it("keeps the step-2/3 lane source free of the frozen contract traps", () => {
    ...
    expect(laneSource).not.toMatch(/RunArtifactContent|artifactStore/);
