...
import { MainWireIntegratedModelStandard71TypedAuthoritySessionV1 as Predecessor } from "@/engine/vnext/MainWireIntegratedModelStandard71TypedAuthoritySessionV1";
...
function advance(session: Session | Predecessor, time: number) {
...
describe("Standard72 history-preserving, single durable checkpoint", () => {
  it.each(["hot-path-lean", "full-invariant"] as const)("preserves the71 physical construction and unchanged forward path in %s", async tier => {
    selectHotPathIntegrityTierV1(tier);
    const source = await Session.create(), old = await Predecessor.create();
    for (let tick = 1; tick <= 30; tick++) {
      expect(advance(source, tick * .002)).toEqual(advance(old, tick * .002));
      expect(source.snapshotAcceptedStateBytes()).toEqual(old.snapshotAcceptedStateBytes());
    }
    const saved = await source.checkpointStandard72Exact(), oldSaved = await old.checkpointStandard71Exact();
    expect(saved.baseStandardCheckpointV2).toEqual(oldSaved.baseStandardCheckpointV2);
    const { numericalContinuation, ...identity } = saved.modelIdentity;
    expect(numericalContinuation).toBeTruthy();
    expect(identity).toEqual(oldSaved.modelIdentity);
    expect(saved.checkpointId).not.toBe(oldSaved.checkpointId);
  ...
  it("rejects another model and fixture instead of relabeling or filling absent state", async () => {
    const source = await warm(), saved = await source.checkpointStandard72Exact();
    const old = await Predecessor.create();
    await expect(Session.restoreStandard72ExactCheckpoint(await old.checkpointStandard71Exact())).rejects.toThrow(/field|schema/);
    await expect(Predecessor.restoreStandard71ExactCheckpoint(saved)).rejects.toThrow(/field|schema/);
