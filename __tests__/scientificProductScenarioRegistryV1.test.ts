...
describe("scientific product scenario registry V1", () => {
  ...
  it("deduplicates protocol work by stable source identity and discards a stale response", async () => {
    const pending: Array<Readonly<{
      command: Readonly<{
        kind: "runGuytonStarlingProtocol" | "runPvRelationsProtocol";
        requestId: string;
        sessionId: string;
      }>;
    ...
    const client = {
      terminate: vi.fn(),
      request: vi.fn((command: Readonly<{
        kind: "runGuytonStarlingProtocol" | "runPvRelationsProtocol";
        requestId: string;
        sessionId: string;
      }>) => new Promise((resolve) => pending.push({ command, resolve }))),
    ...
    const registry = new ScientificProductScenarioRegistryV1(
      healthyResolution(),
      loadedRuntime("protocol-cache-session", client, 95_000),
    );
    const scenarioId = registry.getDescriptorSnapshot()[0]!.id;
    const protocolEvents = vi.fn();
    const unsubscribe = registry.subscribeHemodynamicProtocols(protocolEvents);

    registry.requestHemodynamicProtocol(scenarioId, "pv-relations");
    registry.requestHemodynamicProtocol(scenarioId, "pv-relations");
    expect(client.request).toHaveBeenCalledTimes(1);
    expect(registry.getHemodynamicProtocol(scenarioId, "pv-relations"))
      .toMatchObject({
        status: "running",
        sourceIdentity: {
          revision: 95_000,
    ...
    const runtime = registry.getRuntime(scenarioId)!;
    const store = runtime.controlStore;
    const ownerToken = Symbol("protocol-source-transition");
    const disconnect = store.connectOwner(ownerToken, { current: null });
    ...
    first.resolve(protocolSuccessResponse(
      first.command,
      protocolResultSource(95_000, 95),
    ));
    await flushMicrotasks();
    expect(registry.getHemodynamicProtocol(scenarioId, "pv-relations"))
      .toMatchObject({ status: "idle", result: null });

    registry.requestHemodynamicProtocol(scenarioId, "pv-relations");
    expect(client.request).toHaveBeenCalledTimes(2);
    const second = pending[1]!;
    second.resolve(protocolSuccessResponse(
      second.command,
      protocolResultSource(95_001, 95.002),
    ));
    await flushMicrotasks();
    expect(registry.getHemodynamicProtocol(scenarioId, "pv-relations"))
      .toMatchObject({
...
function protocolSuccessResponse(
  command: Readonly<{
    kind: "runGuytonStarlingProtocol" | "runPvRelationsProtocol";
    requestId: string;
    sessionId: string;
  ...
  return Object.freeze({
    ...
    releaseRef: SCIENTIFIC_PRODUCT_RELEASE_REF_V1,
    sessionOrigin: Object.freeze({ kind: "test" }),
    payload: command.kind === "runPvRelationsProtocol"
      ? Object.freeze({
        kind: "pvRelationsProtocolCompleted" as const,
        result: Object.freeze({ source }),
        sourceSessionUnchanged: true as const,
        observableFrame: Object.freeze({}),
      })
      : Object.freeze({
