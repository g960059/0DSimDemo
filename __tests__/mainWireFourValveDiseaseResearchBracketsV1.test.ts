...
describe("main-wire four-valve disease research input V1", () => {
  it("publishes the complete V2 normal baseline valve parameter set", () => {
    ...
    expect(validateMainWireFourValveDiseaseResearchInputV1(
      MAIN_WIRE_FOUR_VALVE_NORMAL_RESEARCH_INPUT_V1,
    )).toEqual([]);
  });
  ...
  it("publishes 24 complete, explicitly non-diagnostic research brackets", () => {
    ...
    expect(Object.keys(MAIN_WIRE_FOUR_VALVE_DISEASE_RESEARCH_INPUTS_V1)).toHaveLength(24);
    for (const bracket of MAIN_WIRE_FOUR_VALVE_DISEASE_BRACKETS_V1) {
      const researchInput = MAIN_WIRE_FOUR_VALVE_DISEASE_RESEARCH_INPUTS_V1[bracket.bracketId];
      expect(Object.keys(researchInput.valves).sort())
      ...
      expect(validateMainWireFourValveDiseaseResearchInputV1(researchInput)).toEqual([]);
    }
  });
  ...
  it("changes only the target valve area while preserving healthy kinetics", () => {
    for (const bracket of MAIN_WIRE_FOUR_VALVE_DISEASE_BRACKETS_V1) {
      ...
      for (const valveId of MAIN_WIRE_FOUR_VALVE_IDS_V1) {
        const normal = MAIN_WIRE_FOUR_VALVE_NORMAL_RESEARCH_INPUT_V1.valves[valveId];
  ...
  it("composes same-valve stenosis and regurgitation parameter fields without conflict", () => {
    ...
    expect(kinetics(mixed.valves.AoV)).toEqual(kinetics(
      MAIN_WIRE_FOUR_VALVE_NORMAL_RESEARCH_INPUT_V1.valves.AoV,
    ));
    for (const valveId of ["MV", "TV", "PV"] as const) {
      expect(physicalParameters(mixed.valves[valveId])).toEqual(
        physicalParameters(MAIN_WIRE_FOUR_VALVE_NORMAL_RESEARCH_INPUT_V1.valves[valveId]),
      );
    }
  });
  ...
  it("rejects duplicate direction lesions on one valve but permits mixed disease", () => {
    expect(() => composeMainWireFourValveDiseaseResearchInputV1([
      "AS-mild",
      "AS-severe",
  ...
  it("rejects closed reverse EROA larger than forward maximum EOA", () => {
    ...
    expect(validateMainWireFourValveDiseaseResearchInputV1(invalid))
      .toContain(
  ...
  it("rejects spoofed bracket, hash, claim, and per-valve provenance", () => {
    const valid = composeMainWireFourValveDiseaseResearchInputV1(["MR-moderate"]);
    const spoofed = {
      ...valid,
