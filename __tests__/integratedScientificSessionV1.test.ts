...
describe("MainWireIntegratedScientificSession", () => {
  it("T-A1/T-A3 — lands exactly for 500 ordinals and stays in direct V3 lockstep", async () => {
    ...
    const directPreset = await createMainWireIntegratedLanePresetV1();
    let direct = directPreset.cold.acceptedState;
  ...
  it("T-A5/T2/T15 — continues exactly after boundary and off-grid operational checkpoints", async () => {
    const boundarySession = await MainWireIntegratedScientificSession.create();
    ...
    const uninterrupted = await MainWireIntegratedScientificSession.create();
  ...
  while (accepted.acceptedTimeSec !== targetTimeSec) {
    ...
    const result = stepMainWireIntegratedModelV3(
      ...
      accepted,
      {
        ...
        coronary: preset.coronaryStepInput,
        rhythm: {
          configuration: preset.rhythm.configuration,
          externalAfNextBoundaryTimeSec: null,
          externalAtrialSourceBatch: null,
...
function installRhythmInput(
  ...
  sessionInternals.rhythmInput = Object.freeze({
    configuration: sessionInternals.preset.rhythm.configuration,
    ...input,
  });
}
