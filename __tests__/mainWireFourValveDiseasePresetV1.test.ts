...
describe("main-wire four-valve disease preset V1", () => {
  it("publishes the complete V2 normal baseline valve parameter set", () => {
    expect(MAIN_WIRE_FOUR_VALVE_NORMAL_PRESET_V1.bracketIds).toEqual([]);
    expect(MAIN_WIRE_FOUR_VALVE_NORMAL_PRESET_V1.valves).toEqual({
    ...
    expect(MAIN_WIRE_FOUR_VALVE_NORMAL_PRESET_V1.claim).toMatchObject({
      presetRole: "ordered-research-bracket-not-clinical-diagnosis",
    ...
    expect(validateMainWireFourValveDiseasePresetV1(
      MAIN_WIRE_FOUR_VALVE_NORMAL_PRESET_V1,
    )).toEqual([]);
  });
  ...
  it("publishes 24 complete, explicitly non-diagnostic research brackets", () => {
    ...
    expect(Object.keys(MAIN_WIRE_FOUR_VALVE_DISEASE_PRESETS_V1)).toHaveLength(24);
    for (const bracket of MAIN_WIRE_FOUR_VALVE_DISEASE_BRACKETS_V1) {
      const preset = MAIN_WIRE_FOUR_VALVE_DISEASE_PRESETS_V1[bracket.bracketId];
      expect(Object.keys(preset.valves).sort())
        .toEqual([...MAIN_WIRE_FOUR_VALVE_IDS_V1].sort());
      expect(preset.bracketIds).toEqual([bracket.bracketId]);
      expect(bracket.label).toContain("research bracket");
      expect(bracket.label).toContain("not a clinical diagnosis");
      expect(bracket.labelSemantics)
        .toBe("research-bracket-not-clinical-diagnosis");
      expect(bracket.requiresClosedLoopHemodynamicValidation).toBe(true);
      expect(bracket.requiresPostSolveAreaCalibration)
        .toBe(bracket.lesionCode === "PS" || bracket.lesionCode === "PR");
      expect(validateMainWireFourValveDiseasePresetV1(preset)).toEqual([]);
    }
  });
  ...
  it("changes only the target valve area while preserving healthy kinetics", () => {
    for (const bracket of MAIN_WIRE_FOUR_VALVE_DISEASE_BRACKETS_V1) {
      ...
      for (const valveId of MAIN_WIRE_FOUR_VALVE_IDS_V1) {
        const normal = MAIN_WIRE_FOUR_VALVE_NORMAL_PRESET_V1.valves[valveId];
        const candidate = preset.valves[valveId];
        expect(kinetics(candidate)).toEqual(kinetics(normal));
        if (valveId !== bracket.targetValveId) {
          expect(physicalParameters(candidate)).toEqual(physicalParameters(normal));
          continue;
        }
        if (bracket.lesionKind === "stenosis") {
          expect(candidate.maximumForwardEoaCm2).toBe(bracket.areaCm2);
          expect(candidate.closedReverseEroaCm2)
            .toBe(normal.closedReverseEroaCm2);
        } else {
          expect(candidate.closedReverseEroaCm2).toBe(bracket.areaCm2);
          expect(candidate.maximumForwardEoaCm2)
            .toBe(normal.maximumForwardEoaCm2);
        }
      }
    }
  });
  ...
  it("uses the requested area brackets and evidence-strength metadata", () => {
    ...
    expect(area("AR-moderate")).toBe(0.20);
    expect(area("AR-severe")).toBe(0.35);
    expect(area("MR-mild")).toBe(0.10);
    expect(area("MR-moderate")).toBe(0.25);
    expect(area("MR-severe")).toBe(0.45);
    expect(area("TR-mild")).toBe(0.10);
    expect(area("TR-moderate")).toBe(0.30);
    expect(area("TR-severe")).toBe(0.50);
    expect(area("PR-mild")).toBe(0.10);
    expect(area("PR-moderate")).toBe(0.40);
    expect(area("PR-severe")).toBe(0.90);

    for (const code of ["AS", "MS", "AR", "MR", "TR"] as const) {
      expect(bracket(`${code}-moderate`).evidenceBasis)
        .toBe("guideline-area-anchor");
      expect(bracket(`${code}-moderate`).clinicalAreaThresholdStatus)
        .toBe("guideline-area-anchor");
    }
    expect(bracket("TS-mild").evidenceBasis)
      .toBe("engineering-only-bracket");
    expect(bracket("TS-moderate").evidenceBasis)
      .toBe("engineering-only-bracket");
    expect(bracket("TS-severe").evidenceBasis)
      .toBe("guideline-area-anchor");
    expect(bracket("TS-moderate").clinicalAreaThresholdStatus)
      .toBe("engineering-only-no-clinical-grade-anchor");
    expect(bracket("TS-severe").clinicalAreaThresholdStatus)
      .toBe("guideline-significant-endpoint-anchor");
    expect(bracket("PS-moderate").evidenceBasis)
      .toBe("hemodynamic-calibration-required");
    expect(bracket("PR-moderate").evidenceBasis)
      .toBe("hemodynamic-calibration-required");
    expect(bracket("PS-moderate").clinicalAreaThresholdStatus)
      .toBe("no-fixed-clinical-area-threshold");
    expect(bracket("PR-moderate").clinicalAreaThresholdStatus)
      .toBe("no-fixed-clinical-area-threshold");
  });

  it("composes same-valve stenosis and regurgitation parameter fields without conflict", () => {
    const mixed = composeMainWireFourValveDiseasePresetV1([
      "AS-severe",
      "AR-moderate",
    ]);
    expect(mixed.valves.AoV.maximumForwardEoaCm2).toBe(0.8);
    expect(mixed.valves.AoV.closedReverseEroaCm2).toBe(0.20);
    expect(kinetics(mixed.valves.AoV)).toEqual(kinetics(
      MAIN_WIRE_FOUR_VALVE_NORMAL_PRESET_V1.valves.AoV,
    ));
    for (const valveId of ["MV", "TV", "PV"] as const) {
      expect(physicalParameters(mixed.valves[valveId])).toEqual(
        physicalParameters(MAIN_WIRE_FOUR_VALVE_NORMAL_PRESET_V1.valves[valveId]),
      );
    }
  });

  it("makes composition identities and parameter sets independent of order", () => {
    const left = composeMainWireFourValveDiseasePresetV1([
      "MR-moderate",
      "PS-severe",
      "AS-mild",
    ]);
    const right = composeMainWireFourValveDiseasePresetV1([
      "AS-mild",
      "MR-moderate",
      "PS-severe",
    ]);
  ...
  it("rejects duplicate direction lesions on one valve but permits mixed disease", () => {
    expect(() => composeMainWireFourValveDiseasePresetV1([
      "AS-mild",
      "AS-severe",
  ...
  it("rejects closed reverse EROA larger than forward maximum EOA", () => {
    const valid = composeMainWireFourValveDiseasePresetV1(["AS-severe"]);
    const invalid = {
      ...valid,
      valves: {
        ...valid.valves,
    ...
    } as MainWireFourValveDiseasePresetV1;
    expect(validateMainWireFourValveDiseasePresetV1(invalid))
      .toContain(
  ...
  it("rejects spoofed bracket, hash, claim, and per-valve provenance", () => {
    const valid = composeMainWireFourValveDiseasePresetV1(["MR-moderate"]);
    const spoofed = {
      ...valid,
    ...
    } as unknown as MainWireFourValveDiseasePresetV1;
    const issues = validateMainWireFourValveDiseasePresetV1(spoofed);
    expect(issues).toContain("claim must exactly match the canonical V1 claim");
    expect(issues).toContain("parameterIdentityHash does not match preset contents");
...
function bracket(
  ...
  return MAIN_WIRE_FOUR_VALVE_DISEASE_BRACKETS_V1.find((candidate) =>
    candidate.bracketId === bracketId)!;
}
...
function kinetics(
  params: MainWireFourValveDiseasePresetV1["valves"]["MV"],
) {
  return {
