...
export const RIGHT_AV_PASSIVE_DIASTASIS_GUARD_PHASE5DG_ID =
  "right-av-passive-diastasis-guard-phase5dg-result-v1" as const;
export const RIGHT_AV_PASSIVE_DIASTASIS_GUARD_PHASE5DG_RESULT_PATH =
  "data/myocardium/protocols/right-av-passive-diastasis-guard-phase5dg-result-v1.json" as const;
...
type VariantId =
  ...
  | "passive-diastasis-guard-tv-0p50"
  | "passive-diastasis-guard-tv-0p75"
  | "passive-diastasis-guard-both-0p50";
...
type VariantSpec = {
  ...
  readonly passiveAtrialActiveThreshold01: number | null;
  readonly passivePressureGradientThresholdMmHg: number | null;
...
type Classification = {
  ...
  readonly contractilityLowTvfOutcome: {
    ...
    readonly bestTvPassiveGuardAppliedFraction: number | null;
    readonly bestTvPassiveGuardImpulseIntegralMlPerSec: number | null;
  };
  readonly decision:
    | "passive-diastasis-guard-rescues-frontier"
    | "passive-diastasis-guard-partial-or-no-go"
    | "passive-diastasis-guard-breaks-output-or-mv"
...
type Evidence = {
  ...
  readonly id: typeof RIGHT_AV_PASSIVE_DIASTASIS_GUARD_PHASE5DG_ID;
  readonly phase: "5DG";
  readonly profile: {
    ...
    readonly diagnosticQuestion:
      "whether a passive-diastasis AV boundary contract removes the remaining contractility-low TVF third wave without weakening the morphology gate";
  };
...
export function buildRightAvPassiveDiastasisGuardPhase5DGEvidence(): Evidence {
  const slowRelease = {
  ...
  const variants = [
    ...
    acceptedUser0Variant(
      "passive-diastasis-guard-tv-0p50",
      "Current user0 pressure-flow plus passive-diastasis reacceleration guard on TV at 0.50mmHg",
      { LA: inletHeldRelease, RA: inletHeldRelease },
      "accepted-state-valve-pressure-flow-passive-diastasis-guard",
      ["TV"],
      0.02,
    ...
    acceptedUser0Variant(
      "passive-diastasis-guard-tv-0p75",
      "Current user0 pressure-flow plus passive-diastasis reacceleration guard on TV at 0.75mmHg",
      { LA: inletHeldRelease, RA: inletHeldRelease },
      "accepted-state-valve-pressure-flow-passive-diastasis-guard",
      ["TV"],
      0.02,
    ...
    acceptedUser0Variant(
      "passive-diastasis-guard-both-0p50",
      "Current user0 pressure-flow plus passive-diastasis reacceleration guard on MV/TV at 0.50mmHg",
      { LA: inletHeldRelease, RA: inletHeldRelease },
      "accepted-state-valve-pressure-flow-passive-diastasis-guard",
      ["MV", "TV"],
      0.02,
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: RIGHT_AV_PASSIVE_DIASTASIS_GUARD_PHASE5DG_ID,
    phase: "5DG",
    profile: {
...
function acceptedUser0Variant(
  ...
  passiveAtrialActiveThreshold01: number | null,
  passivePressureGradientThresholdMmHg: number | null,
): VariantSpec {
  ...
  return {
    ...
    passiveAtrialActiveThreshold01,
    passivePressureGradientThresholdMmHg,
...
function withAvBoundaryMode(
  ...
  return {
    ...
    ventricularChamberTransactionStep: {
      ...
      ...(passiveAtrialActiveThreshold01 == null ? {} : {
        avValveBoundaryPassiveAtrialActiveThreshold01: passiveAtrialActiveThreshold01,
      }),
      ...(passivePressureGradientThresholdMmHg == null ? {} : {
        avValveBoundaryPassivePressureGradientThresholdMmHg: passivePressureGradientThresholdMmHg,
      }),
    },
  };
}
...
function classify(summaries: readonly VariantSummary[], results: readonly PointResult[]): Classification {
  ...
  const bestTvGuard = bestContractilityLow?.avResiduals?.TV ?? null;
  const bestImproves =
  ...
  return {
    ...
    contractilityLowTvfOutcome: {
      ...
      bestTvPassiveGuardAppliedFraction: bestTvGuard?.passiveDiastasisGuardAppliedFraction ?? null,
      bestTvPassiveGuardImpulseIntegralMlPerSec: bestTvGuard?.passiveDiastasisGuardImpulseIntegralMlPerSec ?? null,
    },
    ...
    notes: [
      ...
      `Best guard variant: ${best.variantId}, causes: ${formatCounts(best.causeCounts)}.`,
      `Baseline residual notes: ${residualNotes(results, baseline.variantId).join(" | ") || "none"}.`,
      `Best guard residual notes: ${residualNotes(results, best.variantId).join(" | ") || "none"}.`,
    ],
  };
}
...
function recommendedNext(classification: Classification): readonly string[] {
  ...
  if (classification.decision === "passive-diastasis-guard-rescues-frontier") {
    return [
      "treat passive-diastasis reacceleration guarding as strong component evidence for a broader AV valve/load/atrial-pressure boundary contract",
      "do not adopt this local guard directly; next formalize the contract with duty limits, pressure-flow energy readbacks, and owner visual trace review",
    ];
  }
  if (classification.decision === "passive-diastasis-guard-breaks-output-or-mv") {
    return [
      "do not continue this passive-diastasis guard because it breaks MV/output while trying to repair TVF",
      "move to a broader chamber pressure/valve/load residual contract rather than another local boundary variant",
    ];
  }
  return [
    "passive-diastasis reacceleration guarding does not cleanly rescue the targeted user0 TVF residual",
    ...
    .flatMap((result) => {
      ...
      return (["MV", "TV"] as const)
        ...
        .map((valve) => {
          ...
          return `${result.pointId}/${valve}: cause=${residual.likelyCause}, extra=${residual.extraPeakCount}, acceptedDiode=${residual.acceptedDiodeHitFraction}, acceptedLeak=${residual.acceptedComplementarityLeakFraction}, acceptedValveStateMean=${residual.acceptedValveStateMean}, acceptedAreaRatioMean=${residual.acceptedAreaRatioMean}, passiveGuardDuty=${residual.passiveDiastasisGuardAppliedFraction}, passiveGuardImpulse=${residual.passiveDiastasisGuardImpulseIntegralMlPerSec}, avPlaneAlign=${residual.avPlaneVelocityExtraAlignmentTheta}, atrialPressureAlign=${residual.atrialPressureExtraAlignmentTheta}, releaseHold=${residual.avPlaneReleaseInletOpenHoldMean}`;
        });
    });
}
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5DG variant summary ${id}.`);
  return summary;
}
...
export function writeRightAvPassiveDiastasisGuardPhase5DGEvidence(): Evidence {
  const evidence = buildRightAvPassiveDiastasisGuardPhase5DGEvidence();
  const outPath = path.resolve(process.cwd(), RIGHT_AV_PASSIVE_DIASTASIS_GUARD_PHASE5DG_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeRightAvPassiveDiastasisGuardPhase5DGEvidence();
