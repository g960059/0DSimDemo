...
export const VENTRICULAR_CHAMBER_TRANSACTION_PHASE5CF_ID =
  "ventricular-chamber-transaction-phase5cf-result-v1" as const;
export const VENTRICULAR_CHAMBER_TRANSACTION_PHASE5CF_RESULT_PATH =
  "data/myocardium/protocols/ventricular-chamber-transaction-phase5cf-result-v1.json" as const;
...
type Classification = {
  ...
  readonly decision:
    ...
    | "ventricular-chamber-transaction-redesign-required";
  readonly notes: readonly string[];
};

type Evidence = {
  readonly schemaVersion: 1;
  readonly id: typeof VENTRICULAR_CHAMBER_TRANSACTION_PHASE5CF_ID;
  readonly phase: "5CF";
  readonly profile: {
    ...
    readonly candidateMechanism:
      "ModelCore ventricularChamberTransactionStep local LV/RV volume-flow-provider-state fixed-point transaction, off by default";
  };
...
export function buildVentricularChamberTransactionPhase5CFEvidence(): Evidence {
  const variants = [
    legacyVariant(),
    currentUser0Variant(),
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: VENTRICULAR_CHAMBER_TRANSACTION_PHASE5CF_ID,
    phase: "5CF",
    profile: {
...
function transactionUser0Variant(): VariantSpec {
  ...
  return {
    ...
    label: "Current user-0 closure with LV/RV local chamber transaction step",
    hypothesis: "Tests whether locally solving LV/RV volume, valve flow, and provider-state advancement in the same fixed-point step rescues raw PV dome and AV inflow morphology.",
    closurePath: "current-user0-all-chamber-landatrial",
    ventricularChamberTransaction: {
      mechanismId: "phase5cf-ventricular-chamber-transaction-user0",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
    },
    experimentalOptions: withVentricularChamberTransactionStep(
      ...
      "phase5cf-ventricular-chamber-transaction-user0",
    ),
  };
}
...
function transactionLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    label: "LV/RV Land legacy atria with LV/RV local chamber transaction step",
    hypothesis: "Separates LV/RV chamber/valve/provider transaction effects from LandAtrial AV-plane/effective-wall timing.",
    closurePath: "lv-rv-land-legacy-atria",
    ventricularChamberTransaction: {
      mechanismId: "phase5cf-ventricular-chamber-transaction-legacy-atria",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
    },
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cf-ventricular-chamber-transaction-legacy-atria",
    ),
  };
}
...
function withVentricularChamberTransactionStep(
  ...
  if (!providers?.LV || !providers.RV) {
    throw new Error("Phase 5CF ventricular chamber transaction requires LV and RV source providers.");
  }
  return {
    ...experimentalOptions,
...
function recommendedNext(classification: Classification): readonly string[] {
  ...
  return [
    "do not adopt the ventricularChamberTransactionStep candidate unless it passes the full morphology envelope",
    "if the local LV/RV transaction remains partial, the next surface must solve the broader chamber/load graph or change the chamber pressure adapter contract",
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CF variant summary ${id}.`);
  return summary;
}
...
function requiredResult(results: readonly PointResult[], variantId: VariantId, pointId: PointId): PointResult {
  ...
  if (!result) throw new Error(`Missing Phase 5CF result ${variantId}/${pointId}.`);
  return result;
}
...
export function writeVentricularChamberTransactionPhase5CFEvidence(): Evidence {
  const evidence = buildVentricularChamberTransactionPhase5CFEvidence();
  const outPath = path.resolve(process.cwd(), VENTRICULAR_CHAMBER_TRANSACTION_PHASE5CF_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeVentricularChamberTransactionPhase5CFEvidence();
