...
export const ACTIVE_SOURCE_PRESSURE_CONTRACT_PHASE5CE_ID =
  "active-source-pressure-contract-v2-phase5ce-result-v1" as const;
export const ACTIVE_SOURCE_PRESSURE_CONTRACT_PHASE5CE_RESULT_PATH =
  "data/myocardium/protocols/active-source-pressure-contract-v2-phase5ce-result-v1.json" as const;
...
type VariantId =
  ...
  | "v2-reference-geometry-user0"
  | "v2-reference-geometry-legacy-atria";
...
type VariantSpec = {
  ...
  readonly activeSourcePressureContractV2: null | {
    readonly mode: "reference-geometry-active-pressure-v1";
...
type Evidence = {
  ...
  readonly id: typeof ACTIVE_SOURCE_PRESSURE_CONTRACT_PHASE5CE_ID;
  readonly phase: "5CE";
  readonly profile: {
    ...
    readonly candidateMechanism:
      "ActiveSourcePressureContractV2 reference-geometry active pressure adapter for LV/RV, off by default";
  };
...
export function buildActiveSourcePressureContractPhase5CEEvidence(): Evidence {
  const variants = [
    legacyVariant(),
    currentUser0Variant(),
    lvRvLandLegacyAtriaVariant(),
    v2ReferenceGeometryUser0Variant(),
    v2ReferenceGeometryLegacyAtriaVariant(),
  ] as const;
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: ACTIVE_SOURCE_PRESSURE_CONTRACT_PHASE5CE_ID,
    phase: "5CE",
    profile: {
...
function legacyVariant(): VariantSpec {
  ...
  return {
    ...
    activeSourcePressureContractV2: null,
...
function v2ReferenceGeometryUser0Variant(): VariantSpec {
  const resolved = resolveModelCoreRuntimeActiveSource({
    mode: MODELCORE_RUNTIME_ALL_CHAMBER_LANDATRIAL_DEFAULT_MODE,
    runtimeParams: DEFAULT_PARAMS,
  });
  return {
    id: "v2-reference-geometry-user0",
    label: "Current user-0 closure with LV/RV ActiveSourcePressureContractV2 reference-geometry adapter",
    hypothesis: "Tests whether removing instantaneous LV/RV geometry-gain amplification from source stress to chamber pressure rescues raw PV dome and AV inflow morphology.",
    closurePath: "current-user0-all-chamber-landatrial",
    activeSourcePressureContractV2: {
      mode: "reference-geometry-active-pressure-v1",
      chambers: ["LV", "RV"],
    },
    experimentalOptions: withReferenceGeometryV2Providers(
      resolved.experimentalOptions,
      "phase5ce-reference-geometry-user0",
    ),
  };
}

function v2ReferenceGeometryLegacyAtriaVariant(): VariantSpec {
  const resolved = resolveModelCoreRuntimeActiveSource({
    mode: MODELCORE_RUNTIME_LV_RV_LAND_DEFAULT_MODE,
    runtimeParams: DEFAULT_PARAMS,
  });
  return {
    id: "v2-reference-geometry-legacy-atria",
    label: "LV/RV Land legacy atria with ActiveSourcePressureContractV2 reference-geometry adapter",
    hypothesis: "Separates LV/RV pressure-adapter geometry from LandAtrial AV-plane/effective-wall timing.",
    closurePath: "lv-rv-land-legacy-atria",
    activeSourcePressureContractV2: {
      mode: "reference-geometry-active-pressure-v1",
      chambers: ["LV", "RV"],
    ...
    experimentalOptions: withReferenceGeometryV2Providers(
      resolved.experimentalOptions,
      "phase5ce-reference-geometry-legacy-atria",
    ),
  };
}

function withReferenceGeometryV2Providers(
  ...
  if (!providers?.LV || !providers.RV) {
    throw new Error("Phase 5CE reference-geometry V2 requires LV and RV source providers.");
  }
  return {
    ...experimentalOptions,
...
function classify(
  ...
  const v2User0 = requiredSummary(summaries, "v2-reference-geometry-user0");
  const v2LegacyAtria = requiredSummary(summaries, "v2-reference-geometry-legacy-atria");
  const bestV2 = bestSummary([v2User0, v2LegacyAtria]);
  ...
  const normalBestV2 = requiredResult(results, bestV2.variantId, "normal-hr75");
  const contractCleanEnough =
    bestV2.grossPassCount === POINTS.length
    && bestV2.contractClassCounts.cleanVentricularSide >= POINTS.length * 2
    && bestV2.contractClassCounts.cleanAvInflow >= POINTS.length * 2;
  return {
    ...
    bestV2Variant: `${bestV2.variantId}:${bestV2.grossPassCount}/${POINTS.length}`,
    ...
    notes: [
      ...
      `Best V2 normal failed labels: ${normalBestV2.failedLabels.join(", ") || "none"}.`,
      `Best V2 gross pass count: ${bestV2.grossPassCount}/${POINTS.length}.`,
...
function recommendedNext(classification: Classification): readonly string[] {
  ...
  return [
    "do not adopt the reference-geometry ActiveSourcePressureContractV2 candidate unless it passes the full morphology envelope",
    "if V2 remains partial, replace the reference-geometry adapter with a chamber-owned contract where chamber pressure, chamber volume/strain, valve-load boundary, and Land provider-state advancement are one transaction",
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CE variant summary ${id}.`);
  return summary;
}
...
function requiredResult(results: readonly PointResult[], variantId: VariantId, pointId: PointId): PointResult {
  ...
  if (!result) throw new Error(`Missing Phase 5CE result ${variantId}/${pointId}.`);
  return result;
}
...
export function writeActiveSourcePressureContractPhase5CEEvidence(): Evidence {
  const evidence = buildActiveSourcePressureContractPhase5CEEvidence();
  const outPath = path.resolve(process.cwd(), ACTIVE_SOURCE_PRESSURE_CONTRACT_PHASE5CE_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeActiveSourcePressureContractPhase5CEEvidence();
