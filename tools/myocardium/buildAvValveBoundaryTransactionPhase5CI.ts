...
export const AV_VALVE_BOUNDARY_TRANSACTION_PHASE5CI_ID =
  "av-valve-boundary-transaction-phase5ci-result-v1" as const;
export const AV_VALVE_BOUNDARY_TRANSACTION_PHASE5CI_RESULT_PATH =
  "data/myocardium/protocols/av-valve-boundary-transaction-phase5ci-result-v1.json" as const;
...
type VariantId =
  ...
  | "av-boundary-tau015-legacy-atria"
  | "av-boundary-tau025-legacy-atria"
  | "av-boundary-tau040-legacy-atria"
  | "av-boundary-tau025-user0";
...
type AvValveBoundaryTransaction = {
...
type PointResult = {
  ...
  readonly avInflowResiduals: Readonly<Record<"MV" | "TV", AvValveBoundaryTransaction>> | null;
...
type Evidence = {
  ...
  readonly id: typeof AV_VALVE_BOUNDARY_TRANSACTION_PHASE5CI_ID;
  readonly phase: "5CI";
  readonly profile: {
    ...
    readonly candidateMechanism:
      "ModelCore ventricularChamberTransactionStep with adjacent-load-node volume-flow-provider-state fixed-point transaction and off-by-default MV/TV bounded-deceleration AV valve boundary candidates";
  };
...
export function buildAvValveBoundaryTransactionPhase5CIEvidence(): Evidence {
  const variants = [
    legacyVariant(),
    currentUser0Variant(),
    ...
    avBoundaryLegacyAtriaVariant("av-boundary-tau015-legacy-atria", 0.015),
    avBoundaryLegacyAtriaVariant("av-boundary-tau025-legacy-atria", 0.025),
    avBoundaryLegacyAtriaVariant("av-boundary-tau040-legacy-atria", 0.04),
    avBoundaryUser0Variant("av-boundary-tau025-user0", 0.025),
  ] as const;
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: AV_VALVE_BOUNDARY_TRANSACTION_PHASE5CI_ID,
    phase: "5CI",
    profile: {
...
function transactionUser0Variant(): VariantSpec {
  ...
  return {
    ...
    ventricularChamberTransaction: {
      mechanismId: "phase5ci-av-inflow-residual-attribution-user0",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      ...
      "phase5ci-av-inflow-residual-attribution-user0",
      null,
    ),
  };
}
...
function transactionLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    closurePath: "lv-rv-land-legacy-atria",
    ventricularChamberTransaction: {
      mechanismId: "phase5ci-av-inflow-residual-attribution-legacy-atria",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
      includeAdjacentLoadNodes: true,
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5ci-av-inflow-residual-attribution-legacy-atria",
      null,
    ),
  };
}

function avBoundaryLegacyAtriaVariant(id: Extract<VariantId, `av-boundary-tau${string}-legacy-atria`>, tauSec: number): VariantSpec {
  const resolved = resolveModelCoreRuntimeActiveSource({
    mode: MODELCORE_RUNTIME_LV_RV_LAND_DEFAULT_MODE,
    runtimeParams: DEFAULT_PARAMS,
  });
  return {
    id,
    label: `LV/RV Land legacy atria with MV/TV bounded-deceleration AV boundary tau=${Math.round(tauSec * 1000)}ms`,
    hypothesis: "Tests whether making the residual AV valve diode boundary continuous inside the chamber transaction removes MVF/TVF extra waves while preserving LV/RV PV domes.",
    closurePath: "lv-rv-land-legacy-atria",
    ventricularChamberTransaction: transactionDescriptor(id, tauSec),
    experimentalOptions: withVentricularChamberTransactionStep(resolved.experimentalOptions, `phase5ci-${id}`, tauSec),
  };
}

function avBoundaryUser0Variant(id: Extract<VariantId, `av-boundary-tau${string}-user0`>, tauSec: number): VariantSpec {
  const resolved = resolveModelCoreRuntimeActiveSource({
    mode: MODELCORE_RUNTIME_ALL_CHAMBER_LANDATRIAL_DEFAULT_MODE,
    runtimeParams: DEFAULT_PARAMS,
  });
  return {
    id,
    label: `Current user-0 closure with MV/TV bounded-deceleration AV boundary tau=${Math.round(tauSec * 1000)}ms`,
    hypothesis: "Checks whether the best legacy-atria AV-boundary candidate transfers back to the current all-chamber LandAtrial default closure.",
    closurePath: "current-user0-all-chamber-landatrial",
    ventricularChamberTransaction: transactionDescriptor(id, tauSec),
    experimentalOptions: withVentricularChamberTransactionStep(resolved.experimentalOptions, `phase5ci-${id}`, tauSec),
  };
...
function transactionDescriptor(mechanismId: string, tauSec: number): NonNullable<VariantSpec["ventricularChamberTransaction"]> {
  return {
    mechanismId,
    iterations: 4,
    relaxation: 0.7,
    chambers: ["LV", "RV"],
    includeAdjacentLoadNodes: true,
    avValveBoundaryMode: "bounded-deceleration",
    avValveBoundaryTauSec: tauSec,
  };
}
...
function withVentricularChamberTransactionStep(
  ...
  mechanismId: string,
  avValveBoundaryTauSec: number | null,
): ModelCoreExperimentalOptions {
  const providers = experimentalOptions.activeSourceProviders;
  if (!providers?.LV || !providers.RV) {
    throw new Error("Phase 5CI AV inflow residual attribution requires LV and RV source providers.");
  }
  return {
    ...experimentalOptions,
    ventricularChamberTransactionStep: {
      ...
      includeAdjacentLoadNodes: true,
      avValveBoundaryMode: avValveBoundaryTauSec == null ? "current-diode" : "bounded-deceleration",
      avValveBoundaryTargetValves: ["MV", "TV"],
      avValveBoundaryTauSec: avValveBoundaryTauSec ?? undefined,
    },
  };
}
...
function countResidualCauses(residuals: readonly AvValveBoundaryTransaction[]): ResidualCauseCounts {
...
function unmeasuredResidual(valve: "MV" | "TV", reason: string): AvValveBoundaryTransaction {
  return {
    valve,
    measured: false,
    morphologyOk: false,
...
function classify(
  ...
  const transactionCandidates = [
    ...
    requiredSummary(summaries, "av-boundary-tau015-legacy-atria"),
    requiredSummary(summaries, "av-boundary-tau025-legacy-atria"),
    requiredSummary(summaries, "av-boundary-tau040-legacy-atria"),
    requiredSummary(summaries, "av-boundary-tau025-user0"),
  ];
...
function recommendedNext(classification: Classification): readonly string[] {
  ...
  const pressureContractDominant =
    ...
    >= causes["valve-diode-or-qdot-contamination"] + causes["valve-state-chatter"];
  const next: string[] = [
    "do not adopt the adjacent-load-node or bounded-deceleration ventricularChamberTransactionStep candidates unless they pass the full morphology envelope",
  ];
  if (valveBoundaryDominant) {
    next.push("move beyond post-diode bounded-deceleration toward an AV valve boundary transaction that solves valve state, pressure gradient, flow, and chamber-volume commit together");
  }
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CI variant summary ${id}.`);
  return summary;
}
...
function requiredResult(results: readonly PointResult[], variantId: VariantId, pointId: PointId): PointResult {
  ...
  if (!result) throw new Error(`Missing Phase 5CI result ${variantId}/${pointId}.`);
  return result;
}
...
export function writeAvValveBoundaryTransactionPhase5CIEvidence(): Evidence {
  const evidence = buildAvValveBoundaryTransactionPhase5CIEvidence();
  const outPath = path.resolve(process.cwd(), AV_VALVE_BOUNDARY_TRANSACTION_PHASE5CI_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeAvValveBoundaryTransactionPhase5CIEvidence();
