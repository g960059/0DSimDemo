...
export const STATEFUL_AVPLANE_RELEASE_PHASE5CV_ID =
  "stateful-avplane-release-phase5cv-result-v1" as const;
export const STATEFUL_AVPLANE_RELEASE_PHASE5CV_RESULT_PATH =
  "data/myocardium/protocols/stateful-avplane-release-phase5cv-result-v1.json" as const;
...
type VariantId =
  ...
  | "accepted-av-complementarity2-user0-la-stateful-release"
  | "accepted-av-complementarity2-user0-ra-stateful-release"
...
type Classification = {
  ...
  readonly user0LaStatefulReleaseGrossPass: string;
  readonly user0RaStatefulReleaseGrossPass: string;
  readonly user0BothStatefulReleaseGrossPass: string;
  readonly user0CurrentMvfTvf: {
    readonly mvfOk: string;
    readonly tvfOk: string;
  };
  readonly statefulReleaseMvfTvf: {
    readonly laStatefulMvfOk: string;
    readonly laStatefulTvfOk: string;
    readonly raStatefulMvfOk: string;
    readonly raStatefulTvfOk: string;
    readonly bothStatefulMvfOk: string;
    readonly bothStatefulTvfOk: string;
  };
  readonly decision:
    | "stateful-release-improves-user0-transfer"
    | "stateful-release-partial-but-static-off-reference-remains-better"
    | "stateful-release-does-not-rescue-user0-transfer"
...
type Evidence = {
  ...
  readonly id: typeof STATEFUL_AVPLANE_RELEASE_PHASE5CV_ID;
  readonly phase: "5CV";
  readonly profile: {
    readonly verificationProfile: "fitFast";
    readonly morphologyProfileId: "normal_sinus_default";
    readonly pointSource: "targeted-normal-preload-contractility-smoke";
    readonly diagnosticQuestion:
      "whether stateful asymmetric LandAtrial AV-plane release improves all-chamber user0 accepted-complementarity transfer residuals without static gain-off adoption";
  };
...
export function buildStatefulAvPlaneReleasePhase5CVEvidence(): Evidence {
  const slowRelease = {
  ...
  const variants = [
    ...
    acceptedUser0Variant(
      "accepted-av-complementarity2-user0-la-stateful-release",
      "Current user0 with LA stateful asymmetric AV-plane release",
      { LA: slowRelease },
    ),
    acceptedUser0Variant(
      "accepted-av-complementarity2-user0-ra-stateful-release",
      "Current user0 with RA stateful asymmetric AV-plane release",
      { RA: slowRelease },
    ),
    acceptedUser0Variant(
      "accepted-av-complementarity2-user0-both-stateful-release",
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: STATEFUL_AVPLANE_RELEASE_PHASE5CV_ID,
    phase: "5CV",
    profile: {
      verificationProfile: "fitFast",
      morphologyProfileId: "normal_sinus_default",
      pointSource: "targeted-normal-preload-contractility-smoke",
      diagnosticQuestion:
        "whether stateful asymmetric LandAtrial AV-plane release improves all-chamber user0 accepted-complementarity transfer residuals without static gain-off adoption",
    },
...
function acceptedLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    experimentalOptions: withAcceptedComplementarity(resolved.experimentalOptions, "phase5cv-accepted-legacy-atria"),
  };
}
...
function acceptedUser0Variant(
  ...
  return {
    ...
    experimentalOptions: withAcceptedComplementarity(experimentalOptions, `phase5cv-${id}`),
  };
}
...
function classify(summaries: readonly VariantSummary[], results: readonly PointResult[]): Classification {
  ...
  const laStateful = requiredSummary(summaries, "accepted-av-complementarity2-user0-la-stateful-release");
  const raStateful = requiredSummary(summaries, "accepted-av-complementarity2-user0-ra-stateful-release");
  ...
  const bestStatefulGross = Math.max(laStateful.grossPassCount, raStateful.grossPassCount, bothStateful.grossPassCount);
  const bestStatefulMvfTvf = Math.max(
    laStateful.mvfOkCount + laStateful.tvfOkCount,
    raStateful.mvfOkCount + raStateful.tvfOkCount,
  ...
  return {
    ...
    user0LaStatefulReleaseGrossPass: `${laStateful.grossPassCount}/${POINTS.length}`,
    user0RaStatefulReleaseGrossPass: `${raStateful.grossPassCount}/${POINTS.length}`,
    ...
    statefulReleaseMvfTvf: {
      laStatefulMvfOk: `${laStateful.mvfOkCount}/${POINTS.length}`,
      laStatefulTvfOk: `${laStateful.tvfOkCount}/${POINTS.length}`,
      raStatefulMvfOk: `${raStateful.mvfOkCount}/${POINTS.length}`,
      raStatefulTvfOk: `${raStateful.tvfOkCount}/${POINTS.length}`,
    ...
    decision: legacy.grossPassCount < Math.ceil(0.5 * POINTS.length)
      ...
      : missingReadback
        ? "configuration-or-readback-gap"
        : bestStatefulGross > currentGross || bestStatefulMvfTvf > currentMvfTvf
          ? (
            staticOff.grossPassCount > bestStatefulGross
              ? "stateful-release-partial-but-static-off-reference-remains-better"
              : "stateful-release-improves-user0-transfer"
          )
          : "stateful-release-does-not-rescue-user0-transfer",
    notes: [
      ...
      `LA stateful-release causes: ${formatCounts(laStateful.causeCounts)}.`,
      `RA stateful-release causes: ${formatCounts(raStateful.causeCounts)}.`,
      ...
      `Both stateful-release residual notes: ${residualNotes(results, bothStateful.variantId).join(" | ") || "none"}.`,
    ],
  };
}
...
function recommendedNext(classification: Classification): readonly string[] {
  ...
  if (classification.decision === "stateful-release-improves-user0-transfer") {
    return [
      "carry stateful AV-plane release timing into the next user0 transfer experiment without claiming atrial physiology acceptance",
      "preserve accepted-boundary complementarity and morphology checker gates; do not retune Land/source/root/qDot",
    ];
  }
  if (classification.decision === "stateful-release-partial-but-static-off-reference-remains-better") {
    return [
      "treat AV-plane release timing as a contributor, but do not adopt this stateful candidate because static-off reference remains stronger",
  ...
  return [
    "stateful AV-plane release does not rescue user0 transfer; focus next on atrial pressure waveform and accepted AV boundary residuals before LandAtrial tuning",
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CV variant summary ${id}.`);
  return summary;
}
...
export function writeStatefulAvPlaneReleasePhase5CVEvidence(): Evidence {
  const evidence = buildStatefulAvPlaneReleasePhase5CVEvidence();
  const outPath = path.resolve(process.cwd(), STATEFUL_AVPLANE_RELEASE_PHASE5CV_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeStatefulAvPlaneReleasePhase5CVEvidence();
