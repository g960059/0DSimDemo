...
export const VENTRICULAR_LAND_KINEMATICS_COUPLER_PHASE5BR_ID =
  "ventricular-land-kinematics-coupler-phase5br-result-v1" as const;
export const VENTRICULAR_LAND_KINEMATICS_COUPLER_PHASE5BR_RESULT_PATH =
  "data/myocardium/protocols/ventricular-land-kinematics-coupler-phase5br-result-v1.json" as const;
...
type VariantId =
  ...
  | "lv-rv-stateful-zeta-landatrial-avplane-off"
  | "lv-rv-stateful-zeta-landatrial-avplane-mid";
...
type Evidence = {
  ...
  readonly id: typeof VENTRICULAR_LAND_KINEMATICS_COUPLER_PHASE5BR_ID;
  readonly phase: "5BR";
  readonly profile: {
...
export function buildVentricularLandKinematicsCouplerPhase5BREvidence(): Evidence {
  const variants = [
    legacyVariant(),
    currentUser0Variant(),
    ...
    customVentricularLandVariant({
      id: "lv-rv-stateful-zeta-landatrial-avplane-off",
      label: "LV/RV stateful zeta-drive coupler with LandAtrial AV-plane off",
      hypothesis: "Tests whether residual AV inflow artifacts are driven by LandAtrial AV-plane geometry after zeta stabilization.",
      atrialPath: "LandAtrial",
      lvTauLambdaActSec: null,
      rvTauLambdaActSec: null,
      kinematicsMode: "raw-wall-lambda",
    ...
    customVentricularLandVariant({
      id: "lv-rv-stateful-zeta-landatrial-avplane-mid",
      label: "LV/RV stateful zeta-drive coupler with LandAtrial AV-plane mid gain",
      hypothesis: "Tests whether reducing, rather than removing, AV-plane effective-wall geometry is robust after zeta stabilization.",
      atrialPath: "LandAtrial",
      lvTauLambdaActSec: null,
      rvTauLambdaActSec: null,
      kinematicsMode: "raw-wall-lambda",
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: VENTRICULAR_LAND_KINEMATICS_COUPLER_PHASE5BR_ID,
    phase: "5BR",
    profile: {
    ...
    classification,
    recommendedNext: [
      "adopt the stateful zeta-drive coupler only if it passes the representative morphology envelope without relying on AV-plane gain removal",
      "keep A1/A2 frozen and do not tune LandAtrial by a single combined target-distance score",
      "if LV/RV PV improves but AV inflow remains non-robust, split the next implementation into DynamicValveTransitionV1 and LandAtrial AV-plane/effective-wall release timing",
...
function customVentricularLandVariant(
  ...
  const lvProvider = calciumScaledLand2017LvSourceOnlyProvider(lvInstrumentation, {
    ...
    sourceProviderId: `phase5br-${spec.id}:LV`,
    calciumScale,
  ...
  const rvProvider = calciumScaledLand2017RvSourceOnlyProvider(rvInstrumentation, {
    ...
    kinematicsMode: spec.kinematicsMode ?? "raw-wall-lambda",
    velocityLengthCouplingMode: spec.velocityLengthCouplingMode ?? "source",
    sourceProviderId: `phase5br-${spec.id}:RV`,
    calciumScale,
    calciumInputMultiplier: "tmax-contractility-user-control",
...
function classify(summaries: readonly VariantSummary[], results: readonly PointResult[]): Classification {
  ...
  const adoptionDecision =
    bestAllChamber?.variantId === "lv-rv-stateful-zeta-landatrial-current"
  ...
  return {
    ...
    notes: [
      ...
      "Adoption requires the current-AV-plane all-chamber candidate to pass the representative preload/afterload/contractility envelope; AV-plane-off rescue alone is insufficient.",
    ],
  };
}
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5BR variant summary ${id}.`);
  return summary;
}
...
function requiredResult(results: readonly PointResult[], variantId: VariantId, pointId: PointId): PointResult {
  ...
  if (!result) throw new Error(`Missing Phase 5BR result ${variantId}/${pointId}.`);
  return result;
}
...
export function writeVentricularLandKinematicsCouplerPhase5BREvidence(): Evidence {
  const evidence = buildVentricularLandKinematicsCouplerPhase5BREvidence();
  const outPath = path.resolve(process.cwd(), VENTRICULAR_LAND_KINEMATICS_COUPLER_PHASE5BR_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeVentricularLandKinematicsCouplerPhase5BREvidence();
