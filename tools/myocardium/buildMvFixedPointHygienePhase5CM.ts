...
export const MV_FIXED_POINT_HYGIENE_PHASE5CM_ID =
  "mv-fixed-point-hygiene-phase5cm-result-v1" as const;
export const MV_FIXED_POINT_HYGIENE_PHASE5CM_RESULT_PATH =
  "data/myocardium/protocols/mv-fixed-point-hygiene-phase5cm-result-v1.json" as const;
...
type Evidence = {
  ...
  readonly id: typeof MV_FIXED_POINT_HYGIENE_PHASE5CM_ID;
  readonly phase: "5CM";
  readonly profile: {
    ...
    readonly candidateMechanism:
      "ModelCore ventricularChamberTransactionStep with adjacent-load-node volume-flow-provider-state fixed-point transaction, TV state-coupled complementarity, and off-by-default MV pressure fixed-point refit candidates";
  };
...
export function buildMvFixedPointHygienePhase5CMEvidence(): Evidence {
  const variants = [
    legacyVariant(),
    currentUser0Variant(),
    lvRvLandLegacyAtriaVariant(),
    mvPressureRefitLegacyAtriaVariant("tv-state-mv-pressure-refit-legacy-atria", ["MV", "TV"], 4, 0.7),
    mvFixedPointRefitLegacyAtriaVariant("tv-state-mv-fixedpoint1-legacy-atria", 1, 1),
    mvFixedPointRefitLegacyAtriaVariant("tv-state-mv-fixedpoint2-legacy-atria", 2, 1),
    mvFixedPointRefitLegacyAtriaVariant("tv-state-mv-fixedpoint3-legacy-atria", 3, 1),
    mvFixedPointRefitLegacyAtriaVariant("tv-state-mv-fixedpoint3-relax07-legacy-atria", 3, 0.7),
    mvFixedPointRefitUser0Variant("tv-state-mv-fixedpoint2-user0", 2, 1),
  ] as const;
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: MV_FIXED_POINT_HYGIENE_PHASE5CM_ID,
    phase: "5CM",
    profile: {
...
function transactionUser0Variant(): VariantSpec {
  ...
  return {
    ...
    ventricularChamberTransaction: {
      mechanismId: "phase5cm-av-inflow-residual-attribution-user0",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      ...
      "phase5cm-av-inflow-residual-attribution-user0",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function transactionLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    closurePath: "lv-rv-land-legacy-atria",
    ventricularChamberTransaction: {
      mechanismId: "phase5cm-av-inflow-residual-attribution-legacy-atria",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
      includeAdjacentLoadNodes: true,
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cm-av-inflow-residual-attribution-legacy-atria",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function withVentricularChamberTransactionStep(
  ...
  if (!providers?.LV || !providers.RV) {
    throw new Error("Phase 5CM AV inflow residual attribution requires LV and RV source providers.");
  }
  return {
    ...experimentalOptions,
...
function requestedPressureRefitConfiguration(
  ...
  if (requested.avValveBoundaryMode === "tv-state-coupled-mv-pressure-fixedpoint-refit") {
    return {
...
function effectivePressureRefitConfiguration(
  ...
  if (effective.avValveBoundaryMode === "tv-state-coupled-mv-pressure-fixedpoint-refit") {
    return {
...
function classify(
  ...
  const transactionCandidates = [
    ...
    requiredSummary(summaries, "tv-state-mv-fixedpoint1-legacy-atria"),
    requiredSummary(summaries, "tv-state-mv-fixedpoint2-legacy-atria"),
    requiredSummary(summaries, "tv-state-mv-fixedpoint3-legacy-atria"),
    requiredSummary(summaries, "tv-state-mv-fixedpoint3-relax07-legacy-atria"),
    requiredSummary(summaries, "tv-state-mv-fixedpoint2-user0"),
  ];
  const bestTransaction = bestSummary(transactionCandidates);
  const user0Transfer = requiredSummary(summaries, "tv-state-mv-fixedpoint2-user0");
  ...
  const partialMvFixedPointRefitPositive =
    (bestTransaction.variantId.includes("mv-pressure-refit") || bestTransaction.variantId.includes("mv-fixedpoint"))
  ...
  return {
    ...
    notes: [
      ...
      `Fixed-point user0 transfer gross pass count: ${user0Transfer.grossPassCount}/${POINTS.length}.`,
      `Fixed-point user0 transfer residual causes: ${formatCounts(user0Transfer.residualCauseCounts)}.`,
...
function recommendedNext(classification: Classification): readonly string[] {
  if (classification.decision === "configuration-integrity-failed") {
    return [
      ...
      "fix ModelCore normalization or the Phase 5CM runner labels, then rerun the same 8-point morphology envelope",
  ...
  if (classification.decision === "legacy-atria-envelope-pass-user0-transfer-blocked") {
    return [
      ...
      "carry tv-state-mv-fixedpoint2-legacy-atria forward as the first 8/8 LV/RV PV plus MVF/TVF morphology-envelope component evidence",
  ...
  if (classification.decision === "partial-positive-mv-fixedpoint-refit") {
    return [
      ...
      "if fixed-point refit improves over Phase 5CK one-pass pressure refit, carry it forward as diagnostic component evidence and rerun a transfer-focused all-chamber user0 surface",
      "if fixed-point refit still leaves a single low-preload MV residual, stop pointwise MV refit escalation and return to a broader chamber pressure-adapter contract redesign",
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CM variant summary ${id}.`);
  return summary;
}
...
function requiredResult(results: readonly PointResult[], variantId: VariantId, pointId: PointId): PointResult {
  ...
  if (!result) throw new Error(`Missing Phase 5CM result ${variantId}/${pointId}.`);
  return result;
}
...
export function writeMvFixedPointHygienePhase5CMEvidence(): Evidence {
  const evidence = buildMvFixedPointHygienePhase5CMEvidence();
  const outPath = path.resolve(process.cwd(), MV_FIXED_POINT_HYGIENE_PHASE5CM_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeMvFixedPointHygienePhase5CMEvidence();
