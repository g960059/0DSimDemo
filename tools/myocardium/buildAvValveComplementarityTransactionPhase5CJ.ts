...
export const AV_VALVE_COMPLEMENTARITY_TRANSACTION_PHASE5CJ_ID =
  "av-valve-complementarity-transaction-phase5cj-result-v1" as const;
export const AV_VALVE_COMPLEMENTARITY_TRANSACTION_PHASE5CJ_RESULT_PATH =
  "data/myocardium/protocols/av-valve-complementarity-transaction-phase5cj-result-v1.json" as const;
...
type VariantId =
  ...
  | "state-coupled-legacy-atria"
  | "state-coupled-relax09-legacy-atria"
  | "state-coupled-mv-only-legacy-atria"
...
type VariantSpec = {
  ...
  readonly ventricularChamberTransaction: null | {
    ...
    readonly avValveBoundaryMode: "current-diode" | "state-coupled-complementarity";
...
type AvValveComplementarityTransaction = {
...
type PointResult = {
  ...
  readonly avInflowResiduals: Readonly<Record<"MV" | "TV", AvValveComplementarityTransaction>> | null;
...
type Classification = {
  ...
  readonly decision:
    ...
    | "partial-positive-tv-state-coupled-mv-residual"
...
type Evidence = {
  ...
  readonly id: typeof AV_VALVE_COMPLEMENTARITY_TRANSACTION_PHASE5CJ_ID;
  readonly phase: "5CJ";
  readonly profile: {
    ...
    readonly candidateMechanism:
      "ModelCore ventricularChamberTransactionStep with adjacent-load-node volume-flow-provider-state fixed-point transaction and off-by-default MV/TV state-coupled-complementarity AV valve boundary candidates";
  };
...
export function buildAvValveComplementarityTransactionPhase5CJEvidence(): Evidence {
  const variants = [
    legacyVariant(),
    currentUser0Variant(),
    ...
    stateCoupledLegacyAtriaVariant("state-coupled-legacy-atria", ["MV", "TV"], 4, 0.7),
    stateCoupledLegacyAtriaVariant("state-coupled-relax09-legacy-atria", ["MV", "TV"], 4, 0.9),
    stateCoupledLegacyAtriaVariant("state-coupled-mv-only-legacy-atria", ["MV"], 4, 0.7),
    stateCoupledLegacyAtriaVariant("state-coupled-tv-only-legacy-atria", ["TV"], 4, 0.7),
    stateCoupledUser0Variant("state-coupled-user0", ["MV", "TV"], 4, 0.7),
  ] as const;
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: AV_VALVE_COMPLEMENTARITY_TRANSACTION_PHASE5CJ_ID,
    phase: "5CJ",
    profile: {
...
function transactionUser0Variant(): VariantSpec {
  ...
  return {
    ...
    ventricularChamberTransaction: {
      mechanismId: "phase5cj-av-inflow-residual-attribution-user0",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      ...
      "phase5cj-av-inflow-residual-attribution-user0",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function transactionLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    closurePath: "lv-rv-land-legacy-atria",
    ventricularChamberTransaction: {
      mechanismId: "phase5cj-av-inflow-residual-attribution-legacy-atria",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
      includeAdjacentLoadNodes: true,
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cj-av-inflow-residual-attribution-legacy-atria",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function stateCoupledUser0Variant(
  id: "state-coupled-user0",
  targetValves: readonly ("MV" | "TV")[],
  iterations: number,
  relaxation: number,
): VariantSpec {
  ...
  return {
    id,
    label: "Current user-0 closure with state-coupled MV+TV AV valve complementarity",
    hypothesis: "Checks whether the best legacy-atria state-coupled AV valve candidate transfers back to the current all-chamber LandAtrial default closure.",
    closurePath: "current-user0-all-chamber-landatrial",
    ventricularChamberTransaction: transactionDescriptor(id, targetValves, iterations, relaxation),
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
...
function transactionDescriptor(
  ...
  return {
    ...
    chambers: ["LV", "RV"],
    includeAdjacentLoadNodes: true,
    avValveBoundaryMode: "state-coupled-complementarity",
...
function withVentricularChamberTransactionStep(
  ...
  mechanismId: string,
  avValveBoundaryMode: "current-diode" | "state-coupled-complementarity",
  targetValves: readonly ("MV" | "TV")[],
  iterations: number,
  relaxation: number,
  ...
  if (!providers?.LV || !providers.RV) {
    throw new Error("Phase 5CJ AV inflow residual attribution requires LV and RV source providers.");
  }
  return {
    ...experimentalOptions,
...
function countResidualCauses(residuals: readonly AvValveComplementarityTransaction[]): ResidualCauseCounts {
...
): AvValveComplementarityTransaction {
...
function unmeasuredResidual(valve: "MV" | "TV", reason: string): AvValveComplementarityTransaction {
  return {
    valve,
    measured: false,
    morphologyOk: false,
...
function classify(
  ...
  const transactionCandidates = [
    requiredSummary(summaries, "transaction-user0"),
    requiredSummary(summaries, "transaction-legacy-atria"),
    requiredSummary(summaries, "state-coupled-legacy-atria"),
    requiredSummary(summaries, "state-coupled-relax09-legacy-atria"),
    requiredSummary(summaries, "state-coupled-mv-only-legacy-atria"),
    requiredSummary(summaries, "state-coupled-tv-only-legacy-atria"),
    requiredSummary(summaries, "state-coupled-user0"),
  ];
  ...
  const partialTvStateCoupledPositive =
    bestTransaction.variantId === "state-coupled-tv-only-legacy-atria"
    && bestTransaction.grossPassCount >= 6
    && bestTransaction.lvPvOkCount === POINTS.length
    && bestTransaction.rvPvOkCount === POINTS.length
    && bestTransaction.tvfOkCount === POINTS.length
    && bestTransaction.mvfOkCount >= 6
    && bestTransaction.mvResidualCauseCounts["valve-diode-or-qdot-contamination"] > 0;
  return {
...
function recommendedNext(classification: Classification): readonly string[] {
  ...
  if (classification.decision === "partial-positive-tv-state-coupled-mv-residual") {
    return [
      "do not adopt the state-coupled TV-only candidate as runtime/default because the full envelope still fails 2/8 and the user0 transfer remains 0/8",
      "carry the TV-only state-coupled valve-state/flow ordering forward as a diagnostic component because it preserves LV/RV PV 8/8 and TVF 8/8 with gross 6/8 under legacy atria",
      "focus the next implementation surface on the two remaining MV residual points (low-preload HR75 and contractility-low HR75) with a left-sided chamber pressure/valve-load complementarity solve, not a symmetric MV+TV state-coupling or parameter/tau sweep",
  ...
  const pressureContractDominant =
    ...
    >= causes["valve-diode-or-qdot-contamination"] + causes["valve-state-chatter"];
  const next: string[] = [
    "do not adopt the adjacent-load-node or state-coupled-complementarity ventricularChamberTransactionStep candidates unless they pass the full morphology envelope",
  ];
  if (valveBoundaryDominant) {
    next.push("state-coupling the AV valve opening before flow/loss evaluation is still not enough if the full morphology envelope remains below acceptance; move the next surface toward a chamber pressure/valve-load complementarity solve or pressure-adapter contract redesign");
  }
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CJ variant summary ${id}.`);
  return summary;
}
...
function requiredResult(results: readonly PointResult[], variantId: VariantId, pointId: PointId): PointResult {
  ...
  if (!result) throw new Error(`Missing Phase 5CJ result ${variantId}/${pointId}.`);
  return result;
}
...
export function writeAvValveComplementarityTransactionPhase5CJEvidence(): Evidence {
  const evidence = buildAvValveComplementarityTransactionPhase5CJEvidence();
  const outPath = path.resolve(process.cwd(), AV_VALVE_COMPLEMENTARITY_TRANSACTION_PHASE5CJ_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeAvValveComplementarityTransactionPhase5CJEvidence();
