...
export const LANDATRIAL_TARGET_SCORING_PHASE5BI_ID =
  "landatrial-target-scoring-phase5bi-result-v1";
export const LANDATRIAL_TARGET_SCORING_PHASE5BI_RESULT_PATH =
...
type CandidateId =
  ...
  | "promoted-landatrial-shadow-la-ra"
  | "promoted-landatrial-shadow-no-avplane-ablation";
type PointId =
  | "normal-hr75"
  | "low-preload-hr75"
  | "high-preload-hr75"
...
type CandidateSpec = {
  ...
  readonly disableAtrialAvPlane: boolean;
...
type Evidence = {
  ...
  readonly id: typeof LANDATRIAL_TARGET_SCORING_PHASE5BI_ID;
  readonly phase: "Phase 5BI";
  readonly claimBoundary: "landatrial-shadow-target-scoring-no-default-no-physiology-acceptance";
  ...
  readonly avPlaneAblationDelta: {
    readonly baseCandidateId: "promoted-landatrial-shadow-la-ra";
    readonly ablationCandidateId: "promoted-landatrial-shadow-no-avplane-ablation";
    readonly deltaMeanCombinedTargetDistanceBaseMinusAblation: number | null;
    readonly deltaBothAtriaRawReadableCountBaseMinusAblation: number;
    readonly interpretation: string;
  };
  readonly runs: readonly Run[];
  readonly summary: {
    readonly diagnosticStatus:
      | "landatrial-shadow-scored-gaps-recorded"
      | "landatrial-shadow-scored-health-or-output-blocked"
      | "landatrial-shadow-scored-runtime-error";
...
const CANDIDATES: readonly CandidateSpec[] = [
  {
    ...
    disableAtrialAvPlane: false,
    rationale: "Current LV+RV Land plus sourced root/Zc default with the existing atrial active-stress path.",
  },
  {
    id: "promoted-landatrial-shadow-la-ra",
    usesLandAtrialShadow: true,
    disableAtrialAvPlane: false,
    rationale: "Promoted Phase 5BH LA+RA LandAtrial shadow pack with direct wall/AV-plane readbacks.",
  },
  {
    id: "promoted-landatrial-shadow-no-avplane-ablation",
    usesLandAtrialShadow: true,
    disableAtrialAvPlane: true,
    rationale: "Same promoted LandAtrial shadow pack with LA/RA active.avPlaneGainMl set to zero to isolate AV-plane contribution.",
  },
] as const;

export function buildLandAtrialTargetScoringPhase5BIEvidence(): Evidence {
  ...
  const avPlaneAblationDelta = computeAvPlaneAblationDelta(candidateSummaries);
  const evidenceWithoutHash: Omit<Evidence, "normalizedSha256"> = {
    schemaVersion: 1,
    id: LANDATRIAL_TARGET_SCORING_PHASE5BI_ID,
    phase: "Phase 5BI",
    claimBoundary: "landatrial-shadow-target-scoring-no-default-no-physiology-acceptance",
    ...
    points: POINTS,
    candidateSummaries,
    avPlaneAblationDelta,
    runs,
    summary: {
      diagnosticStatus,
      bestCandidateByCombinedTargetDistanceId: best?.candidateId ?? null,
      currentInterpretation: interpretation(candidateSummaries, best, diagnosticStatus, avPlaneAblationDelta),
      recommendedNext: recommendedNext(diagnosticStatus, avPlaneAblationDelta),
      blockers: [
        "Phase 5BI scores the LandAtrial shadow path; it does not accept atrial physiology or flip all chambers to runtime default.",
...
function runPoint(candidate: CandidateSpec, point: PointSpec): Run {
  try {
    const params: Partial<CoreRuntimeParams> = {
      ...defaultParams(),
      HR: point.HR,
      nodeOverrides: candidate.disableAtrialAvPlane
        ? {
    ...
    const laInstrumentation = candidate.usesLandAtrialShadow
      ? createModelCoreLand2017LvSourceProviderInstrumentation()
      : null;
    const raInstrumentation = candidate.usesLandAtrialShadow
      ? createModelCoreLand2017LvSourceProviderInstrumentation()
      : null;
...
function createCandidateProviders(
  ...
  if (!candidate.usesLandAtrialShadow) return {};
...
function classifyDiagnosticStatus(
  ...
  if (summaries.some((summary) => summary.runtimeErrorPointIds.length > 0)) {
    return "landatrial-shadow-scored-runtime-error";
  }
  const promoted = summaries.find((summary) => summary.candidateId === "promoted-landatrial-shadow-la-ra");
  if (!promoted || promoted.healthOkPointIds.length < POINTS.length || promoted.outputPreservedPointIds.length < POINTS.length) {
    return "landatrial-shadow-scored-health-or-output-blocked";
  }
  return "landatrial-shadow-scored-gaps-recorded";
}
...
function computeAvPlaneAblationDelta(
  summaries: readonly CandidateSummary[],
): Evidence["avPlaneAblationDelta"] {
  const base = summaries.find((summary) => summary.candidateId === "promoted-landatrial-shadow-la-ra");
  const ablation = summaries.find((summary) =>
    summary.candidateId === "promoted-landatrial-shadow-no-avplane-ablation");
  const distanceDelta = base?.meanCombinedTargetDistance != null && ablation?.meanCombinedTargetDistance != null
    ? round(base.meanCombinedTargetDistance - ablation.meanCombinedTargetDistance)
    : null;
  const readableDelta = (base?.bothAtriaRawReadablePointIds.length ?? 0)
    - (ablation?.bothAtriaRawReadablePointIds.length ?? 0);
  const interpretation = distanceDelta == null
    ? "AV-plane ablation delta is unavailable because one candidate did not produce finite target distances."
    : distanceDelta < 0
      ? "Direct target distance is lower with AV-plane enabled than with the no-AV-plane ablation."
      : distanceDelta > 0
        ? "Direct target distance is lower with AV-plane disabled in this broad scoring artifact."
        : "Direct target distance is unchanged by this AV-plane ablation.";
  return {
    baseCandidateId: "promoted-landatrial-shadow-la-ra",
    ablationCandidateId: "promoted-landatrial-shadow-no-avplane-ablation",
    deltaMeanCombinedTargetDistanceBaseMinusAblation: distanceDelta,
    deltaBothAtriaRawReadableCountBaseMinusAblation: readableDelta,
    interpretation,
  };
}
...
function interpretation(
  ...
  avPlaneAblationDelta: Evidence["avPlaneAblationDelta"],
): string {
  const promoted = summaries.find((summary) => summary.candidateId === "promoted-landatrial-shadow-la-ra");
  return [
    "Phase 5BI freezes A1/A2 and scores the promoted LandAtrial shadow baseline against sourced atrial volume-function targets plus direct wall-lambda strain targets.",
    `Promoted LandAtrial health-ok=${promoted?.healthOkPointIds.length ?? 0}/${POINTS.length}, output-preserved=${promoted?.outputPreservedPointIds.length ?? 0}/${POINTS.length}, Land solve failures=${promoted?.totalLandSolveFailureCount ?? "missing"}.`,
    `Best combined-distance candidate by this diagnostic artifact: ${best?.candidateId ?? "none"}.`,
    `AV-plane ablation: ${avPlaneAblationDelta.interpretation}`,
...
function recommendedNext(
  ...
  if (diagnosticStatus !== "landatrial-shadow-scored-gaps-recorded") {
    return [
  ...
  return [
    "use this direct target score to choose the next LandAtrial atrial parameter-pack and AV-plane/effective-geometry calibration phase",
    avPlaneAblationDelta.deltaMeanCombinedTargetDistanceBaseMinusAblation != null
      && avPlaneAblationDelta.deltaMeanCombinedTargetDistanceBaseMinusAblation > 0
      ? "inspect whether the current AV-plane gain/geometry worsens direct target distance before increasing AV-plane coupling"
      : "treat AV-plane readbacks as live calibration signals rather than tuning A1/A2 bridge gains",
...
function writeEvidence(): Evidence {
  const evidence = buildLandAtrialTargetScoringPhase5BIEvidence();
  const outPath = path.resolve(process.cwd(), LANDATRIAL_TARGET_SCORING_PHASE5BI_RESULT_PATH);
...
if (isDirectRun) {
  ...
  console.log(JSON.stringify({
    ...
    promotedLandAtrialHealthOkPointIds: evidence.candidateSummaries
      .find((summary) => summary.candidateId === "promoted-landatrial-shadow-la-ra")
      ?.healthOkPointIds ?? [],
    promotedLandAtrialBothAtriaRawReadablePointIds: evidence.candidateSummaries
      .find((summary) => summary.candidateId === "promoted-landatrial-shadow-la-ra")
      ?.bothAtriaRawReadablePointIds ?? [],
    avPlaneAblationDelta: evidence.avPlaneAblationDelta,
  }, null, 2));
}
