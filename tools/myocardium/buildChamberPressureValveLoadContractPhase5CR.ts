...
export const CHAMBER_PRESSURE_VALVE_LOAD_CONTRACT_PHASE5CR_ID =
  "chamber-pressure-valve-load-contract-phase5cr-result-v1" as const;
export const CHAMBER_PRESSURE_VALVE_LOAD_CONTRACT_PHASE5CR_RESULT_PATH =
  "data/myocardium/protocols/chamber-pressure-valve-load-contract-phase5cr-result-v1.json" as const;
...
type VariantId =
  ...
  | "pair-coupled-av-valve-load-legacy-atria"
  | "pair-coupled-av-valve-load-user0";
...
type VariantSpec = {
  ...
  readonly avValveBoundary: null | {
    readonly mode: "accepted-state-av-boundary-fixedpoint" | "accepted-state-av-boundary-pair-fixedpoint";
...
type Classification = {
  ...
  readonly pairLegacyGrossPass: string;
  readonly pairUser0TransferGrossPass: string;
  ...
  readonly decision:
    ...
    | "pair-coupled-valve-load-no-go"
    | "pair-coupled-valve-load-component-signal"
    | "pair-coupled-valve-load-transfer-signal";
  readonly notes: readonly string[];
};

type Evidence = {
  readonly schemaVersion: 1;
  readonly id: typeof CHAMBER_PRESSURE_VALVE_LOAD_CONTRACT_PHASE5CR_ID;
  readonly phase: "5CR";
  readonly profile: {
    ...
    readonly diagnosticQuestion:
      "whether accepted-state pair fixed-point valve-load transaction broadens AV and semilunar flow ownership inside one chamber side";
  };
...
export function buildChamberPressureValveLoadContractPhase5CREvidence(): Evidence {
  const variants = [
    currentUser0Variant(),
    lvRvLandLegacyAtriaVariant(),
    acceptedComplementarityLegacyAtriaVariant(),
    pairCoupledLegacyAtriaVariant(),
    pairCoupledUser0Variant(),
  ] as const;
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: CHAMBER_PRESSURE_VALVE_LOAD_CONTRACT_PHASE5CR_ID,
    phase: "5CR",
    profile: {
...
function acceptedComplementarityLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    experimentalOptions: withAcceptedComplementarity(resolved.experimentalOptions, "phase5cr-accepted-legacy-atria"),
  };
}

function pairCoupledLegacyAtriaVariant(): VariantSpec {
  const resolved = resolveModelCoreRuntimeActiveSource({
    mode: MODELCORE_RUNTIME_LV_RV_LAND_DEFAULT_MODE,
    runtimeParams: DEFAULT_PARAMS,
  });
  return {
    id: "pair-coupled-av-valve-load-legacy-atria",
    label: "LV/RV Land legacy atria with side-coupled AV plus semilunar accepted valve-load fixed point",
    closurePath: "lv-rv-land-legacy-atria",
    avValveBoundary: {
      mode: "accepted-state-av-boundary-pair-fixedpoint",
      targetValves: ["MV", "TV"],
      iterations: 2,
      relaxation: 1,
    },
    experimentalOptions: withPairCoupledValveLoad(resolved.experimentalOptions, "phase5cr-pair-coupled-legacy-atria"),
  };
}

function pairCoupledUser0Variant(): VariantSpec {
  const resolved = resolveModelCoreRuntimeActiveSource({
    mode: MODELCORE_RUNTIME_ALL_CHAMBER_LANDATRIAL_DEFAULT_MODE,
    runtimeParams: DEFAULT_PARAMS,
  });
  return {
    id: "pair-coupled-av-valve-load-user0",
    label: "Current user-0 closure with side-coupled AV plus semilunar accepted valve-load fixed point",
    closurePath: "current-user0-all-chamber-landatrial",
    avValveBoundary: {
      mode: "accepted-state-av-boundary-pair-fixedpoint",
      targetValves: ["MV", "TV"],
    ...
    experimentalOptions: withPairCoupledValveLoad(resolved.experimentalOptions, "phase5cr-pair-coupled-user0"),
  };
}
...
function withPairCoupledValveLoad(
  experimentalOptions: ModelCoreExperimentalOptions,
  mechanismId: string,
): ModelCoreExperimentalOptions {
  return {
...
function classify(
  ...
  const pairLegacy = requiredSummary(summaries, "pair-coupled-av-valve-load-legacy-atria");
  const pairUser0 = requiredSummary(summaries, "pair-coupled-av-valve-load-user0");
  const configOk = audits.every((audit) =>
    audit.requestedIterations == null
    || audit.matches
  );
  const pairLegacyResidualNotes = failedResidualNotes(results, pairLegacy.variantId);
  return {
    ...
    pairLegacyGrossPass: `${pairLegacy.grossPassCount}/${POINTS.length}`,
    pairUser0TransferGrossPass: `${pairUser0.grossPassCount}/${POINTS.length}`,
    acceptedLegacyDiagnostics: {
      acceptedDiagnosticsAvailableCount: pairLegacy.acceptedDiagnosticsAvailableCount,
      diagnosticMismatchCount: pairLegacy.diagnosticMismatchCount,
      standardCauseCounts: pairLegacy.standardCauseCounts,
      acceptedCauseCounts: pairLegacy.acceptedCauseCounts,
    },
    decision: !configOk
      ? "configuration-integrity-failed"
      : pairLegacy.grossPassCount >= 7 && pairUser0.grossPassCount >= 7
        ? "pair-coupled-valve-load-transfer-signal"
        : pairLegacy.grossPassCount > acceptedLegacy.grossPassCount
          ? "pair-coupled-valve-load-component-signal"
          : "pair-coupled-valve-load-no-go",
    notes: [
      `Pair-coupled legacy residual notes: ${pairLegacyResidualNotes.join(" | ") || "none"}.`,
      `5CO accepted-complementarity legacy gross: ${acceptedLegacy.grossPassCount}/${POINTS.length}.`,
      `5CR pair-coupled legacy gross: ${pairLegacy.grossPassCount}/${POINTS.length}.`,
      `5CR pair-coupled user0 transfer gross: ${pairUser0.grossPassCount}/${POINTS.length}.`,
      `Pair-coupled legacy diagnostic mismatches: ${pairLegacy.diagnosticMismatchCount}.`,
      `Pair-coupled user0 diagnostic mismatches: ${pairUser0.diagnosticMismatchCount}.`,
      `Pair-coupled legacy accepted-boundary causes: ${formatCounts(pairLegacy.acceptedCauseCounts)}.`,
      `Pair-coupled user0 accepted-boundary causes: ${formatCounts(pairUser0.acceptedCauseCounts)}.`,
    ],
  };
}
...
function recommendedNext(classification: Classification): readonly string[] {
  ...
  if (classification.decision === "pair-coupled-valve-load-transfer-signal") {
    return [
      "treat the pair-coupled AV plus semilunar valve-load transaction as component evidence only until visual review and a follow-up all-chamber transfer artifact pass",
      "do not wire runtime/default before preserving accepted-boundary diagnostics and output preservation in a focused PR",
    ];
  }
  if (classification.decision === "pair-coupled-valve-load-component-signal") {
    return [
      "carry pair-coupled inlet/outlet valve-load ownership forward as partial component evidence",
      "attribute remaining user0 transfer residuals before any LandAtrial tuning or runtime/default wiring",
    ];
  }
  return [
    "pair-coupled AV plus semilunar valve-load ownership does not beat the 5CO accepted-complementarity baseline",
    "move the next surface beyond side-local valve-load fixed point, toward a chamber pressure/source-state/valve-load residual contract",
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CR variant summary ${id}.`);
  return summary;
}
...
export function writeChamberPressureValveLoadContractPhase5CREvidence(): Evidence {
  const evidence = buildChamberPressureValveLoadContractPhase5CREvidence();
  const outPath = path.resolve(process.cwd(), CHAMBER_PRESSURE_VALVE_LOAD_CONTRACT_PHASE5CR_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeChamberPressureValveLoadContractPhase5CREvidence();
