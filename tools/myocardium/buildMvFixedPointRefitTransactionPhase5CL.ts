...
export const MV_FIXED_POINT_REFIT_TRANSACTION_PHASE5CL_ID =
  "mv-fixed-point-refit-transaction-phase5cl-result-v1" as const;
export const MV_FIXED_POINT_REFIT_TRANSACTION_PHASE5CL_RESULT_PATH =
  "data/myocardium/protocols/mv-fixed-point-refit-transaction-phase5cl-result-v1.json" as const;
...
type Evidence = {
  ...
  readonly id: typeof MV_FIXED_POINT_REFIT_TRANSACTION_PHASE5CL_ID;
  readonly phase: "5CL";
  readonly profile: {
...
export function buildMvFixedPointRefitTransactionPhase5CLEvidence(): Evidence {
  const variants = [
    legacyVariant(),
    currentUser0Variant(),
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: MV_FIXED_POINT_REFIT_TRANSACTION_PHASE5CL_ID,
    phase: "5CL",
    profile: {
...
function transactionUser0Variant(): VariantSpec {
  ...
  return {
    ...
    ventricularChamberTransaction: {
      mechanismId: "phase5cl-av-inflow-residual-attribution-user0",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      ...
      "phase5cl-av-inflow-residual-attribution-user0",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function transactionLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    closurePath: "lv-rv-land-legacy-atria",
    ventricularChamberTransaction: {
      mechanismId: "phase5cl-av-inflow-residual-attribution-legacy-atria",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
      includeAdjacentLoadNodes: true,
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cl-av-inflow-residual-attribution-legacy-atria",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function withVentricularChamberTransactionStep(
  ...
  if (!providers?.LV || !providers.RV) {
    throw new Error("Phase 5CL AV inflow residual attribution requires LV and RV source providers.");
  }
  return {
    ...experimentalOptions,
...
function classify(
  ...
  return {
    ...
    decision: user0TransferCleanEnough
      ? "residuals-clean-enough-for-runtime-shadow"
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CL variant summary ${id}.`);
  return summary;
}
...
function requiredResult(results: readonly PointResult[], variantId: VariantId, pointId: PointId): PointResult {
  ...
  if (!result) throw new Error(`Missing Phase 5CL result ${variantId}/${pointId}.`);
  return result;
}
...
export function writeMvFixedPointRefitTransactionPhase5CLEvidence(): Evidence {
  const evidence = buildMvFixedPointRefitTransactionPhase5CLEvidence();
  const outPath = path.resolve(process.cwd(), MV_FIXED_POINT_REFIT_TRANSACTION_PHASE5CL_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeMvFixedPointRefitTransactionPhase5CLEvidence();
