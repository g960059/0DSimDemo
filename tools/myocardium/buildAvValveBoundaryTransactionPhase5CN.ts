...
export const AV_VALVE_BOUNDARY_TRANSACTION_PHASE5CN_ID =
  "av-valve-boundary-transaction-phase5cn-result-v1" as const;
export const AV_VALVE_BOUNDARY_TRANSACTION_PHASE5CN_RESULT_PATH =
  "data/myocardium/protocols/av-valve-boundary-transaction-phase5cn-result-v1.json" as const;
...
type Evidence = {
  ...
  readonly id: typeof AV_VALVE_BOUNDARY_TRANSACTION_PHASE5CN_ID;
  readonly phase: "5CN";
  readonly profile: {
    ...
    readonly candidateMechanism:
      "ModelCore ventricularChamberTransactionStep with adjacent-load-node volume-flow-provider-state transaction and off-by-default accepted-state AV valve boundary candidates";
  };
...
export function buildAvValveBoundaryTransactionPhase5CNEvidence(): Evidence {
  const variants = [
    legacyVariant(),
    currentUser0Variant(),
    ...
    acceptedAvBoundaryUser0Variant(),
  ] as const;
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: AV_VALVE_BOUNDARY_TRANSACTION_PHASE5CN_ID,
    phase: "5CN",
    profile: {
...
function transactionUser0Variant(): VariantSpec {
  ...
  return {
    ...
    ventricularChamberTransaction: {
      mechanismId: "phase5cn-av-inflow-residual-attribution-user0",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      ...
      "phase5cn-av-inflow-residual-attribution-user0",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function transactionLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    closurePath: "lv-rv-land-legacy-atria",
    ventricularChamberTransaction: {
      mechanismId: "phase5cn-av-inflow-residual-attribution-legacy-atria",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
      includeAdjacentLoadNodes: true,
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cn-av-inflow-residual-attribution-legacy-atria",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function acceptedAvBoundaryLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    label: "LV/RV Land legacy atria with accepted-state AV valve boundary",
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cn-accepted-av-boundary-legacy-atria",
      "accepted-state-av-boundary",
      ["MV", "TV"],
      4,
      0.7,
...
function acceptedAvBoundaryUser0Variant(): VariantSpec {
  ...
  return {
    ...
    label: "Current user-0 closure with accepted-state AV valve boundary",
    hypothesis: "Checks whether the accepted-state AV valve boundary transfers to all-chamber LandAtrial without LandAtrial tuning.",
    closurePath: "current-user0-all-chamber-landatrial",
    ventricularChamberTransaction: transactionDescriptor(
      "accepted-av-boundary-user0",
      ["MV", "TV"],
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cn-accepted-av-boundary-user0",
      "accepted-state-av-boundary",
      ["MV", "TV"],
      4,
      0.7,
...
function withVentricularChamberTransactionStep(
  ...
  if (!providers?.LV || !providers.RV) {
    throw new Error("Phase 5CN AV inflow residual attribution requires LV and RV source providers.");
  }
  return {
    ...experimentalOptions,
...
function classify(
  ...
  const transactionCandidates = [
    ...
    requiredSummary(summaries, "accepted-av-boundary-user0"),
  ];
  const bestTransaction = bestSummary(transactionCandidates);
  const user0Transfer = requiredSummary(summaries, "accepted-av-boundary-user0");
  ...
  return {
    ...
    notes: [
      ...
      `Accepted-boundary user0 transfer gross pass count: ${user0Transfer.grossPassCount}/${POINTS.length}.`,
      `Accepted-boundary user0 transfer residual causes: ${formatCounts(user0Transfer.residualCauseCounts)}.`,
...
function recommendedNext(classification: Classification): readonly string[] {
  if (classification.decision === "configuration-integrity-failed") {
    return [
      ...
      "fix ModelCore normalization or the Phase 5CN runner labels, then rerun the same 8-point morphology envelope",
  ...
  if (valveBoundaryDominant) {
    next.push("accepted-state AV valve boundary improves the legacy-atria surface only partially if the full morphology envelope remains below acceptance; move the next surface toward a broader chamber pressure/valve-load complementarity solve or pressure-adapter contract redesign");
  }
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CN variant summary ${id}.`);
  return summary;
}
...
function requiredResult(results: readonly PointResult[], variantId: VariantId, pointId: PointId): PointResult {
  ...
  if (!result) throw new Error(`Missing Phase 5CN result ${variantId}/${pointId}.`);
  return result;
}
...
export function writeAvValveBoundaryTransactionPhase5CNEvidence(): Evidence {
  const evidence = buildAvValveBoundaryTransactionPhase5CNEvidence();
  const outPath = path.resolve(process.cwd(), AV_VALVE_BOUNDARY_TRANSACTION_PHASE5CN_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeAvValveBoundaryTransactionPhase5CNEvidence();
