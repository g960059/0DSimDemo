...
import { activeSourcePressureContractV2Provider } from "@/engine/myocardium/activeSourcePressureContractV2";
import {
...
export const PRESSURE_LOAD_CONTRACT_PHASE5CP_ID =
  "pressure-load-contract-phase5cp-result-v1" as const;
export const PRESSURE_LOAD_CONTRACT_PHASE5CP_RESULT_PATH =
  "data/myocardium/protocols/pressure-load-contract-phase5cp-result-v1.json" as const;
...
type VariantId =
  ...
  | "bounded-geometry-gain115-legacy-atria"
  | "bounded-geometry-gain125-legacy-atria"
  | "bounded-geometry-gain115-accepted-complementarity2-legacy-atria"
  | "bounded-geometry-gain115-accepted-complementarity2-user0";
...
type VariantSpec = {
  ...
  readonly activeSourcePressureContractV2?: null | {
    readonly mode: "bounded-geometry-active-pressure-v1";
...
type Classification = {
  ...
  readonly bestPressureContractVariant: string;
  readonly bestPressureContractResidualCauseCounts: ResidualCauseCounts;
  readonly user0TransferGrossPass: string;
  readonly decision:
    | "configuration-integrity-failed"
    | "pressure-load-contract-clean-enough-for-runtime-shadow"
    | "legacy-atria-pressure-contract-pass-user0-transfer-blocked"
    | "partial-positive-pressure-load-contract"
...
type Evidence = {
  ...
  readonly id: typeof PRESSURE_LOAD_CONTRACT_PHASE5CP_ID;
  readonly phase: "5CP";
  readonly profile: {
    ...
    readonly candidateMechanism:
      "off-by-default ActiveSourcePressureContractV2 bounded active pressure gain for LV/RV, alone and combined with the Phase 5CO accepted-state AV fixed-point complementarity component";
  };
...
export function buildPressureLoadContractPhase5CPEvidence(): Evidence {
  const variants = [
    currentUser0Variant(),
    lvRvLandLegacyAtriaVariant(),
    acceptedAvComplementarityLegacyAtriaVariant("accepted-av-complementarity2-legacy-atria", 2, 1),
    boundedGeometryLegacyAtriaVariant("bounded-geometry-gain115-legacy-atria", 1.15),
    boundedGeometryLegacyAtriaVariant("bounded-geometry-gain125-legacy-atria", 1.25),
    boundedGeometryAcceptedComplementarityLegacyAtriaVariant(),
    boundedGeometryAcceptedComplementarityUser0Variant(),
  ] as const;
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: PRESSURE_LOAD_CONTRACT_PHASE5CP_ID,
    phase: "5CP",
    profile: {
...
function boundedGeometryLegacyAtriaVariant(
  id: "bounded-geometry-gain115-legacy-atria" | "bounded-geometry-gain125-legacy-atria",
  activeGainCeilingRatio: number,
): VariantSpec {
  const resolved = resolveModelCoreRuntimeActiveSource({
    mode: MODELCORE_RUNTIME_LV_RV_LAND_DEFAULT_MODE,
    runtimeParams: DEFAULT_PARAMS,
  ...
  return {
    id,
    label: `LV/RV Land legacy atria with bounded active pressure geometry gain (${activeGainCeilingRatio}x reference ceiling)`,
    hypothesis: "Tests whether preserving current passive pressure while capping only instantaneous active source-stress-to-pressure gain removes PV dome and AV inflow artifacts without fixing geometry gain entirely.",
    closurePath: "lv-rv-land-legacy-atria",
    ventricularChamberTransaction: null,
    activeSourcePressureContractV2: {
      mode: "bounded-geometry-active-pressure-v1",
      activeGainCeilingRatio,
      chambers: ["LV", "RV"],
    },
    experimentalOptions: withBoundedGeometryV2Providers(
...
function boundedGeometryAcceptedComplementarityLegacyAtriaVariant(): VariantSpec {
  const resolved = resolveModelCoreRuntimeActiveSource({
    mode: MODELCORE_RUNTIME_LV_RV_LAND_DEFAULT_MODE,
    runtimeParams: DEFAULT_PARAMS,
  });
  return {
    id: "bounded-geometry-gain115-accepted-complementarity2-legacy-atria",
    label: "LV/RV Land legacy atria with bounded active pressure geometry gain plus accepted-state AV complementarity",
    hypothesis: "Tests whether the pressure-adapter contract and the best Phase 5CO AV boundary component are complementary rather than individually insufficient.",
    closurePath: "lv-rv-land-legacy-atria",
    ...
    activeSourcePressureContractV2: {
      mode: "bounded-geometry-active-pressure-v1",
      activeGainCeilingRatio: 1.15,
      chambers: ["LV", "RV"],
    },
    experimentalOptions: withVentricularChamberTransactionStep(
      withBoundedGeometryV2Providers(
        resolved.experimentalOptions,
        "phase5cp-bounded-geometry-gain115-accepted-complementarity2-legacy-atria",
        1.15,
      ),
      "phase5cp-bounded-geometry-gain115-accepted-complementarity2-legacy-atria",
      "accepted-state-av-boundary-fixedpoint",
...
function boundedGeometryAcceptedComplementarityUser0Variant(): VariantSpec {
  const resolved = resolveModelCoreRuntimeActiveSource({
    mode: MODELCORE_RUNTIME_ALL_CHAMBER_LANDATRIAL_DEFAULT_MODE,
    runtimeParams: DEFAULT_PARAMS,
  });
  return {
    id: "bounded-geometry-gain115-accepted-complementarity2-user0",
    label: "Current user-0 closure with bounded active pressure geometry gain plus accepted-state AV complementarity",
    hypothesis: "Checks whether the legacy-atria pressure/load contract signal transfers to the current all-chamber LandAtrial default without LandAtrial tuning.",
    closurePath: "current-user0-all-chamber-landatrial",
    ventricularChamberTransaction: transactionDescriptor(
      "bounded-geometry-gain115-accepted-complementarity2-user0",
      ["MV", "TV"],
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      withBoundedGeometryV2Providers(
        resolved.experimentalOptions,
        "phase5cp-bounded-geometry-gain115-accepted-complementarity2-user0",
        1.15,
      ),
      "phase5cp-bounded-geometry-gain115-accepted-complementarity2-user0",
      "accepted-state-av-boundary-fixedpoint",
...
function withBoundedGeometryV2Providers(
  ...
  if (!providers?.LV || !providers.RV) {
    throw new Error("Phase 5CP bounded-geometry V2 requires LV and RV source providers.");
  }
  return {
    ...experimentalOptions,
    activeSourceProviders: {
      ...providers,
      LV: activeSourcePressureContractV2Provider(providers.LV, {
        sourceProviderId: `${idPrefix}:LV`,
        mode: "bounded-geometry-active-pressure-v1",
        activeGainCeilingRatio,
      }),
      RV: activeSourcePressureContractV2Provider(providers.RV, {
...
function transactionUser0Variant(): VariantSpec {
  ...
  return {
    ...
    ventricularChamberTransaction: {
      mechanismId: "phase5cp-av-inflow-residual-attribution-user0",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cp-av-inflow-residual-attribution-user0",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function transactionLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    closurePath: "lv-rv-land-legacy-atria",
    ventricularChamberTransaction: {
      mechanismId: "phase5cp-av-inflow-residual-attribution-legacy-atria",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
      includeAdjacentLoadNodes: true,
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cp-av-inflow-residual-attribution-legacy-atria",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function acceptedAvBoundaryLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cp-accepted-av-boundary-legacy-atria",
      "accepted-state-av-boundary",
      ["MV", "TV"],
      4,
      0.7,
...
function acceptedAvBoundaryUser0Variant(): VariantSpec {
  ...
  return {
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cp-accepted-av-boundary-user0",
      "accepted-state-av-boundary",
      ["MV", "TV"],
      4,
      0.7,
...
function acceptedAvComplementarityUser0Variant(): VariantSpec {
  ...
  return {
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5cp-accepted-av-complementarity3-relax07-user0",
      "accepted-state-av-boundary-fixedpoint",
      ["MV", "TV"],
      4,
      0.7,
...
function withVentricularChamberTransactionStep(
  ...
  const providers = experimentalOptions.activeSourceProviders;
  if (!providers?.LV || !providers.RV) {
    throw new Error("Phase 5CP AV inflow residual attribution requires LV and RV source providers.");
  }
  return {
    ...experimentalOptions,
...
function classify(
  ...
  const phase5coBest = requiredSummary(summaries, "accepted-av-complementarity2-legacy-atria");
  const pressureContractCandidates = [
    requiredSummary(summaries, "bounded-geometry-gain115-legacy-atria"),
    requiredSummary(summaries, "bounded-geometry-gain125-legacy-atria"),
    requiredSummary(summaries, "bounded-geometry-gain115-accepted-complementarity2-legacy-atria"),
  ];
  const transfer = requiredSummary(summaries, "bounded-geometry-gain115-accepted-complementarity2-user0");
  const bestPressureContract = bestSummary(pressureContractCandidates);
  const comparisonCandidates = [
    requiredSummary(summaries, "accepted-av-complementarity2-legacy-atria"),
    ...pressureContractCandidates,
    transfer,
  ];
  const bestTransaction = bestSummary(comparisonCandidates);
  ...
  const focusBestPressureContract = requiredResult(results, bestPressureContract.variantId, "low-preload-hr75");
  const bestLegacyOrAnyCleanEnough =
    bestPressureContract.grossPassCount === POINTS.length
    && bestPressureContract.contractClassCounts.cleanVentricularSide >= POINTS.length * 2
    && bestPressureContract.residualCauseCounts["clean-biphasic"] >= POINTS.length * 2;
  const user0TransferCleanEnough =
    transfer.grossPassCount === POINTS.length
    && transfer.contractClassCounts.cleanVentricularSide >= POINTS.length * 2
    && transfer.residualCauseCounts["clean-biphasic"] >= POINTS.length * 2;
  const partialPressureContractPositive =
    bestPressureContract.grossPassCount > phase5coBest.grossPassCount
    && bestPressureContract.lvPvOkCount === POINTS.length
    && bestPressureContract.rvPvOkCount === POINTS.length
    && bestPressureContract.outputPreservedCount >= phase5coBest.outputPreservedCount;
  return {
    legacyGrossPass: "not-run",
    ...
    bestPressureContractVariant: `${bestPressureContract.variantId}:${bestPressureContract.grossPassCount}/${POINTS.length}`,
    bestPressureContractResidualCauseCounts: bestPressureContract.residualCauseCounts,
    user0TransferGrossPass: `${transfer.grossPassCount}/${POINTS.length}`,
    decision: !refitConfigurationIntegrity.allConfigMatchesLabel
      ...
      : bestLegacyOrAnyCleanEnough
        ? "legacy-atria-pressure-contract-pass-user0-transfer-blocked"
        : partialPressureContractPositive
          ? "partial-positive-pressure-load-contract"
          : "av-valve-boundary-transaction-redesign-required",
    notes: [
      ...
      `Best pressure-contract low-preload residual-point failed labels: ${focusBestPressureContract.failedLabels.join(", ") || "none"}.`,
      `Phase 5CO accepted-complementarity baseline gross pass count: ${phase5coBest.grossPassCount}/${POINTS.length}.`,
      `Best pressure-contract gross pass count: ${bestPressureContract.grossPassCount}/${POINTS.length}.`,
      `Best transaction residual causes: ${formatCounts(bestTransaction.residualCauseCounts)}.`,
      `Best pressure-contract residual causes: ${formatCounts(bestPressureContract.residualCauseCounts)}.`,
      `Bounded pressure-contract user0 transfer gross pass count: ${transfer.grossPassCount}/${POINTS.length}.`,
      `Bounded pressure-contract user0 transfer residual causes: ${formatCounts(transfer.residualCauseCounts)}.`,
      ...
      "The pressure-contract decision uses the full representative morphology envelope, not a normal-point rescue.",
    ],
  };
}
...
function recommendedNext(classification: Classification): readonly string[] {
  if (classification.decision === "configuration-integrity-failed") {
    return [
      "do not interpret the morphology counts until requested/effective AV boundary settings match each variant label",
      "fix ModelCore normalization or the Phase 5CP runner labels, then rerun the same 8-point morphology envelope",
      "do not adopt, wire, or transfer the bounded pressure/load contract while configuration integrity is failed",
    ];
  }
  if (classification.decision === "pressure-load-contract-clean-enough-for-runtime-shadow") {
    return [
      "rerun the full morphology envelope with the same bounded pressure/load contract before runtime shadow",
      "resume LandAtrial AV-plane/effective-wall release timing only after LV/RV PV plus MVF/TVF remain robust",
    ];
  }
  if (classification.decision === "legacy-atria-pressure-contract-pass-user0-transfer-blocked") {
    return [
      "do not adopt the bounded pressure/load contract as runtime/default yet because all-chamber user0 transfer remains below the full morphology envelope",
      "next run a transfer-focused all-chamber user0 residual attribution without reopening A1/A2 or tuning valve/qDot/root/Zc/Tref/source-stress",
  ...
  if (classification.decision === "partial-positive-pressure-load-contract") {
    return [
      "do not adopt the bounded pressure/load contract as runtime/default unless the full envelope passes and transfers to all-chamber user0",
      "carry the bounded pressure/load contract forward only as component evidence if it clearly beats the Phase 5CO accepted-complementarity baseline",
      "if bounded gain still leaves AV inflow residuals, move to a broader chamber pressure/valve/load transaction rather than another scalar gain cap",
      "keep LandAtrial AV-plane/effective-wall release timing frozen until LV/RV PV plus MVF/TVF pass the morphology envelope",
    ];
  }
  const causes = classification.bestPressureContractResidualCauseCounts;
  ...
  const pressureContractDominant =
    ...
    >= causes["valve-diode-or-qdot-contamination"] + causes["valve-state-chatter"];
  const next: string[] = [
    "do not adopt the bounded active-pressure geometry-gain contract or accepted-state AV complementarity candidates unless they pass the full morphology envelope",
  ];
  if (valveBoundaryDominant) {
    next.push("bounded pressure gain plus accepted-state AV complementarity still leaves valve-boundary-dominant residuals; move the next surface toward a broader chamber pressure/valve/load transaction rather than more local scalar caps");
  }
  if (pressureContractDominant) {
    next.push("prioritize a broader chamber pressure-adapter contract or graph-level chamber/load transaction because residual inflow waves align with pressure-gradient or ventricular-pressure rebound");
  }
  next.push(
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CP variant summary ${id}.`);
  return summary;
}
...
function requiredResult(results: readonly PointResult[], variantId: VariantId, pointId: PointId): PointResult {
  ...
  if (!result) throw new Error(`Missing Phase 5CP result ${variantId}/${pointId}.`);
  return result;
}
...
export function writePressureLoadContractPhase5CPEvidence(): Evidence {
  const evidence = buildPressureLoadContractPhase5CPEvidence();
  const outPath = path.resolve(process.cwd(), PRESSURE_LOAD_CONTRACT_PHASE5CP_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writePressureLoadContractPhase5CPEvidence();
