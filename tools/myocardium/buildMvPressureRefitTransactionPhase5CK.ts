...
export const MV_PRESSURE_REFIT_TRANSACTION_PHASE5CK_ID =
  "mv-pressure-refit-transaction-phase5ck-result-v1" as const;
export const MV_PRESSURE_REFIT_TRANSACTION_PHASE5CK_RESULT_PATH =
  "data/myocardium/protocols/mv-pressure-refit-transaction-phase5ck-result-v1.json" as const;
...
type VariantId =
  ...
  | "mv-pressure-refit-only-legacy-atria"
  | "tv-state-mv-pressure-refit-legacy-atria"
  | "tv-state-mv-pressure-refit-relax09-legacy-atria"
  | "tv-state-mv-pressure-refit-user0";
...
type Classification = {
  ...
  readonly decision:
    ...
    | "partial-positive-mv-pressure-refit"
...
type Evidence = {
  ...
  readonly id: typeof MV_PRESSURE_REFIT_TRANSACTION_PHASE5CK_ID;
  readonly phase: "5CK";
  readonly profile: {
    ...
    readonly candidateMechanism:
      "ModelCore ventricularChamberTransactionStep with adjacent-load-node volume-flow-provider-state fixed-point transaction, TV state-coupled complementarity, and off-by-default MV pressure-refit candidates";
  };
...
export function buildMvPressureRefitTransactionPhase5CKEvidence(): Evidence {
  const variants = [
    legacyVariant(),
    currentUser0Variant(),
    ...
    mvPressureRefitLegacyAtriaVariant("mv-pressure-refit-only-legacy-atria", ["MV"], 4, 0.7),
    mvPressureRefitLegacyAtriaVariant("tv-state-mv-pressure-refit-legacy-atria", ["MV", "TV"], 4, 0.7),
    mvPressureRefitLegacyAtriaVariant("tv-state-mv-pressure-refit-relax09-legacy-atria", ["MV", "TV"], 4, 0.9),
    mvPressureRefitUser0Variant("tv-state-mv-pressure-refit-user0", ["MV", "TV"], 4, 0.7),
  ] as const;
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: MV_PRESSURE_REFIT_TRANSACTION_PHASE5CK_ID,
    phase: "5CK",
    profile: {
...
function transactionUser0Variant(): VariantSpec {
  ...
  return {
    ...
    ventricularChamberTransaction: {
      mechanismId: "phase5ck-av-inflow-residual-attribution-user0",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      ...
      "phase5ck-av-inflow-residual-attribution-user0",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function transactionLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    closurePath: "lv-rv-land-legacy-atria",
    ventricularChamberTransaction: {
      mechanismId: "phase5ck-av-inflow-residual-attribution-legacy-atria",
      iterations: 4,
      relaxation: 0.7,
      chambers: ["LV", "RV"],
      includeAdjacentLoadNodes: true,
    ...
    experimentalOptions: withVentricularChamberTransactionStep(
      resolved.experimentalOptions,
      "phase5ck-av-inflow-residual-attribution-legacy-atria",
      "current-diode",
      ["MV", "TV"],
      4,
      0.7,
...
function mvPressureRefitUser0Variant(
  id: "tv-state-mv-pressure-refit-user0",
  targetValves: readonly ("MV" | "TV")[],
  iterations: number,
  relaxation: number,
): VariantSpec {
  ...
  return {
    id,
    label: "Current user-0 closure with TV state coupling and MV pressure-refit",
    hypothesis: "Checks whether the best legacy-atria MV pressure-refit candidate transfers back to the current all-chamber LandAtrial default closure.",
    closurePath: "current-user0-all-chamber-landatrial",
    ventricularChamberTransaction: transactionDescriptor(
      id,
      targetValves,
...
function transactionDescriptor(
  ...
  iterations: number,
  relaxation: number,
  mode: "state-coupled-complementarity" | "tv-state-coupled-mv-pressure-refit" = "state-coupled-complementarity",
...
function withVentricularChamberTransactionStep(
  ...
  if (!providers?.LV || !providers.RV) {
    throw new Error("Phase 5CK AV inflow residual attribution requires LV and RV source providers.");
  }
  return {
    ...experimentalOptions,
...
function classify(
  ...
  const transactionCandidates = [
    ...
    requiredSummary(summaries, "mv-pressure-refit-only-legacy-atria"),
    requiredSummary(summaries, "tv-state-mv-pressure-refit-legacy-atria"),
    requiredSummary(summaries, "tv-state-mv-pressure-refit-relax09-legacy-atria"),
    requiredSummary(summaries, "tv-state-mv-pressure-refit-user0"),
  ];
  ...
  const partialMvPressureRefitPositive =
  ...
  return {
    ...
    decision: residualsCleanEnough
      ? "residuals-clean-enough-for-runtime-shadow"
      : partialMvPressureRefitPositive
        ? "partial-positive-mv-pressure-refit"
        : "av-valve-boundary-transaction-redesign-required",
    notes: [
...
function recommendedNext(classification: Classification): readonly string[] {
  ...
  if (classification.decision === "partial-positive-mv-pressure-refit") {
    return [
      "do not adopt the MV pressure-refit candidate as runtime/default unless the full envelope passes and transfers to all-chamber user0",
      "if MV pressure-refit improves over Phase 5CJ TV-only state coupling, carry it forward as diagnostic component evidence and rerun a narrower left-sided complementarity surface against the remaining failed points",
      "if MV pressure-refit does not improve over 5CJ, stop local AV-boundary refinements and return to a chamber pressure-adapter contract redesign",
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CK variant summary ${id}.`);
  return summary;
}
...
function requiredResult(results: readonly PointResult[], variantId: VariantId, pointId: PointId): PointResult {
  ...
  if (!result) throw new Error(`Missing Phase 5CK result ${variantId}/${pointId}.`);
  return result;
}
...
export function writeMvPressureRefitTransactionPhase5CKEvidence(): Evidence {
  const evidence = buildMvPressureRefitTransactionPhase5CKEvidence();
  const outPath = path.resolve(process.cwd(), MV_PRESSURE_REFIT_TRANSACTION_PHASE5CK_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeMvPressureRefitTransactionPhase5CKEvidence();
