...
export const TV_BOUNDARY_RESIDUAL_FOCUS_PHASE5CX_ID =
  "tv-boundary-residual-focus-phase5cx-result-v1" as const;
export const TV_BOUNDARY_RESIDUAL_FOCUS_PHASE5CX_RESULT_PATH =
  "data/myocardium/protocols/tv-boundary-residual-focus-phase5cx-result-v1.json" as const;
...
type VariantId =
  ...
  | "accepted-av-complementarity2-user0-both-held-adverse-forward-scale025"
  | "accepted-av-complementarity2-user0-both-held-adverse-forward-scale000";
...
type VariantSpec = {
  ...
  readonly adverseGradientForwardScale: number;
...
type Classification = {
  ...
  readonly user0BothHeldScale025GrossPass: string;
  readonly user0BothHeldScale000GrossPass: string;
  readonly user0BothHeldBaselineMvfTvf: { readonly mvfOk: string; readonly tvfOk: string };
  readonly user0BothHeldScale025MvfTvf: { readonly mvfOk: string; readonly tvfOk: string };
  readonly user0BothHeldScale000MvfTvf: { readonly mvfOk: string; readonly tvfOk: string };
  readonly decision:
    | "adverse-gradient-forward-scale-improves-tv-residual"
    | "adverse-gradient-forward-scale-does-not-rescue-tv-residual"
    | "adverse-gradient-forward-scale-breaks-output-or-mv"
...
type Evidence = {
  ...
  readonly id: typeof TV_BOUNDARY_RESIDUAL_FOCUS_PHASE5CX_ID;
  readonly phase: "5CX";
  readonly profile: {
    ...
    readonly diagnosticQuestion:
      "whether suppressing accepted-boundary adverse-gradient forward coasting separates the remaining TVF residual after inlet-held AV-plane release";
  };
...
export function buildTvBoundaryResidualFocusPhase5CXEvidence(): Evidence {
  const slowRelease = {
  ...
  const variants = [
    ...
    acceptedUser0Variant(
      "accepted-av-complementarity2-user0-both-held-adverse-forward-scale025",
      "Current user0 with inlet-held release and adverse-gradient forward scale 0.25",
      { LA: inletHeldRelease, RA: inletHeldRelease },
      0.25,
    ),
    acceptedUser0Variant(
      "accepted-av-complementarity2-user0-both-held-adverse-forward-scale000",
      "Current user0 with inlet-held release and adverse-gradient forward scale 0",
      { LA: inletHeldRelease, RA: inletHeldRelease },
      0,
    ),
  ] as const;
  ...
  const evidenceWithoutHash = {
    schemaVersion: 1,
    id: TV_BOUNDARY_RESIDUAL_FOCUS_PHASE5CX_ID,
    phase: "5CX",
    profile: {
...
function acceptedLegacyAtriaVariant(): VariantSpec {
  ...
  return {
    ...
    adverseGradientForwardScale: 1,
    experimentalOptions: withAcceptedComplementarity(resolved.experimentalOptions, "phase5cx-accepted-legacy-atria", 1),
  };
}
...
function acceptedUser0Variant(
  ...
  adverseGradientForwardScale: number,
): VariantSpec {
  const resolved = resolveModelCoreRuntimeActiveSource({
  ...
  return {
    ...
    adverseGradientForwardScale,
    experimentalOptions: withAcceptedComplementarity(experimentalOptions, `phase5cx-${id}`, adverseGradientForwardScale),
  };
}
...
function withAcceptedComplementarity(
  ...
  return {
    ...
    ventricularChamberTransactionStep: {
      ...
      avValveBoundaryAdverseGradientForwardScale: adverseGradientForwardScale,
    },
  };
}
...
function classify(summaries: readonly VariantSummary[], results: readonly PointResult[]): Classification {
  ...
  const scale025 = requiredSummary(summaries, "accepted-av-complementarity2-user0-both-held-adverse-forward-scale025");
  const scale000 = requiredSummary(summaries, "accepted-av-complementarity2-user0-both-held-adverse-forward-scale000");
  ...
  const bestScaledTvf = Math.max(scale025.tvfOkCount, scale000.tvfOkCount);
  const bestScaledGross = Math.max(scale025.grossPassCount, scale000.grossPassCount);
  const scaledBreaksMvOrOutput =
    (scale025.mvfOkCount < baseline.mvfOkCount || scale025.outputPreservedCount < baseline.outputPreservedCount)
    && (scale000.mvfOkCount < baseline.mvfOkCount || scale000.outputPreservedCount < baseline.outputPreservedCount);
  return {
    ...
    user0BothHeldScale025GrossPass: `${scale025.grossPassCount}/${POINTS.length}`,
    user0BothHeldScale000GrossPass: `${scale000.grossPassCount}/${POINTS.length}`,
    user0BothHeldBaselineMvfTvf: { mvfOk: `${baseline.mvfOkCount}/${POINTS.length}`, tvfOk: `${baseline.tvfOkCount}/${POINTS.length}` },
    user0BothHeldScale025MvfTvf: { mvfOk: `${scale025.mvfOkCount}/${POINTS.length}`, tvfOk: `${scale025.tvfOkCount}/${POINTS.length}` },
    user0BothHeldScale000MvfTvf: { mvfOk: `${scale000.mvfOkCount}/${POINTS.length}`, tvfOk: `${scale000.tvfOkCount}/${POINTS.length}` },
    decision: legacy.grossPassCount < Math.ceil(0.5 * POINTS.length)
      ? "accepted-legacy-baseline-unusable"
      : missingReadback
        ? "configuration-or-readback-gap"
        : scaledBreaksMvOrOutput
          ? "adverse-gradient-forward-scale-breaks-output-or-mv"
          : bestScaledTvf > baseline.tvfOkCount || bestScaledGross > baseline.grossPassCount
            ? "adverse-gradient-forward-scale-improves-tv-residual"
            : "adverse-gradient-forward-scale-does-not-rescue-tv-residual",
    notes: [
      `Accepted legacy causes: ${formatCounts(legacy.causeCounts)}.`,
      `Both inlet-held baseline causes: ${formatCounts(baseline.causeCounts)}.`,
      `Adverse-scale 0.25 causes: ${formatCounts(scale025.causeCounts)}.`,
      `Adverse-scale 0 causes: ${formatCounts(scale000.causeCounts)}.`,
      `Both inlet-held baseline residual notes: ${residualNotes(results, baseline.variantId).join(" | ") || "none"}.`,
      `Adverse-scale 0.25 residual notes: ${residualNotes(results, scale025.variantId).join(" | ") || "none"}.`,
      `Adverse-scale 0 residual notes: ${residualNotes(results, scale000.variantId).join(" | ") || "none"}.`,
    ],
  };
}
...
function recommendedNext(classification: Classification): readonly string[] {
  ...
  if (classification.decision === "adverse-gradient-forward-scale-improves-tv-residual") {
    return [
      "treat adverse-gradient forward coasting as a remaining TVF residual contributor, but do not adopt this scalar brake",
      "replace this diagnostic with a physical valve complementarity/closure contract before any runtime/default claim",
    ];
  }
  if (classification.decision === "adverse-gradient-forward-scale-breaks-output-or-mv") {
    return [
      "do not continue scalar adverse-gradient braking because it breaks MV/output while trying to repair TVF",
      "move to a physical valve complementarity/closure contract rather than another scalar limiter",
    ];
  }
  return [
    "adverse-gradient forward coasting ablation does not rescue TVF; classify the residual with a physical valve-state/pressure-flow contract next",
    "keep A1/A2 frozen and do not use scalar braking, qDot, valve threshold, or Land parameter tuning as an adoption path",
  ];
}
...
function requiredSummary(summaries: readonly VariantSummary[], id: VariantId): VariantSummary {
  ...
  if (!summary) throw new Error(`Missing Phase 5CX variant summary ${id}.`);
  return summary;
}
...
export function writeTvBoundaryResidualFocusPhase5CXEvidence(): Evidence {
  const evidence = buildTvBoundaryResidualFocusPhase5CXEvidence();
  const outPath = path.resolve(process.cwd(), TV_BOUNDARY_RESIDUAL_FOCUS_PHASE5CX_RESULT_PATH);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const evidence = writeTvBoundaryResidualFocusPhase5CXEvidence();
