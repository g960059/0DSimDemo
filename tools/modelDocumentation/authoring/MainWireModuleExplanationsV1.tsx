...
export function MainWireModuleExplanationsV1({ moduleIds, equations, locale, reading = false }: {
  moduleIds: readonly string[]; equations: MainWireEquationDataV1; locale: Locale; reading?: boolean;
}) {
  return <div className="space-y-8">{moduleIds.map(id => {
    ...
    const primaryEquation = m.equation && <><Equation expression={m.equation} /><p className="text-xs leading-6 text-wb-subtle">{m.equationNote?.[locale]}</p></>;
    return <article key={id} id={id} className="scroll-mt-24">
      ...
      <ModelDocumentDetailV1 title={locale === "ja" ? "数式・仮定を詳しく" : "Equations and assumptions"}>
        ...
        <MainWireModuleEquationDetailsV1 data={equations} id={id} locale={locale} Equation={Equation} includeParameters={!reading} />
