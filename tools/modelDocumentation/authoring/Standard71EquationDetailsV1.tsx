...
import data from "@/studio/presentation/modelDocumentation/standard71-equation-data-v1.json";
...
export function MainWireDetailedCircuitV1({ locale, Equation }: { locale: Locale; Equation: Eq }) {
  const routes = [
...
function ParameterTables({ id, locale }: { id: string; locale: Locale }) {
...
export function MainWireModuleEquationDetailsV1({ id, locale, Equation }: { id: string; locale: Locale; Equation: Eq }) {
  ...
  return <div className="mt-6 border-t border-wb-line pt-1" data-equation-module={id}>
    ...
    <ParameterTables id={id} locale={locale}/>
  </div>;
}

export function MainWireAssemblyAndInitialStateV1({ locale, Equation }: { locale: Locale; Equation: Eq }) {
