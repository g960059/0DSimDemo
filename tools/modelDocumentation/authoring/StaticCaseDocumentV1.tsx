  ...
  return <div className={reading ? "" : "h-full overflow-y-auto bg-wb-app text-wb-text"} data-testid="model-documentation-v2">
    <Body className={reading ? "space-y-10" : "mx-auto max-w-4xl space-y-9 px-5 py-8 sm:px-10 sm:py-12"}>
      ...
      <header>
        ...
        <p className="mt-3 text-xs text-wb-subtle">{t("研究・教育用の症例設定です。患者の診断・治療には用いません。", "A case for research and education, not for patient diagnosis or treatment.")}</p>
      </header>
      ...
      <section id="baseline" className={section}>
        ...
        <Detail title={t("波形と定常性", "Waveform and settlement")}>
          ...
          <p className={`mt-3 ${p}`}>{t("この追加記録は以前のlab-003で行い、当時のlab-004との数値状態・解法予測子・構成・開始時観測値の一致を確認したものです。今回のモデルの起動記録そのものではありません。同じ構成の過去の補助検証として参照し、今回再実行したとは扱いません。", "This extension was run on lab-003 and checked against the then-current lab-004 numerical state, solver predictor, construction and initial observations. It is not this model's launch record. It is retained as historical supporting evidence for the same construction, not a fresh execution.")}</p>
        </Detail>
        <Detail title={t("PV解析と受動的な容量特性", "PV analysis and passive accommodation")}>
          <p className={p}>{t("以下のPV解析は、同じ構成の過去の2 ms起動状態に対する負荷変更解析です。今回の起動状態での再実行ではありません。1 msの記録を選んでも、この解析の刻み幅は変わりません。縦軸は経壁圧です。", "These loading analyses use a historical 2 ms launch with the same construction, not a rerun from this model's launch. Selecting the 1 ms record does not change the grid of these analyses. Pressure is transmural.")}</p>
