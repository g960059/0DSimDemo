...
import { STANDARD71_DOCUMENTATION_SNAPSHOT_V1 as doc, standard71BaselineRowsV1,
...
import { MainWireDetailedCircuitV1, MainWireModuleEquationDetailsV1, MainWireAssemblyAndInitialStateV1 } from "./Standard71EquationDetailsV1";
...
export function Standard71DocumentV1({ locale, recordIndex = 0 }: { locale: Locale; recordIndex?: 0 | 1 }) {
  const root = React.useRef<HTMLElement>(null);
  const grid = recordIndex;
  const o = doc.observations[grid];
  const rows = standard71BaselineRowsV1(grid, locale);
  ...
  return <div className="h-full overflow-y-auto bg-wb-app text-wb-text" data-testid="model-documentation-v2">
    <main ref={root} className="mx-auto max-w-4xl space-y-10 px-5 py-8 sm:px-10 sm:py-12">
      <header>
        ...
        <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">Main Wire Standard 71</h1>
        ...
        <p className="mt-3 text-xs leading-6 text-wb-subtle">{text(locale, "研究・教育用 · ローカル採択候補（公開登録前）· 臨床的妥当性は未確立", "Research and education · local candidate, not publicly registered · not clinically validated")}</p>
      </header>
      ...
      <section id="overview" className={section}>
        ...
        <Detail title={text(locale, "詳しい回路と区画（node）", "Detailed circuit and compartments (nodes)")}>
          <MainWireDetailedCircuitV1 locale={locale} Equation={Equation} />
        </Detail>
      ...
      <section id="mechanisms" className={section}>
        ...
        <div className="space-y-8">{STANDARD71_MODULE_IDS_V1.map(id => {
          ...
          return <article key={id} id={id}>
            ...
            <Detail title={text(locale, "数式・仮定を詳しく", "Equations and assumptions")}>
              ...
              <MainWireModuleEquationDetailsV1 id={id} locale={locale} Equation={Equation} />
        ...
        <div id="assembly" className="mt-8 scroll-mt-4">
          ...
          <Detail title={text(locale, "連立方程式・初期条件を詳しく", "Coupled equations and initial conditions")}>
            <MainWireAssemblyAndInitialStateV1 locale={locale} Equation={Equation} />
          </Detail>
        </div>
      </section>
      ...
      <section id="baseline" className={section}>
        ...
        <div className="my-5 rounded-lg border border-wb-line bg-wb-panel p-4 text-sm leading-7">
          <p>{text(locale, "採択候補としての評価を通過。臨床的な正常性の保証ではありません。", "Eligible as a candidate, not a guarantee of clinical normality.")}</p>
        ...
        <label data-document-record-selector className="mb-4 flex flex-wrap items-center gap-3 text-xs text-wb-muted">{text(locale, "表示する検証記録", "Assessment record")}
          <select data-document-action="record" aria-label={text(locale, "検証記録", "Assessment record")} value={grid} onChange={() => {}} className={`rounded border border-wb-line bg-wb-panel px-3 py-2 text-wb-text ${focus}`}>
            <option value={0}>{text(locale, "2 ms · Standard71独立再実行と一致", "2 ms · matched independent Standard71 replay")}</option>
            <option value={1}>{text(locale, "1 ms · 同一物理構成の研究記録", "1 ms · identical physical research construction")}</option>
          </select>
        </label>
      ...
      <section id="record" className={section}>
        ...
        <Detail title={text(locale, "この版で変わったこと", "Changes in this version")}>
          <p className={paragraph}>{text(locale, "Standard70から心室の材料係数・Ca波形とbaselineの設定を調整し、体動脈コンプライアンスに固定倍率を加えました。大動脈近位部は慣性なしで流量を求めます。新しい状態変数や弁方程式は追加していません。画面に公開する項目とESPVR・EDPVR・Starlingの解析は、互換性のある最新の定義を引き継ぎます。旧版の評価記録は書き換えません。", "Relative to Standard70, ventricular material/calcium and the reference operating point were recalibrated, with a fixed systemic compliance multiplier and algebraic aortic root. No new continuous state or valve equation was added. The latest compatible Surface and ESPVR/EDPVR/Starling analyses are inherited. Historical votes and reference intervals are not rewritten.")}</p>
        </Detail>
        <Detail title={text(locale, "固定した識別情報と記録", "Pinned identities and evidence record")}>
          ...
          <p className="mt-5 text-xs leading-6 text-wb-muted">{text(locale, "起動には、定常周期の境界から次の表示時刻まで実際に計算を進めた保存状態を使います。細かい時間刻みと血液量の増減に関する検証は、同じ数理モデル・係数を使った研究記録に基づきます。Standard71の初期状態からの再計算との一致は、2 msで確認しています。保存データには、日本語で要約した説明の元の記録も含まれます。", "Launch uses a state actually advanced to the next presentation time. Fine-grid/reserve evidence comes from the identical physical construction. Standard71's cold replay was compared at 2 ms. The download includes original records underlying the localized explanations.")}</p>
          <a href="#document-measurements" data-document-action="measurements" download="standard71-documentation-snapshot-v1.json" className="mt-4 inline-block text-sm text-wb-accent underline">{text(locale, "表示データ・測定記録をダウンロード", "Download presentation and measurement record")}</a>
