...
import {
  runMainWireNormalAdultFiveWallAorticOutflowWholeOrganKuwCalciumWindkesselResearchV1,
...
export const MAIN_WIRE_AORTIC_OUTFLOW_ACTIVATION_REDISTRIBUTION_INTERACTION_V1_ID =
  "main-wire-aortic-outflow-activation-redistribution-interaction-v1" as const;
...
const arms = calciumProfileIds.flatMap((calciumProfileId) =>
  kuwProfileIds.map((kuwProfileId) => {
    const run =
      runMainWireNormalAdultFiveWallAorticOutflowWholeOrganKuwCalciumWindkesselResearchV1(
    ...
    const profileLabel = calciumProfileId ?? "canonical-calcium";
    const armId = `${profileLabel}__${kuwProfileId}`;
    const beat = run.periodicResult.retainedCompleteBeats.at(-1)!;
    const calcium = beat.samples.map((sample) =>
      Math.max(
        0,
...
const report = Object.freeze({
  ...
  experimentId:
    MAIN_WIRE_AORTIC_OUTFLOW_ACTIVATION_REDISTRIBUTION_INTERACTION_V1_ID,
  ...
  interpretationBoundary: Object.freeze({
    existingDelayedMixtureEstablishesSpatialActivationModel: false as const,
    interactionScreenPrecedesActivationModelConstruction: true as const,
    sourceWholeOrganKuwBracketOnly: true as const,
