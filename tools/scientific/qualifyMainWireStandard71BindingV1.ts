...
import { MAIN_WIRE_INTEGRATED_STUDIO_STANDARD71_MODEL_ID_V1 } from "@/domain/model/MainWireStandardIdentityV1";
...
const same = (a: unknown, b: unknown, label: string) => {
  if (canonicalJsonStringify(a) !== canonicalJsonStringify(b)) throw new Error(`Standard71 parity mismatch: ${label}`);
};
...
for (let cycle = 1; cycle <= policy.maximumCycleCount; cycle++) {
  ...
  if (!run.allDynamicMcsAcceptedFlowsExactlyZero || !run.oneComposedCalciumOwnerOnly
    ...
    throw new Error("Standard71 cold run violates conservation or owner invariants");
  }
  trace = run.traceSamples; boundaries.push(accepted); if (boundaries.length > 3) boundaries.shift(); cycles = cycle;
  if (cycle % 10 === 0) process.stderr.write(`Standard71 cold: cycle ${cycle}, residual ${period1.overall.maximumNormalizedDelta}\n`);
  if (classification.status !== "not-converged") break;
}
if (classification.status !== "period1-converged" || beat === null) throw new Error("Standard71 requires its own settled complete beat");
...
if (rest.status !== "passed") throw new Error("Standard71 fresh rest admission failed");
...
const checkpoint = await checkpointMainWireIntegratedModelStandard71V1(fixture.standard71AssemblyId, base);
const restored = await Session.restoreStandard71ExactCheckpoint(checkpoint);
same(await restored.checkpointStandard71Exact(), checkpoint, "own checkpoint roundtrip");
...
if (launchAdvance.status !== "advanced" || launchTimeSec - checkpoint.acceptedTimeSec > .002) throw new Error("Standard71 launch needs one bounded real grid-alignment advance");
executionPlanBaseTickAtTimeV1(schedule, restored.currentAcceptedState().acceptedTimeSec);
const launchCheckpoint = await restored.checkpointStandard71Exact();
same(restored.observe().completedBeatMetrics, beat, "launch complete beat unchanged");
same(await (await Session.restoreStandard71ExactCheckpoint(launchCheckpoint)).checkpointStandard71Exact(), launchCheckpoint, "launch roundtrip");
const output = resolve(values.output);
await mkdir(output, { recursive: false });
const report = { schemaId: "main-wire-standard71-local-binding-evidence-v1", modelId: MAIN_WIRE_INTEGRATED_STUDIO_STANDARD71_MODEL_ID_V1,
  status: "cold-replay-and-checkpoint-parity-passed", clinicalNormalityClaimed: false, published: false,
  initialization: "own-standard71-cold", nominalDtSec: .002, cycles, classification,
  ...
  inheritedQualification: "Fine-step pressure-rate and preload-reserve evidence belongs to the reviewed identical physical research construction; not a new71 fine/reserve run. Live adapter/analysis parity remains separate.",
