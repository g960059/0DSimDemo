...
export const CIRCULATION_BRIDGE_SMOKE_REPORT_PATH_V1 =
  "data/mechanics2/reports/circulation-bridge-smoke-report-v1.json" as const;

export function writeCirculationBridgeSmokeReportV1():
ReturnType<typeof runCirculationBridgeSmokeBenchV1> & {
  readonly normalizedSha256: string;
} {
  const report = runCirculationBridgeSmokeBenchV1();
  const reportWithHash = { ...report, normalizedSha256: hashStable(report) };
  const outPath = path.resolve(process.cwd(), CIRCULATION_BRIDGE_SMOKE_REPORT_PATH_V1);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const report = writeCirculationBridgeSmokeReportV1();
