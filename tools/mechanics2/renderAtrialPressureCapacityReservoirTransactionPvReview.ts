...
import {
  applyAtrialPressureCapacityReservoirTransactionVariantV1,
  ATRIAL_PRESSURE_CAPACITY_RESERVOIR_TRANSACTION_VARIANTS_V1,
  runAtrialPressureCapacityReservoirTransactionBenchV1,
  type AtrialPressureCapacityReservoirTransactionVariantIdV1,
...
const OUTPUT_PATH =
  "data/mechanics2/visuals/atrial-pressure-capacity-reservoir-transaction-pv-review.svg" as const;
...
const reviewVariantId = selectReviewVariantId();
const reviewVariant = variantById(reviewVariantId);
...
const panels = PROFILES.map((profileId, index): Panel => {
  ...
  return {
    ...
    series: [
      ...
      {
        label: reviewVariantId,
        color: "#22c55e",
        samples: runLeftHeartSubsystemV2(
          applyAtrialPressureCapacityReservoirTransactionVariantV1(params, reviewVariant),
        ).finalBeatSamples,
      },
    ],
  };
...
const svg = renderSvg(panels, reviewVariantId);
mkdirSync(dirname(OUTPUT_PATH), { recursive: true });
writeFileSync(OUTPUT_PATH, `${svg}\n`);
console.log(OUTPUT_PATH);

function selectReviewVariantId(): AtrialPressureCapacityReservoirTransactionVariantIdV1 {
  if (report.summary.bestVariantId !== "compliance-node-no-avplane") return report.summary.bestVariantId;
  const activeCandidates = report.variantSummaries.filter((summary) =>
  ...
  return [...activeCandidates].sort((a, b) =>
    ...
    || b.maxAVPlaneReservoirTractionPressureMmHg - a.maxAVPlaneReservoirTractionPressureMmHg
...
function variantById(
  ...
  const variant = ATRIAL_PRESSURE_CAPACITY_RESERVOIR_TRANSACTION_VARIANTS_V1.find((candidate) =>
...
function renderSvg(
  ...
  bestVariantId: AtrialPressureCapacityReservoirTransactionVariantIdV1,
): string {
  ...
  <text x="28" y="30" fill="#e5e7eb" font-family="Arial, sans-serif" font-size="18" font-weight="700">Atrial pressure-capacity reservoir transaction PV review</text>
