...
export const RESERVOIR_BRIDGE_SMOKE_REPORT_PATH_V1 =
  "data/mechanics2/reports/reservoir-bridge-smoke-report-v1.json" as const;

export function writeReservoirBridgeSmokeReportV1():
ReturnType<typeof runReservoirBridgeSmokeBenchV1> & {
  readonly normalizedSha256: string;
} {
  const report = runReservoirBridgeSmokeBenchV1();
  const reportWithHash = { ...report, normalizedSha256: hashStable(report) };
  const outPath = path.resolve(process.cwd(), RESERVOIR_BRIDGE_SMOKE_REPORT_PATH_V1);
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  const report = writeReservoirBridgeSmokeReportV1();
