...
const outPath = resolve(
  repoRoot,
  "data/mechanics2/visuals/atrial-av-plane-lamv-transaction-review.svg",
);
...
const report = runAtrialAVPlaneLaMvTransactionReviewBenchV1();
const rawVariant = ATRIAL_AV_PLANE_LAMV_TRANSACTION_VARIANTS_V1.find((variantConfig) =>
  variantConfig.variantId === "raw-traction-reference"
)!;
const bestVariant = ATRIAL_AV_PLANE_LAMV_TRANSACTION_VARIANTS_V1.find((variantConfig) =>
  variantConfig.variantId === report.summary.bestLaMvTransactionVariantId
)!;
...
const panels = profileIds.map((profileId, index) => {
  const raw = runLeftHeartSubsystemV2(applyLaMvTransactionVariant(baseParams[index]!, rawVariant));
  const best = runLeftHeartSubsystemV2(applyLaMvTransactionVariant(baseParams[index]!, bestVariant));
...
svg.push(`<text x="34" y="34" fill="#e5e7eb" font-family="Inter,Arial,sans-serif" font-size="22" font-weight="700">Atrial AV-plane LA/MV transaction review</text>`);
svg.push(`<text x="34" y="58" fill="#9ca3af" font-family="Inter,Arial,sans-serif" font-size="13">AV-plane traction is released against same-step accepted MV opening instead of previous-step MV opening.</text>`);
svg.push(`<text x="34" y="82" fill="#9ca3af" font-family="Inter,Arial,sans-serif" font-size="12">best LA/MV variant: ${report.summary.bestLaMvTransactionVariantId}, source ${report.summary.bestLaMvTransactionSourceSurfacePass}/7, topology ${report.summary.bestLaMvTransactionTopologyPass}/7, MVF ${report.summary.bestLaMvTransactionMvfCleanCount}/7</text>`);
svg.push(`<line x1="908" y1="58" x2="950" y2="58" stroke="#22c55e" stroke-width="3"/>`);
svg.push(`<text x="958" y="62" fill="#22c55e" font-family="Inter,Arial,sans-serif" font-size="13">raw previous-open traction release</text>`);
svg.push(`<line x1="908" y1="80" x2="950" y2="80" stroke="#f97316" stroke-width="3"/>`);
svg.push(`<text x="958" y="84" fill="#f97316" font-family="Inter,Arial,sans-serif" font-size="13">best same-step LA/MV transaction</text>`);
...
function renderPanel(
  ...
  renderTraction(out, x + 4 * padX + 3 * plotW, y + padTop, plotW, plotH, panel.raw, panel.best);
}
...
function pathForTrace(
  ...
  sy: (value: number) => number,
  key: "qMvMlPerSec" | "mvOpen01" | "laAVPlaneReservoirTractionPressureMmHg",
): string {
  return samples
    .map((sample, index) =>
