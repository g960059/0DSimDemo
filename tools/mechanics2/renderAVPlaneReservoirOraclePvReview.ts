...
const OUTPUT_PATH =
  "data/mechanics2/visuals/av-plane-reservoir-oracle-pv-review.svg" as const;
...
type VisualVariantId = "baseline" | "phase-reference-r12-b0-p1" | "avplane-eject60-r12-b0-p1";
...
const panels = PROFILES.map((profileId, index): Panel => {
  ...
  return {
    ...
    series: [
      ...
      {
        variantId: "avplane-eject60-r12-b0-p1",
        label: "AV-plane ejection",
        color: "#f97316",
        samples: runLeftHeartSubsystemV2(applyAVPlaneEject60(params)).finalBeatSamples,
      },
    ],
  };
});
...
function applyAVPlaneEject60(params: LeftHeartSubsystemParamsV2): LeftHeartSubsystemParamsV2 {
  return {
    ...applyPhaseReference(params),
    laLobeGeneratorMode: "av-plane-reservoir-booster-state-shadow",
    laEffectiveGeometryMode: "av-plane-reservoir-capacity-volume-shadow",
    laReservoirSuctionStartTheta: 0.06,
    laReservoirSuctionEndTheta: 0.58,
    laAVPlaneEjectionRateStartMlPerSec: 18,
    laAVPlaneEjectionRateEndMlPerSec: 60,
  };
}
...
function renderSvg(inputPanels: readonly Panel[]): string {
  ...
  const legend = [
    ...
    legendItem(324, 58, "#f97316", "AV-plane ejection"),
  ].join("\n");
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
  <rect width="100%" height="100%" fill="#0f172a"/>
  <text x="28" y="30" fill="#e5e7eb" font-family="Arial, sans-serif" font-size="18" font-weight="700">AV-plane reservoir oracle PV visual review</text>
