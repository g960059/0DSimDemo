...
Numerical follow-up after the six-cell coarse study: reference and both-R0
only, at 2 ms and 1 ms, with per-cycle accepted Newton/backtrack statistics.
The extra 2 ms runs repeat the existing contrast only to collect diagnostics;
they do not enlarge the candidate search. The fine-grid reference uses its
own qualified 1 ms source checkpoint. No tolerance or metric definition changes.
