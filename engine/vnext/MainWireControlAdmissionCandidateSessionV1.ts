...
// Candidate-only facade. Keep the published Session import closure unchanged.
...
export class MainWireStaticCaseSessionV1 {
  ...
  readonly #recoverSteps: boolean;

  private constructor(fixture: Fixture, restored: Restored | null = null, fork: State | null = null, plan?: Plan, recoverSteps = false) {
  ...
  /** Used only by the explicitly different control-admission candidate. */
  static createWithStepRecovery(...inputs: [...Inputs, plan?: Plan]): MainWireStaticCaseSessionV1 {
    return new MainWireStaticCaseSessionV1(createFixture(inputs[0], inputs[1], inputs[2], inputs[3]), null, null, inputs[4], true);
  }
  static async restoreWithStepRecovery(checkpoint: unknown, ...inputs: [...Inputs, plan?: Plan]): Promise<MainWireStaticCaseSessionV1> {
    const fixture = createFixture(inputs[0], inputs[1], inputs[2], inputs[3]);
    return new MainWireStaticCaseSessionV1(fixture, await restoreMainWireStaticCaseV1(fixture, checkpoint), null, inputs[4], true);
  }
  ...
  advanceToPresentationTimeWithSelectedOutputProjectionV1(...args: Parameters<BaseOwner["advanceToPresentationTimeWithSelectedOutputProjectionV1"]>) {
    if (this.#recoverSteps) return advanceMainWireProjectionWithRecoveryV1(
  ...
  warmStart(inputs: NonNullable<Inputs[1]>, mechanism: NonNullable<Inputs[3]> = this.#fixture.mechanismResearchInputs, plan?: Plan) {
    ...
    return new MainWireStaticCaseSessionV1(target, null, state, plan, this.#recoverSteps);
  }

  forkAtFixedGlobalTotalBloodVolume(totalBloodVolumeMl: number) {
    return new MainWireStaticCaseSessionV1(this.#fixture, null,
      fixedTbv({ source: this.currentAcceptedState(), runtime: this.#fixture, targetGlobalTotalBloodVolumeMl: totalBloodVolumeMl }), this.#plan, this.#recoverSteps);
  }

  forkResponsiveStarlingAtFixedGlobalTotalBloodVolume(totalBloodVolumeMl: number) {
    return new MainWireStaticCaseSessionV1(this.#fixture, null,
      responsiveStarling({ source: this.currentAcceptedState(), runtime: this.#fixture, targetGlobalTotalBloodVolumeMl: totalBloodVolumeMl }), this.#plan, this.#recoverSteps);
  }
