...
export class MainWireScientificResearchControlTargetStateValidationErrorV0
  ...
  constructor(issues: readonly string[]) {
    ...
    super(`main-wire research control target state rejected: ${immutable.join("; ")}`);
    this.name = "MainWireScientificResearchControlTargetStateValidationErrorV0";
...
export async function createMainWireScientificResearchControlBaselineTargetStateV0():
  ...
  if (
    ...
    throw new Error(
      `main-wire research control baseline escaped its pinned V0 digest: ${baseline.targetStateSha256}`,
    );
  }
  return baseline;
}
...
export async function loadMainWireScientificResearchControlTargetStateV0(
  ...
  try {
    ...
    if (error instanceof MainWireScientificResearchControlTargetStateValidationErrorV0) {
  ...
  if (
    ...
    issues.push("control target state.targetStateSha256 must be a lowercase SHA-256 digest");
  }
  const catalog = await createMainWireScientificResearchControlCatalogV0();
  ...
  if (controls !== null) {
    ...
    if (canonicalJsonStringify(payload) !== canonicalJsonStringify(expectedPayload)) {
      issues.push("control target state payload must exactly match its release-bound catalog");
    }
  }
  const computedDigest = await sha256CanonicalJsonHex(payload);
  if (canonical.targetStateSha256 !== computedDigest) {
    issues.push("control target state.targetStateSha256 does not match its canonical payload");
  }
  if (ignoredDigest === undefined) {
    issues.push("control target state.targetStateSha256 is required");
  }
  if (issues.length > 0) {
    throw new MainWireScientificResearchControlTargetStateValidationErrorV0(issues);
  }
  return canonical as unknown as MainWireScientificResearchControlTargetStateV0;
}
...
function mainWireScientificResearchControlTargetStatePayloadV0(
  ...
  return {
    schemaId: MAIN_WIRE_SCIENTIFIC_RESEARCH_CONTROL_TARGET_STATE_V0_SCHEMA_ID,
    schemaVersion: 0,
    ...
    catalogRef: {
      ...
      catalogVersion: MAIN_WIRE_SCIENTIFIC_RESEARCH_CONTROL_CATALOG_V0_VERSION,
...
function loadCompleteControlValuesV0(
  ...
  for (const controlId of MAIN_WIRE_SCIENTIFIC_RESEARCH_CONTROL_IDS_V0) {
    const candidate = canonical[controlId];
    const allowed = MAIN_WIRE_SCIENTIFIC_RESEARCH_CONTROL_VALUE_DOMAINS_V0[
      controlId
    ].allowedValues as readonly number[];
    if (typeof candidate !== "number" || !allowed.includes(candidate)) {
      issues.push(`${controlId} must be one of ${allowed.join(", ")}`);
    }
  }
  if (issues.length > 0) {
    throw new MainWireScientificResearchControlTargetStateValidationErrorV0(issues);
