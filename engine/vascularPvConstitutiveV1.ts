...
export function ptmFromStressedVolume(
  ...
  const loVolume = venousStressedVolumeForInverse(law, compiledLaw, lo);
  const hiVolume = venousStressedVolumeForInverse(law, compiledLaw, hi);
  ...
  // The constitutive helper supplies non-venous and explicit fixed-iteration
  // primitives only. Adaptive inversion has one owner in vascularPvConvergentV1.
  if (options.termination !== "fixed-iterations") {
    throw new Error("Adaptive venous inversion requires the canonical vascularPv entry");
  }
