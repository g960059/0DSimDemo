import {
  validateMainWireIntegratedModelStandard67CheckpointV1,
...
export const MAIN_WIRE_INTEGRATED_MODEL_STANDARD67_CANONICAL_CHECKPOINT_V3_ID =
  "circleheart-main-wire-integrated-model-standard67-canonical-checkpoint-v3" as const;

export type MainWireIntegratedModelStandard67CanonicalCheckpointV3 = Readonly<{
  checkpointId:
    typeof MAIN_WIRE_INTEGRATED_MODEL_STANDARD67_CANONICAL_CHECKPOINT_V3_ID;
  schemaVersion: 3;
  revision: number;
  acceptedTimeSec: number;
  standard67Checkpoint: MainWireIntegratedModelStandard67CheckpointV1;
 ...
 * accepted epoch, and writes one canonical binary envelope. The Standard67
 * object wrapper continues to own numerical continuation semantics; this V3
 * boundary adds only algorithmic predictor history.
 */
export async function encodeMainWireIntegratedModelStandard67CanonicalCheckpointV3(
  standard67CheckpointInput: unknown,
  coupledPredictorInput: unknown,
): Promise<Uint8Array> {
  const standard67CheckpointPromise =
    validateMainWireIntegratedModelStandard67CheckpointV1(
      standard67CheckpointInput,
    );
  ...
  assertPredictorAcceptedClockV3(
    standard67Checkpoint.revision,
    standard67Checkpoint.acceptedTimeSec,
    coupledPredictor,
  );
  return encodeCanonicalFlatCheckpointV1(Object.freeze({
    checkpointId:
      MAIN_WIRE_INTEGRATED_MODEL_STANDARD67_CANONICAL_CHECKPOINT_V3_ID,
    schemaVersion: 3 as const,
    revision: standard67Checkpoint.revision,
    acceptedTimeSec: standard67Checkpoint.acceptedTimeSec,
    standard67Checkpoint,
    coupledPredictor,
  }) satisfies MainWireIntegratedModelStandard67CanonicalCheckpointV3);
}

/** Decodes, recursively owns, and cross-validates one canonical V3 image. */
export async function decodeMainWireIntegratedModelStandard67CanonicalCheckpointV3(
  input: Uint8Array,
): Promise<MainWireIntegratedModelStandard67CanonicalCheckpointV3> {
  return validateAndOwnMainWireIntegratedModelStandard67CanonicalCheckpointV3(
    await decodeCanonicalFlatCheckpointV1(input),
  );
}

export async function validateAndOwnMainWireIntegratedModelStandard67CanonicalCheckpointV3(
  input: unknown,
): Promise<MainWireIntegratedModelStandard67CanonicalCheckpointV3> {
  ...
  const standard67CheckpointPromise =
    validateMainWireIntegratedModelStandard67CheckpointV1(
      ownDataValueV3(record, "standard67Checkpoint"),
    );
  const coupledPredictor =
    validateAndOwnMainWireFiveWallCoupledPredictorCheckpointV2(
  ...
  if (
    revision !== standard67Checkpoint.revision
    || !Object.is(acceptedTimeSec, standard67Checkpoint.acceptedTimeSec)
  ) {
...
function plainExactRecordV3(
  ...
  if (input === null || typeof input !== "object" || Array.isArray(input)) {
    throw new Error("Standard67 canonical checkpoint must be a plain object");
  }
  ...
  if (
    ...
    throw new Error(
      "Standard67 canonical checkpoint has unexpected fields",
    );
  }
  return input;
}
...
function ownDataValueV3(record: object, key: string): unknown {
  ...
  if (descriptor === undefined || !("value" in descriptor)) {
    throw new Error(
      `Standard67 canonical checkpoint ${key} must be a data field`,
    );
  }
