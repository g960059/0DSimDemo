...
export type AtrialOneFiberPassiveEvaluationV1 = {
  readonly modelId: typeof ATRIAL_ONE_FIBER_PASSIVE_V1_ID;
  readonly parameterSetId: string;
  readonly parameterIdentityHash: string;
  readonly atriumId: "LA" | "RA";
...
export function evaluateAtrialOneFiberPassiveV1(
  ...
  const passive = evaluatePassiveEnergy(fiberLogStrain, params);
  const virtualWorkCoefficient =
    wallVolumeMl * dFiberLogStrainDCavityVolumePerMl;
  const transmuralPressurePa =
    virtualWorkCoefficient * passive.stressPa;
  const dTransmuralPressureDCavityVolumePaPerMl = wallVolumeMl * (
    passive.tangentPa * dFiberLogStrainDCavityVolumePerMl ** 2
    + passive.stressPa * d2FiberLogStrainDCavityVolume2PerMl2
  );
