...
export function applyExactEventCalciumStimulusV1(
  ...
  const [riseDrive, decayDrive] = validateState(stateBeforeEvent, "stateBeforeEvent");
...
export function advanceExactEventCalciumV1(
  ...
  while (eventIndex < events.length) {
    ...
    if (event.timeSec <= start || event.timeSec > end) {
      throw new Error("advanceExactEventCalciumV1 requires event times in (startTimeSec, endTimeSec]");
    }
    ...
    while (eventIndex < events.length) {
      const coincident = validateEvent(events[eventIndex], `events[${eventIndex}]`);
...
export function convertPeriodicBiexponentialToExactEventCalciumV1(
  ...
  if (
    ...
    throw new Error("periodic biexponential peak must lie within the reference cycle");
  }
  ...
  if (
    ...
    throw new Error("periodic biexponential must have a positive peak-to-trough excursion");
  }
...
function validatePeriodicInput(
  ...
  requireNonnegativeFinite(periodic.diastolicCalciumUM, "periodic.diastolicCalciumUM");
  requireNonnegativeFinite(periodic.peakAmplitudeUM, "periodic.peakAmplitudeUM");
  requirePositiveFinite(periodic.riseTimeConstantSec, "periodic.riseTimeConstantSec");
  requirePositiveFinite(periodic.decayTimeConstantSec, "periodic.decayTimeConstantSec");
  if (!(periodic.decayTimeConstantSec > periodic.riseTimeConstantSec)) {
    throw new Error("periodic.decayTimeConstantSec must exceed periodic.riseTimeConstantSec");
  }
}
...
function validateState(
  ...
  if (state.length !== 2) throw new Error(`${field} must contain [riseDrive, decayDrive]`);
...
function validateEvent(event: ExactEventCalciumEventV1, field: string): ExactEventCalciumEventV1 {
