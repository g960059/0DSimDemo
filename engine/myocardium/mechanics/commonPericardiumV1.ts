 ...
 * coordinate is the total intrapericardial heart volume. The same pressure,
 * dPsi/dVh, is therefore work-conjugate to every chamber cavity volume.
 */
...
export type CommonPericardiumParametersV1 = Readonly<{
  referenceHeartVolumeM3: number;
  exponentialPressureScalePa: number;
  exponentialStiffness: number;
  /** Optional uniform external offset; clinical effusion is owned by fluid volume. */
  prescribedPressureOffsetPa: number;
}>;
...
export function evaluateCommonPericardiumV1(
  ...
  const prescribedPressureOffset = requireNonNegativeFinite(
    parameters.prescribedPressureOffsetPa,
    "prescribedPressureOffsetPa",
  );
  ...
  const storedEnergyJ = prescribedPressureOffset * volume
    + pressureScale * referenceVolume / stiffness
      * expm1MinusX(exponent);
  const excessPressurePa = prescribedPressureOffset
