...
export function evaluateMainWireCommonPericardiumBindingV1(
  ...
  if (binding.mode !== "on" && binding.mode !== "exact-off") {
    throw new Error("unsupported common-pericardium binding mode");
  }
  ...
  if (
    ...
    throw new Error(
      "prescribedPericardialFluidVolumeM3 must be nonnegative and finite",
    );
  }
