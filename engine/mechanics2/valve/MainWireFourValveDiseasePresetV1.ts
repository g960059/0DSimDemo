...
export const MAIN_WIRE_FOUR_VALVE_DISEASE_PRESET_V1_ID =
  "main-wire-four-valve-disease-preset-v1" as const;
...
export const MAIN_WIRE_FOUR_VALVE_DISEASE_PRESET_CLAIM_V1 = Object.freeze({
  presetRole: "ordered-research-bracket-not-clinical-diagnosis" as const,
...
export type MainWireFourValveDiseasePresetV1 = Readonly<{
  presetId: typeof MAIN_WIRE_FOUR_VALVE_DISEASE_PRESET_V1_ID;
  ...
  valves: Readonly<Record<ValveName,
    MainWireQuasiSteadyOrificeValveParamsV2>>;
  claim: typeof MAIN_WIRE_FOUR_VALVE_DISEASE_PRESET_CLAIM_V1;
}>;
...
export const MAIN_WIRE_FOUR_VALVE_NORMAL_PRESET_V1 =
  composeMainWireFourValveDiseasePresetV1([]);

export const MAIN_WIRE_FOUR_VALVE_DISEASE_PRESETS_V1 = Object.freeze(
  Object.fromEntries(MAIN_WIRE_FOUR_VALVE_DISEASE_BRACKET_IDS_V1.map(
    (bracketId) => [
      bracketId,
      composeMainWireFourValveDiseasePresetV1([bracketId]),
    ],
  )),
) as Readonly<Record<
  MainWireFourValveDiseaseBracketIdV1,
...
export function composeMainWireFourValveDiseasePresetV1(
  inputs: readonly (
    MainWireFourValveDiseaseBracketIdV1
    | MainWireFourValveDiseaseBracketV1
  )[],
  ...
  const identityBody = Object.freeze({
    ...
    bracketIds,
    valves,
    claim: MAIN_WIRE_FOUR_VALVE_DISEASE_PRESET_CLAIM_V1,
  });
  const parameterIdentityHash = stableHash(sanitizeForStableHash(identityBody));
  const result = Object.freeze({
    presetId: MAIN_WIRE_FOUR_VALVE_DISEASE_PRESET_V1_ID,
    parameterSetId: presetParameterSetId(bracketIds, parameterIdentityHash),
    parameterIdentityHash,
    bracketIds,
    brackets,
    valves,
    claim: MAIN_WIRE_FOUR_VALVE_DISEASE_PRESET_CLAIM_V1,
  } satisfies MainWireFourValveDiseasePresetV1);
  const issues = validateMainWireFourValveDiseasePresetV1(result);
  if (issues.length > 0) {
    throw new Error(`invalid four-valve disease preset: ${issues.join("; ")}`);
  }
  return result;
}

export function validateMainWireFourValveDiseasePresetV1(
  preset: MainWireFourValveDiseasePresetV1,
): readonly string[] {
  const issues: string[] = [];
  if (preset.presetId !== MAIN_WIRE_FOUR_VALVE_DISEASE_PRESET_V1_ID) {
    issues.push("presetId must identify four-valve disease preset V1");
  }
  if (typeof preset.parameterSetId !== "string"
    || preset.parameterSetId.trim() === "") {
    issues.push("parameterSetId must be non-empty");
  }
  if (!/^[0-9a-f]{8}$/.test(preset.parameterIdentityHash)) {
    issues.push("parameterIdentityHash must be an eight-character hex hash");
  }
  const rawBracketIds = [...preset.bracketIds] as readonly string[];
  ...
  if (
    knownBrackets.length !== preset.brackets.length
    || stableHash(sanitizeForStableHash(knownBrackets))
      !== stableHash(sanitizeForStableHash(preset.brackets))
  ) {
  ...
  if (
    stableHash(sanitizeForStableHash(preset.claim))
  ...
  const keys = Object.keys(preset.valves).sort();
  ...
  for (const valveId of MAIN_WIRE_FOUR_VALVE_IDS_V1) {
    const valve = preset.valves[valveId];
  ...
  const expectedIdentityHash = stableHash(sanitizeForStableHash(Object.freeze({
    ...
    bracketIds: preset.bracketIds,
    valves: preset.valves,
    claim: preset.claim,
  })));
  if (preset.parameterIdentityHash !== expectedIdentityHash) {
    issues.push("parameterIdentityHash does not match preset contents");
  }
  const expectedParameterSetId = presetParameterSetId(
    preset.bracketIds,
    expectedIdentityHash,
  );
  if (preset.parameterSetId !== expectedParameterSetId) {
