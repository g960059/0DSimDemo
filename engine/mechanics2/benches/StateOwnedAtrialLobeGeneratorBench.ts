...
export const STATE_OWNED_ATRIAL_LOBE_GENERATOR_REPORT_ID_V1 =
  "state-owned-atrial-lobe-generator-report-v1" as const;

type VariantIdV1 =
  | "fiber-pressure-no-lobe-state"
  | "fiber-pressure-reservoir-suction-p1"
  | "fiber-pressure-reservoir-suction-p2"
  | "fiber-pressure-reservoir-suction-p3"
  | "fiber-pressure-reservoir-suction-slow-p2"
  | "fiber-pressure-reservoir-suction-wide-p2"
  | "fiber-pressure-reservoir-suction-geometry-capacity-p2";

type VariantV1 = {
  readonly variantId: VariantIdV1;
  readonly suctionGainMmHg: number;
  readonly riseTauSec: number;
  readonly fallTauSec: number;
  readonly startTheta: number;
  readonly endTheta: number;
...
type RowV1 = {
  ...
  readonly maxSuctionDrive01: number;
  readonly minSuctionPressureMmHg: number;
...
export type StateOwnedAtrialLobeGeneratorReportV1 = {
  readonly reportId: typeof STATE_OWNED_ATRIAL_LOBE_GENERATOR_REPORT_ID_V1;
  readonly gateId: "stateOwnedAtrialLobeGeneratorV1";
  readonly mode: "left-heart-state-owned-atrial-lobe-generator-no-runtime";
  ...
  readonly decision: {
    readonly stateOwnedAtrialLobeGeneratorStatus:
      | "state-owned-atrial-lobe-generator-signal"
      | "state-owned-atrial-lobe-generator-mixed"
      | "state-owned-atrial-lobe-generator-blocked";
...
const VARIANTS: readonly VariantV1[] = [
  {
    variantId: "fiber-pressure-no-lobe-state",
    suctionGainMmHg: 0,
    riseTauSec: 0.08,
    fallTauSec: 0.16,
    startTheta: 0.08,
  ...
  {
    variantId: "fiber-pressure-reservoir-suction-p1",
    suctionGainMmHg: 1,
    riseTauSec: 0.08,
    fallTauSec: 0.16,
    startTheta: 0.08,
  ...
  {
    variantId: "fiber-pressure-reservoir-suction-p2",
    suctionGainMmHg: 2,
    riseTauSec: 0.08,
    fallTauSec: 0.16,
    startTheta: 0.08,
  ...
  {
    variantId: "fiber-pressure-reservoir-suction-p3",
    suctionGainMmHg: 3,
    riseTauSec: 0.08,
    fallTauSec: 0.16,
    startTheta: 0.08,
  ...
  {
    variantId: "fiber-pressure-reservoir-suction-slow-p2",
    suctionGainMmHg: 2,
    riseTauSec: 0.16,
    fallTauSec: 0.30,
    startTheta: 0.08,
  ...
  {
    variantId: "fiber-pressure-reservoir-suction-wide-p2",
    suctionGainMmHg: 2,
    riseTauSec: 0.08,
    fallTauSec: 0.16,
    startTheta: 0.02,
  ...
  {
    variantId: "fiber-pressure-reservoir-suction-geometry-capacity-p2",
    suctionGainMmHg: 2,
    riseTauSec: 0.08,
    fallTauSec: 0.16,
    startTheta: 0.08,
...
export function runStateOwnedAtrialLobeGeneratorBenchV1():
StateOwnedAtrialLobeGeneratorReportV1 {
  ...
  const status = bestVariant.contractPass === PROFILE_IDS.length
    ? "state-owned-atrial-lobe-generator-signal"
    : bestVariant.laPvLobeQualityPass > 2 || sourceSurfaceBest.sourceSurfacePass >= 5
      ? "state-owned-atrial-lobe-generator-mixed"
      : "state-owned-atrial-lobe-generator-blocked";
  return {
    reportId: STATE_OWNED_ATRIAL_LOBE_GENERATOR_REPORT_ID_V1,
    gateId: "stateOwnedAtrialLobeGeneratorV1",
    mode: "left-heart-state-owned-atrial-lobe-generator-no-runtime",
    variants: VARIANTS,
    rows,
    variantSummaries,
    bestVariant,
    ...
    decision: {
      stateOwnedAtrialLobeGeneratorStatus: status,
      nextAction: status === "state-owned-atrial-lobe-generator-signal"
        ? "Proceed only to source-aware four-chamber review. Runtime and physiology claims remain blocked."
        : status === "state-owned-atrial-lobe-generator-mixed"
          ? "Use this state-owned lobe signal to design a broader atrial chamber/valve transaction; keep runtime and physiology claims blocked."
          : "Keep atrial promotion blocked; state-owned reservoir suction did not recover enough lobe quality or source-surface morphology.",
      blockedClaims: [
        "runtime-wiring",
...
function applyVariant(params: LeftHeartSubsystemParamsV2, variant: VariantV1): LeftHeartSubsystemParamsV2 {
  return {
    ...params,
    laPressureSourceMode: "fiber-chamber-total-pressure-shadow",
    laLobeGeneratorMode: variant.suctionGainMmHg > 0 ? "reservoir-suction-state-shadow" : "none",
    laReservoirSuctionPressureGainMmHg: variant.suctionGainMmHg,
    laReservoirSuctionRiseTauSec: variant.riseTauSec,
    laReservoirSuctionFallTauSec: variant.fallTauSec,
    laReservoirSuctionStartTheta: variant.startTheta,
    laReservoirSuctionEndTheta: variant.endTheta,
...
function rowForRun(
  ...
  const base = {
    ...
    maxSuctionDrive01: round(Math.max(0, ...beat.map((sample) => sample.laReservoirSuctionDrive01))),
    minSuctionPressureMmHg: round(Math.min(0, ...beat.map((sample) => sample.laReservoirSuctionPressureMmHg))),
...
function summarizeVariant(variantId: VariantIdV1, rows: readonly RowV1[]): VariantSummaryV1 {
  return {
    ...
    maxSuctionDrive01: round(Math.max(0, ...rows.map((row) => row.maxSuctionDrive01))),
    minSuctionPressureMmHg: round(Math.min(0, ...rows.map((row) => row.minSuctionPressureMmHg))),
