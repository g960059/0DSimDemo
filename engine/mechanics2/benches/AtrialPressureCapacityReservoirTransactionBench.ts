...
export const ATRIAL_PRESSURE_CAPACITY_RESERVOIR_TRANSACTION_REPORT_ID_V1 =
  "atrial-pressure-capacity-reservoir-transaction-report-v1" as const;

export type AtrialPressureCapacityReservoirTransactionVariantIdV1 =
  | "compliance-node-no-avplane"
  | "pressure-capacity-gain12-traction02"
  | "pressure-capacity-gain16-traction03"
  | "pressure-capacity-gain20-traction04"
  | "pressure-capacity-gain24-traction04"
  | "pressure-capacity-gain32-traction04"
  | "pressure-capacity-gain24-traction06"
  | "pressure-capacity-gain32-traction06"
  | "pressure-capacity-gain32-traction08"
  | "pressure-capacity-gain24-traction-negative04";

type GeometryEffectV1 = "capacity" | "stretch";

export type AtrialPressureCapacityReservoirTransactionVariantV1 = {
  readonly variantId: AtrialPressureCapacityReservoirTransactionVariantIdV1;
...
export type AtrialPressureCapacityReservoirTransactionReportV1 = {
  readonly reportId: typeof ATRIAL_PRESSURE_CAPACITY_RESERVOIR_TRANSACTION_REPORT_ID_V1;
  readonly gateId: "atrialPressureCapacityReservoirTransactionV1";
  readonly mode: "left-heart-atrial-pressure-capacity-reservoir-transaction-no-runtime";
  readonly variants: readonly AtrialPressureCapacityReservoirTransactionVariantV1[];
  ...
  readonly summary: {
    ...
    readonly bestVariantId: AtrialPressureCapacityReservoirTransactionVariantIdV1;
    ...
    readonly maxSourceSurfaceVariantId: AtrialPressureCapacityReservoirTransactionVariantIdV1;
    readonly maxLobeQualityPass: number;
    readonly maxLobeQualityVariantId: AtrialPressureCapacityReservoirTransactionVariantIdV1;
    readonly maxDtHalfContractPass: number;
    readonly maxDtHalfContractVariantId: AtrialPressureCapacityReservoirTransactionVariantIdV1;
    readonly dominantLobeFailureCounts: Record<DominantLobeFailureClassV1, number>;
  };
  readonly decision: {
    readonly atrialPressureCapacityReservoirTransactionStatus:
      | "atrial-pressure-capacity-reservoir-transaction-signal"
      | "atrial-pressure-capacity-reservoir-transaction-mixed"
      | "atrial-pressure-capacity-reservoir-transaction-blocked";
...
export const ATRIAL_PRESSURE_CAPACITY_RESERVOIR_TRANSACTION_VARIANTS_V1:
readonly AtrialPressureCapacityReservoirTransactionVariantV1[] = [
  variant("compliance-node-no-avplane", 0, 0, 0, "capacity", 0, 0, 1.0, 30, 1.0, 18, 60, 0.06, 0.58, 0.055, 0.30, 0.085, 0.20),
  variant("pressure-capacity-gain12-traction02", 12, 0, 0.2, "capacity", 0, 0, 1.0, 30, 1.0, 18, 60, 0.06, 0.58, 0.055, 0.30, 0.085, 0.20),
  variant("pressure-capacity-gain16-traction03", 16, 0, 0.3, "capacity", 0, 0, 1.0, 30, 1.0, 18, 60, 0.06, 0.58, 0.055, 0.30, 0.085, 0.20),
  variant("pressure-capacity-gain20-traction04", 20, 0, 0.4, "capacity", 0, 0, 1.0, 30, 1.0, 18, 60, 0.06, 0.58, 0.055, 0.30, 0.085, 0.20),
  variant("pressure-capacity-gain24-traction04", 24, 0, 0.4, "capacity", 0, 0, 1.0, 30, 1.0, 18, 60, 0.06, 0.58, 0.055, 0.30, 0.085, 0.20),
  variant("pressure-capacity-gain32-traction04", 32, 0, 0.4, "capacity", 0, 0, 1.0, 30, 1.0, 18, 60, 0.06, 0.58, 0.055, 0.30, 0.085, 0.20),
  variant("pressure-capacity-gain24-traction06", 24, 0, 0.6, "capacity", 0, 0, 1.0, 30, 1.0, 18, 60, 0.06, 0.58, 0.055, 0.30, 0.085, 0.20),
  variant("pressure-capacity-gain32-traction06", 32, 0, 0.6, "capacity", 0, 0, 1.0, 30, 1.0, 18, 60, 0.06, 0.58, 0.055, 0.30, 0.085, 0.20),
  variant("pressure-capacity-gain32-traction08", 32, 0, 0.8, "capacity", 0, 0, 1.0, 30, 1.0, 18, 60, 0.06, 0.58, 0.055, 0.30, 0.085, 0.20),
  variant("pressure-capacity-gain24-traction-negative04", 24, 0, -0.4, "capacity", 0, 0, 1.0, 30, 1.0, 18, 60, 0.06, 0.58, 0.055, 0.30, 0.085, 0.20),
];

export function runAtrialPressureCapacityReservoirTransactionBenchV1():
AtrialPressureCapacityReservoirTransactionReportV1 {
  const rawParamsByProfile = buildLeftHeartDynamicReserveVariantEnvelopeV1(LEFT_VARIANT_ID);
  const baselineVariant = ATRIAL_PRESSURE_CAPACITY_RESERVOIR_TRANSACTION_VARIANTS_V1[0]!;
  const baselineParamsByProfile = rawParamsByProfile.map((params) =>
    applyAtrialPressureCapacityReservoirTransactionVariantV1(params, baselineVariant)
  );
  ...
  const rows = PROFILE_IDS.flatMap((profileId, index) => {
    ...
    return ATRIAL_PRESSURE_CAPACITY_RESERVOIR_TRANSACTION_VARIANTS_V1.map((variant) => {
      const params = variant.variantId === "compliance-node-no-avplane"
        ? baselineParams
        : applyAtrialPressureCapacityReservoirTransactionVariantV1(rawParamsByProfile[index]!, variant);
  ...
  const variantSummaries = ATRIAL_PRESSURE_CAPACITY_RESERVOIR_TRANSACTION_VARIANTS_V1.map((variant) =>
  ...
  const status = bestVariant.contractPass === PROFILE_IDS.length && bestVariant.dtHalfContractPass === PROFILE_IDS.length
    ...
    : lobeBest.laPvLobeQualityPass > 2
      ...
      : "atrial-pressure-capacity-reservoir-transaction-blocked";
  return {
    reportId: ATRIAL_PRESSURE_CAPACITY_RESERVOIR_TRANSACTION_REPORT_ID_V1,
    gateId: "atrialPressureCapacityReservoirTransactionV1",
    mode: "left-heart-atrial-pressure-capacity-reservoir-transaction-no-runtime",
    variants: ATRIAL_PRESSURE_CAPACITY_RESERVOIR_TRANSACTION_VARIANTS_V1,
    rows,
    variantSummaries,
    bestVariant,
    summary: {
    ...
    decision: {
      atrialPressureCapacityReservoirTransactionStatus: status,
      nextAction: status === "atrial-pressure-capacity-reservoir-transaction-signal"
        ...
        : status === "atrial-pressure-capacity-reservoir-transaction-mixed"
          ? "Use this pressure-only AV-capacity result to decide whether rate-gated AV-plane pressure-capacity mechanics can own the missing v-loop; do not tune pressure substitution or simple lobe generators."
          : "Keep atrial promotion blocked; pressure-only AV-capacity transaction did not recover source-preserving opposed lobe quality.",
      blockedClaims: [
        "runtime-wiring",
...
export function applyAtrialPressureCapacityReservoirTransactionVariantV1(
  params: LeftHeartSubsystemParamsV2,
  variant: AtrialPressureCapacityReservoirTransactionVariantV1,
  ...
  return {
    ...base,
    laLobeGeneratorMode: "av-plane-pressure-capacity-reservoir-transaction-v1",
    ...
    laAVPlaneReservoirReferenceGainMl: 0,
    laAVPlaneReservoirTractionGainMmHgPerNormPerSec: variant.tractionGainMmHgPerNormPerSec,
...
function summarizeVariant(
  variantId: AtrialPressureCapacityReservoirTransactionVariantIdV1,
  rows: readonly RowV1[],
): VariantSummaryV1 {
  return {
    variantId,
...
): AtrialPressureCapacityReservoirTransactionVariantV1 {
  return {
    variantId,
