...
export const LEFT_HEART_OUTPUT_RESERVE_CALIBRATION_REPORT_ID_V1 =
  "left-heart-output-reserve-calibration-report-v1" as const;

type LeftHeartOutputReserveVariantIdV1 =
  | "v2-fixed2-soft-baseline"
  | "semilunar-loss2-root-runoff70"
  | "static-output-reserve-broad5"
  | "active-length-blend-lv7"
  | "active-length-blend-mvf6"
  | "bernoulli-high-no-clamp"
  | "prior-local-high-rescue";
...
export type LeftHeartOutputReservePointResultV1 = {
...
export type LeftHeartOutputReserveVariantResultV1 = {
  readonly variantId: LeftHeartOutputReserveVariantIdV1;
  ...
  readonly pointResults: readonly LeftHeartOutputReservePointResultV1[];
  readonly summary: {
...
export type LeftHeartOutputReserveCalibrationReportV1 = {
  readonly reportId: typeof LEFT_HEART_OUTPUT_RESERVE_CALIBRATION_REPORT_ID_V1;
  readonly gateId: "leftHeartOutputReserveCalibrationGateV1";
  readonly sourceSurface: "v2-fixed2-soft";
  readonly variantResults: readonly LeftHeartOutputReserveVariantResultV1[];
  readonly decision: {
    readonly leftHeartGateBStatus: "improved-but-blocked" | "no-improvement";
    readonly bestStrictVariantId: LeftHeartOutputReserveVariantIdV1;
    readonly bestStrictPassCount: number;
    readonly baselinePassCount: number;
    readonly bestMorphologyVariantId: LeftHeartOutputReserveVariantIdV1;
    readonly bestMorphologyOkCount: number;
    readonly activeLengthBlendSignal: "limited-component-signal" | "not-broad-improving";
...
const VARIANTS: readonly VariantSpecV1[] = [
  {
    ...
    description: "Current V2 fixed-point soft-pressure surface before output-reserve calibration.",
  ...
  },
  {
    ...
    description: "Previous broad outflow signal: semilunar loss2 plus faster root runoff.",
    lvPressureScaleMultiplier: 1,
    lvTrefMultiplier: 1,
    activeLengthExternalBlend01: 0,
  ...
  {
    variantId: "static-output-reserve-broad5",
    description: "Best static broad calibration found after the pulmonary-boundary gate: suppress lower-bound suction and increase LV pressure/fiber plus semilunar/root load.",
  ...
  {
    variantId: "active-length-blend-lv7",
    description: "Tests whether mixing external chamber length into active length-dependence fixes high-drive overpressure without losing the LV PV envelope.",
    lvPressureScaleMultiplier: 1,
  ...
  {
    variantId: "active-length-blend-mvf6",
    description: "Tests a stronger external-length active blend on a morphology-favorable semilunar surface.",
    lvPressureScaleMultiplier: 1.05,
    lvTrefMultiplier: 1.05,
    activeLengthExternalBlend01: 0.3,
    aovResistanceMultiplier: 1,
  ...
  {
    variantId: "bernoulli-high-no-clamp",
    description: "High Bernoulli semilunar surface that removes hard clamp duty but still tests high-drive pressure/MVF behavior.",
    lvPressureScaleMultiplier: 1,
    lvTrefMultiplier: 1.1,
    activeLengthExternalBlend01: 0,
    aovResistanceMultiplier: 1,
    aovInertanceMultiplier: 2,
    aovBernoulliMultiplier: 24,
    rootOutResistanceMultiplier: 0.7,
    minLvVolumeMl: 30,
    absoluteMinLvVolumeMl: 20,
    lvLowerSoftLimitGainMultiplier: 0.05,
  ...
  {
    variantId: "prior-local-high-rescue",
    description: "Prior local high-contractility rescue from the pulmonary-boundary gate; retained to show why local high-drive repair is not broad calibration.",
    lvPressureScaleMultiplier: 1,
    lvTrefMultiplier: 1,
    activeLengthExternalBlend01: 0,
...
export function runLeftHeartOutputReserveCalibrationBenchV1(): LeftHeartOutputReserveCalibrationReportV1 {
  const variantResults = VARIANTS.map(runVariant);
  const baseline = requiredVariant(variantResults, "v2-fixed2-soft-baseline");
  ...
  const bestMorphology = [...variantResults].sort((a, b) =>
    b.summary.morphologyOkCount - a.summary.morphologyOkCount
    || b.summary.pass - a.summary.pass
    || b.summary.outputOkCount - a.summary.outputOkCount,
  )[0]!;
  const bestBlend = variantResults
    .filter((variant) => variant.intervention.activeLengthExternalBlend01 > 0)
    .sort((a, b) => b.summary.pass - a.summary.pass || b.summary.morphologyOkCount - a.summary.morphologyOkCount)[0];
  const activeLengthBlendSignal =
    bestBlend != null && bestBlend.summary.pass > baseline.summary.pass && bestBlend.summary.pass < bestStrict.summary.pass
      ? "limited-component-signal"
      : "not-broad-improving";
  return {
    reportId: LEFT_HEART_OUTPUT_RESERVE_CALIBRATION_REPORT_ID_V1,
    gateId: "leftHeartOutputReserveCalibrationGateV1",
    sourceSurface: "v2-fixed2-soft",
    variantResults,
    decision: {
      leftHeartGateBStatus: bestStrict.summary.pass > baseline.summary.pass
      ...
      bestMorphologyVariantId: bestMorphology.variantId,
      bestMorphologyOkCount: bestMorphology.summary.morphologyOkCount,
      activeLengthBlendSignal,
      remainingBlockers: [
        "contractility-low remains low-output or loses biphasic MVF under static calibration",
        "contractility-high remains MVF-kink/overpressure/clamp-limited under static calibration",
      ],
      nextAction: "Do not broaden to right-heart or LandAtrial. The 5/7 static-output-reserve signal should feed a left-heart chamber-load contract that handles high-drive reserve dynamically instead of fixed semilunar/root/fiber scaling.",
    },
    claimBoundary: {
      runtimeWiring: false,
...
function runVariant(spec: VariantSpecV1): LeftHeartOutputReserveVariantResultV1 {
...
function runPoint(params: LeftHeartSubsystemParamsV2): LeftHeartOutputReservePointResultV1 {
  ...
  const classifications = classify(params.fixtureId, finalBeat, aovEjectionDeltaMl, strokeVolumeDeltaMl, dtStable);
...
): LeftHeartOutputReservePointResultV1 {
  return {
    pointId,
    status: "inconclusive",
    finalBeat,
...
function classify(
  ...
  dtStable: boolean,
): LeftHeartOutputReservePointResultV1["classifications"] {
  ...
  const morphologyOk = lvPvOk && mvfOk && dtStable && flowCoupled && clampFree;
...
function failureReasonsFor(
  classifications: LeftHeartOutputReservePointResultV1["classifications"],
  beat: BeatSummaryV1,
  aovEjectionDeltaMl: number,
  strokeVolumeDeltaMl: number,
): readonly string[] {
...
function requiredVariant(
  variants: readonly LeftHeartOutputReserveVariantResultV1[],
  variantId: LeftHeartOutputReserveVariantIdV1,
): LeftHeartOutputReserveVariantResultV1 {
  const variant = variants.find((candidate) => candidate.variantId === variantId);
  if (variant == null) throw new Error(`Missing output-reserve variant: ${variantId}`);
  return variant;
}
