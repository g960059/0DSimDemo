...
const E2E_EXPERIMENT_ID = "workbench-e2e-primary";
...
test.beforeEach(async ({ page }, testInfo) => {
  ...
  if (testInfo.title.includes("selector creates")) {
    await page.goto("/ja/experiments");
    return;
  }
  await page.goto(`/ja/experiments/${E2E_EXPERIMENT_ID}`);
...
test("@desktop selector creates a model-independent opaque Workbench route", async ({
  ...
  await expect(page).toHaveURL(/\/ja\/experiments\/workbench-[A-Za-z0-9_-]+$/);
