...
type ScenarioWindowV1 = Readonly<{
  inputEpoch: number;
  samples: MainWireCardiacCycleAcceptedSampleV1[];
}>;
...
export class AcceptedScalarAnalysisWindowStoreV1 {
  ...
  appendFrames(frames: readonly StudioSimulationFrameV2[]): void {
    ...
    for (const [scenarioId, scenarioFrames] of grouped) {
      const previous = this.#windows.get(scenarioId);
      // A delivered batch owns one mutable copy; published snapshots remain immutable.
      if (previous) this.#windows.set(scenarioId, {
        inputEpoch: previous.inputEpoch, samples: [...previous.samples],
      });
      let scenarioChanged = false;
      for (const frame of scenarioFrames) {
        scenarioChanged = this.#appendFrame(scenarioId, frame) || scenarioChanged;
      }
      if (!scenarioChanged && previous) this.#windows.set(scenarioId, previous);
      const current = this.#windows.get(scenarioId);
      if (scenarioChanged && current) {
        const first = firstAtOrAfterV1(current.samples,
          current.samples.at(-1)!.acceptedTimeSec - this.#windowSec);
        if (first > 0) this.#windows.set(scenarioId, {
          inputEpoch: current.inputEpoch, samples: current.samples.slice(first),
        });
      }
      changed = scenarioChanged || changed;
    }
  ...
  #appendFrame(scenarioId: string, frame: StudioSimulationFrameV2): boolean {
    ...
    if (previous && current?.inputEpoch === frame.inputEpoch
      && frame.acceptedRevision === previous.acceptedRevision
      && frame.acceptedTimeSec === previous.acceptedTimeSec
      && this.#requiredExactOutputIds.every((id) => previous.values[id] === values[id])) {
      return false;
    }
    let retained: MainWireCardiacCycleAcceptedSampleV1[];
    if (
      current === undefined
