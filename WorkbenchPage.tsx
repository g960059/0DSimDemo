...
import { SimulationParams, SimInstance, PanelDef, PanelType, PanelInstanceConfig, type ControllerItem, type DockviewViewState, type LegendPosition, type WorkbenchWorkspace, type WorkbenchZoneId } from './types';
...
function Workbench() {
  ...
  return (
    ...
    >
      ...
      <AddPanelDialog
        panelType={addingPanelType}
        instances={instances}
        config={addingPanelConfig}
        setConfig={setAddingPanelConfig}
        onCancel={() => { setAddingPanelType(null); setAddingPanelZone(null); }}
        onConfirm={confirmAddPanel}
      />

      <SaveLessonDialog
        isOpen={isLessonDialogOpen}
        lessonTitle={lessonTitle}
        noteExcerptText={noteExcerpt(notes[panels.find((panel) => panel.type === 'NOTE')?.id ?? ''] ?? EMPTY_NOTE_SPINE)}
        stepsCount={stepsDraft.length}
        onTitleChange={setLessonTitle}
        onCancel={() => setIsLessonDialogOpen(false)}
        onSave={saveCurrentLesson}
      />
