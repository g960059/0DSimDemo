...
4. Advance atrial figure-eight work in a separate lane at HR75/90 normal and
   ...
   `volumeRateMlPerSec` provider input, and Phase 5AX adds pressure-decomposition
   debug output. The next implementation step is an A2 viscous/conduit plus
   tension-state booster prototype, still diagnostic-only. Absolute atrial
   waveform targets must be sourced before parameter tuning. A1 is a diagnostic
