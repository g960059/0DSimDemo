...
4. Advance atrial figure-eight work in a separate lane at HR75/90 normal and
   ...
   debug output, and Phase 5AY implements the first A2 prototype. The prototype
   is health/readback-finite but does not improve readable figure-eight evidence
   over A1, so absolute atrial waveform and reservoir/conduit/booster targets
   must be sourced before further parameter tuning. A1 is a diagnostic bridge,
   not the final model; the forward physiology direction is reusable AV-plane
   reservoir coupling plus LandAtrial parameter-pack shadow work. Treat
