...
4. Advance atrial figure-eight work in a separate lane at HR75/90 normal and
   ...
   selection; Phase 5AU records the A2 readout contract and narrows the next
   implementation step to self `volumeRateMlPerSec` input plus
   pressure-decomposition debug output before viscous/conduit and tension-state
   booster prototyping. Absolute atrial waveform targets must be sourced before
   parameter tuning. Treat HR105/120 as edge evidence, not as an LV Land default
   gate.
