...
4. Advance atrial figure-eight work in a separate lane at HR75/90 normal and
   ...
   and Phase 5BA attributes the limited A2 conduit signal to the viscous/conduit
   term rather than booster or AV-plane-extra terms. A2 still has no selection
   claim; use the target pack to constrain any next A2/AV-plane or LandAtrial
   tuning rather than tuning by visual loop shape alone. A1 is a diagnostic
   bridge, not the final model; the forward physiology direction is reusable
   AV-plane reservoir coupling plus LandAtrial parameter-pack shadow work. Treat
   HR105/120 as edge evidence, not as an LV Land default gate.
