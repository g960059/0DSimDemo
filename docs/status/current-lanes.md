...
2/14 AV valve runs, preserves forward volume in 27/28 valve runs, and is
pressure-flow-causality clean in only 4/28. The standalone local
DynamicValveTransitionV1 path is therefore not supported for runtime wiring or
closed-loop morphology acceptance. Treat SeriesElasticV1 and
...
`delayed-close-hysteretic-area-soft-reverse` reduces extra AV waves in only
2/14 AV valve runs, preserves forward volume in 27/28 valve runs, and is
pressure-flow-causality clean in only 4/28. Therefore standalone local
DynamicValveTransitionV1 is not supported for runtime wiring or closed-loop
