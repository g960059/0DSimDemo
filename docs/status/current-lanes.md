...
1. Phase 5CE is now the active blocker for education-visible gross morphology:
   ...
   mapping is also not enough. The next implementation surface is a chamber-
   owned transaction where chamber pressure evaluation, chamber volume/strain,
   valve-load boundary, and Land provider-state advancement are solved together.
   ...
   wrappers, reference-geometry pressure mapping, or LandAtrial parameter
   tuning to hide the PV double-dome and AV inflow artifacts.
