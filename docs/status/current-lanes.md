...
|---|---|---|---|---|
| myocardium | Phase 5AB records `arterial-root-inertance-closed-loop-phase5ab-result-v1`, a diagnostic-only closed-loop effective AoV/root-boundary inertance sweep over the full Phase 5X synthetic user-knob matrix plus a bounded TBV 4350 low-preload edge, for stock active and developer-only LV Land. It carries forward the Phase 5AA lower-inertance Pareto region through the existing `AoV_L` carrier at total 2x/3x/4x, while keeping qDot clamps, valve thresholds, valve loss terms, load/preload, Land parameters, Tref, and source-stress scale fixed. It records 120 runs: 119 measure, 111 are health ok, Land solve failure count is zero, and 72/83 health-ok candidate comparisons have lower AoV-open qDot clamp engagement with output preserved, out of 90 total candidate comparisons, including 36 health-ok Land comparisons. Median Pareto clamp reduction is `0.829895`, median Pareto forward-volume ratio is `1.007398`, and 67 health-ok candidate comparisons show at least one positive morphology proxy, with 70 raw morphology proxy signals tracked separately. Non-ok or unmeasured runs are boundary-tracked and excluded from the health-ok headline. The low-preload edge is period-1 for current stock and Land under this closure, but remains edge evidence only. Legacy active-stress is explicitly frozen as the positive-control reference rather than deleted. SDIRK2 alternans closure remains parallel science work, not the product migration gate. No official case, Workbench, state-schema, runtime UI, production registry, or production runtime wiring exists; Studio remains owner-led static/mock rather than model validation. | Runtime default flip, legacy active-stress deletion, official case reauthoring, Workbench runtime wiring, state-schema migration, Level 3/4 acceptance, official morphology, structural alternans removal, final no-alternans, production/default arterial root/Zc/inertance adoption, direct Ao_SA adoption, direct Zc/reflection signals, valve/load candidate timing acceptance, and Land normal-floor LVEDP attribution remain blocked; arterial load, valve/load, classifier/window, PVein_LA/filling-inertance, and atrial filling lanes remain separate | carry only the lower-output-preserving effective AoV/root inertance region into a separate off-by-default root/Zc prototype or direct impedance bench; separately attribute the Land normal-floor LVEDP blocker, keep SDIRK2 closure against the frozen reference parallel, and consider a separate default-flip PR only after discharge-path and normal-floor blockers are bounded | no final no-alternans acceptance, no official morphology pass, no clinical/scientific validation |
| morphology | Phase M1 records `morphology-blocker-bundle-phase-m1-result-v1`, a compact post-PR #219 current-main residual blocker classification. Phase 5X adds a user-knob robustness preflight using synthetic normal-floor/preload/HR/contractility/afterload/arterial-stiffness/venous-tone points rather than official-case tuning. Phase 5Y localizes the Phase 5X LV qDot blocker to real direct AoV qDot raw/post clamp engagement plus classifier-window amplification, not to an absence of qDot evidence. Phase 5Z tests the short ejection-window denominator hypothesis after per-beat normalization and does not support it as dominant: 13/14 Land points are `no-phase5x-window-amplification`, so qDot/valve-threshold tuning is explicitly not the next fix. Phase 5AA records an offline prescribed-pressure arterial root inertance bench, and Phase 5AB carries the lower-inertance Pareto region into closed loop through existing `AoV_L`, finding 72/83 health-ok output-preserved candidate comparisons with lower AoV-open qDot clamp engagement and 67 health-ok candidate comparisons with at least one positive morphology proxy, with raw totals tracked separately. Phase 5AB is still diagnostic only: no direct Ao_SA adoption, no direct Zc/reflection availability, no valve-timing acceptance, and no fix acceptance. Filling comparator evidence remains 39/42 interpretable; the residual gap is `lv-failure-dobutamine` branch 1 RV beats 1-3 missing only `eaLikeInflowProxy`. Arterial-load comparator evidence remains 42/42 internally interpretable, with direct Zc/reflection signals still explicit `missing-no-proxy` records and proxy use forbidden. | residual dobutamine RV E/A-like proxy gap; direct Zc/reflection signal gap; off-by-default root/Zc prototype or direct impedance bench missing; valve/load timing diagnostics missing; RV filling morphology blockers | carry the Phase 5AB lower-output-preserving effective inertance region into an off-by-default root/Zc prototype or direct impedance bench; keep PVein_LA/filling inertance and atrial A1/refined figure-eight filling separate; derive/emit residual dobutamine RV `eaLikeInflowProxy` | no root-cause acceptance, no fix acceptance, no official morphology pass |
...
| arterial load | Phase 5Y records real AoV qDot raw/post clamp engagement, and Phase 5Z no longer supports short-window denominator amplification as the dominant explanation after per-beat normalization. Phase 5AA adds the offline prescribed-pressure AoV/root inertance bench, and Phase 5AB carries the lower-inertance region into a full closed-loop diagnostic through the existing `AoV_L` effective root-boundary carrier. The Phase 5AB matrix records 120 stock/Land runs over the full Phase 5X synthetic matrix plus a bounded low-preload edge; 72/83 health-ok candidate comparisons preserve output while lowering AoV-open qDot clamp engagement, out of 90 total candidate comparisons, including 36 health-ok Land comparisons. Median Pareto clamp reduction is `0.829895`, median Pareto forward-volume ratio is `1.007398`, and non-ok/unmeasured runs are boundary-tracked. This supports a narrower discharge-path inertance region, but remains diagnostic: it is not direct Ao_SA adoption, direct Zc calibration, valve timing acceptance, qDot clamp removal, root-cause acceptance, or fix acceptance. Existing arterial-load comparator evidence remains proxy-limited; direct Zc/reflection signals are still explicit `missing-no-proxy` records and must not be inferred from pressure/flow proxies. | no production/off-by-default root/Zc implementation; no direct Zc/reflection signal; no candidate valve/load timing diagnostic; no fix acceptance evidence that physical inertance makes qDot clamps unnecessary in the live production closure | carry only the lower-output-preserving effective inertance region into an off-by-default root/Zc prototype or direct impedance bench with qDot and valve thresholds fixed; success signal remains lower dQ/dt/qDot clamp engagement and longer, smoother ejection without myocardium, Tref, qDot, valve-threshold, or load tuning | production/default Zc adoption, qDot clamp removal, valve-threshold tuning, official morphology pass |
...
1. Carry the Phase 5AB closed-loop effective AoV/root inertance signal into an
   off-by-default root/Zc prototype or direct impedance bench, with qDot clamps,
   valve thresholds, valve loss terms, load/preload, Tref, source-stress scale,
   and Land parameters still fixed. The next step is not qDot clamp removal or
   default adoption; it is narrowing a physical discharge-path candidate that
   can later retire the clamp by mechanism rather than tuning.
