...
4. Advance atrial figure-eight work in a separate lane at HR75/90 normal and
   ...
   selection; Phase 5AU records the A2 readout contract and Phase 5AW adds the
   self `volumeRateMlPerSec` provider input, so the next implementation step is
   ...
   parameter tuning. A1 is a diagnostic bridge, not the final model; the forward
   physiology direction is reusable AV-plane reservoir coupling plus LandAtrial
   parameter-pack shadow work. Treat HR105/120 as edge evidence, not as an LV
   Land default gate.
