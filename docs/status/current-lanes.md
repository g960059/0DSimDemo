...
2. Interpret Phase 5W developer-only LV Land envelope evidence before any production-shadow dry-run; keep official cases, Workbench, production registries, runtime UI, and state schema blocked.
3. Keep provider-local SDIRK2 solver hardening narrow; Phase 5C-R did not unlock final no-alternans.
4. Run paired stock-active versus developer-only LV Land morphology matrix before using Phase 5W to justify production-shadow morphology.
