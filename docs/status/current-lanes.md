...
4. Advance atrial figure-eight work in a separate lane at HR75/90 normal and
   ...
   loops. A2 still has no selection claim; use the target pack and reusable
   coupling from LandAtrial shadow work rather than tuning by visual loop shape
   alone. A1 is a diagnostic bridge, not the final model; the forward physiology
   direction is reusable AV-plane reservoir coupling plus LandAtrial
   parameter-pack shadow work, with wall/AV-plane strain proxy as the next
