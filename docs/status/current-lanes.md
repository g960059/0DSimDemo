...
1. Phase 5CD is now the active blocker for education-visible gross morphology:
   ...
   failures. The next implementation surface is an
   ActiveSourcePressureContractV2 where chamber pressure evaluation, chamber
   volume/strain, valve-load boundary, and Land provider-state advancement are
   one chamber-owned transaction. Do not use global zeta-disablement,
   qDot/root/Zc/valve retuning, Tref, source-stress scaling, pressure-source
   lag, fixed-point BE, coupled Newton wrappers, or LandAtrial parameter tuning
   to hide the PV double-dome and AV inflow artifacts.
