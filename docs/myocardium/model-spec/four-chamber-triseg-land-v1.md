...
- **atrial-human-prior-v1**: a project-derived parameter prior for LA/RA, with separate calcium timing, calcium sensitivity, cycling-rate scale, and tissue-class \(T_{ref}\) prior; this is not an extra source-to-wall gain.
...
These two complete manifests do not yet exist in this specification and are a Phase-A1 implementation deliverable and release blocker. Until their schema instances and isolated-twitch target packs exist, the family names above define architecture but do not constitute a numerically reproducible parameterization.
...
SLS-off MUST retain reservoir/conduit/pump phase ordering and the core figure-eight topology; loop area, lobe amplitude, filling, and venous-wave amplitude need not be invariant. A material contribution of SLS to those quantitative features may be claimed only from independent multi-rate and multi-load evidence.
...
2. verify that the defect persists in the SLS-off causal ablation;
...
The published Taylor approximations are used only inside their checked thickness-curvature range. Lumens et al. reported errors below 1% for fiber strain and below 2% for wall tension over \(|z|<0.8\). V1 therefore treats \(|z|\ge0.8\) as outside the reference-oracle validity envelope unless an exact through-wall integration or a newly verified approximation is supplied.
...
At an implicit DAE stage, patch-area equilibrium and calcium/Land/SLS residuals are solved monolithically. A condensed implementation may locally eliminate the material states as functions of stage strain only if it returns the exact consistent tangent; freezing previous-step material state during the area solve is prohibited.
...
The algebraic Jacobian \(\partial g/\partial z\) must remain nonsingular over the declared physiological domain. Near loss of uniqueness is a model-health failure, not a reason to choose an arbitrary septal root.

### 14.2 Reference solver

The bring-up reference is monolithic backward Euler:
...
where \(x_{Ca}\) contains the five \((r_p,d_p)\) pairs and \(x_d\) contains all circulation, Land, and SLS states. Any prescribed-calcium-drive event inside a candidate step splits the step before stages are formed and applies the jump exactly.
...
- fixed-prior SLS-on is the canonical 59-state baseline;
- SLS-off preserves reservoir/conduit/pump ordering and core figure-eight topology, while loop area and amplitudes may change.
...
- whole-heart BE SLS identity residual, formed by multiplying each wall's energy-density identity in Section 7.2 by \(V_{w0}\), summing the resulting joule-valued terms over all walls, and normalizing by their absolute sum plus \(\mathcal P_{floor}\Delta t\): \(<10^{-10}\);
...
- SLS-off may change loop area and amplitudes but preserves reservoir/conduit/pump ordering and does not create or erase the core figure-eight topology;
...
3. Couple LA/RA one-fiber and the published TriSeg release reference in monolithic BE.
4. Pass periodic, multi-start, perturbation, conservation, and energy gates.

### Phase C — production numerical method

1. Exact calcium stage propagation and exact SLS stage elimination.
