...
beat 1 は cold accepted state から始まり、その state は Diagnostic V2 sample を
...
closure panel は `calcium-event-state` を含む全8 group を表示する。PV panel と
pressure panel の LA/LV/Ao は intracavitary absolute node pressure を使う。一方、
constitutive law、cavity/wall work、SLS ledger は transmural pressure を使い、共通
pericardial work を二重計上しない。この圧所有を title と claim boundary に明記する。
...
## 5522.11 mL evidence matrix

全caseは PR477 の semantic owner
`main-wire-normal-adult-noncoronary-fixed-tbv-5522p11ml-v1` を使い、fixed/resolved
TBV をともに `5522.11 mL` とする。healthy analytic 2/1 ms と effusion 2/1 ms は、
同じ converged 4 ms cycle-boundary checkpoint から分岐する。SLS-off と
exact-event representation は protocol identity が異なるため canonical start とする。

tracked evidence は次である。

- `data/myocardium/verification/mainwire-accepted-interval-diagnostics-v3/*-summary-v2.json`
- `data/myocardium/verification/mainwire-accepted-interval-diagnostics-v3/analytic-vs-event-dt2ms-v1.json`
- `data/myocardium/verification/mainwire-accepted-interval-diagnostics-v3/envelope-manifest-v3.json`
- `data/myocardium/visuals/mainwire-accepted-interval-diagnostics-v3-*-review-v2.html`
- `data/myocardium/visuals/mainwire-accepted-interval-diagnostics-v3-*-review-v2.svg`

full accepted trace は同じdirectoryの `*-raw.json` として再生成するが、容量の大きい
再現可能中間物なので追跡しない。effusion 2/1 ms は32拍を完走しても period-1 未到達なら、
その事実を保持し morphology acceptance を行わない。analytic-vs-event 比較は同一初期化・
同一2 ms gridで representation 以外の protocol component を固定し、parameter fitting、
smoothing、resampling を行わない。
