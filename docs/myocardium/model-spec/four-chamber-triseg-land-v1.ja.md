...
- **atrial-human-prior-v1**: LA/RA用にプロジェクトが導出したパラメータ事前値。カルシウム時相、カルシウム感受性、およびサイクリング速度スケールを別にする。\(T_{ref}\) は各完全inventoryに明示するが、細胞からone-fiber応力へのmappingを独立ownerで同定するまでは心房用に独立変更しない。これは追加のソース–壁ゲインではない。
...
Lewalleらはその後、37 °Cの化学的透過化ヒトLA/LV線維（9 donorからLA 59、LV 67 preparation）を測定し、LAでは長さ依存活性化が弱い一方、張力再発達が速いことを示した。実測 \(k_{tr}\) はLAで \(40\pm13\,\mathrm{s}^{-1}\)、LVで \(15\pm3\,\mathrm{s}^{-1}\) である（[Lewalle et al., 2026](https://doi.org/10.1016/j.yjmcc.2025.12.001)）。同研究の公開較正モデルはLand方程式を継承するが、太いフィラメントの \(B_{off}/U_{off}\) 状態と力依存OFF–ON feedbackを追加している。したがって、その微視的fit parameterはv1の6状態Landへ**直接移植できない**。V1では、protocolを対応させたヒト力–カルシウム、急速再伸長、\(k_{tr}\) の観測量を独立component targetとして登録し、単純モデルが事前宣言したhold-out component dataに失敗した場合に限り、OFF-state modelを将来の競合topologyとする。公開code snapshotはprovenanceとして固定するが、その \(k_U\) が論文の固定parameter tableと不一致であることなどから、parameter setとしては昇格させない。

**現在の心房Land根拠実装。** 文献referenceと現candidate監査は分離し、それぞれSHA-256 `3655a58973ac884eefaefc3d7e29555717f4aef36899283a1a51064de3ea27b7`、`3b9b106a8971f56fbe5890d49b0ce0d4e765ab0d865e03ad77d9cb6afed38485` でcanonical化する。現candidateはLand--Niedererの6状態primitive mappingを保持する一方、実験的に妥当化済みparameter setへの昇格は明示的に不合格とする。そのbase parameter packは、Land 2017のwhole-organ kineticsと40.5 kPaのcellular/skinned \(T_{ref}\)を組み合わせたhybridであり、完全なintact-human parameter vectorとは呼ばない。120 kPaのwhole-organ \(T_{ref}\)はwall-scale mapping未解決の候補として残す。[Gerach et al. 2021](https://doi.org/10.3390/math9111247)（\(CaT50Ref=1.05\,\mu\mathrm{M}\)、\(\beta_1=-0.5\,\mu\mathrm{M}\)）は暗黙に移植せず、直後に報告する同一protocol比較済みの心房競合候補として保持する。Strocchi 2023は単一parameter vectorではなくhistory-matched ensembleのcontextとして扱う。別の単離shortening--hold--restretch component protocolでは、bit-exactに同一のevent前状態から4 armを分岐し、1、0.5、0.25 msの3 gridで実行する。result hashは `1de5a8a72c28b648c0f72d1e3dabda330a2a2c5dd9e8b57509649dddd975a8ff` である。0.25 msではrestretch minus matched holdが64.2537 Pa（active-off自己再上昇を差し引くと63.3186 Pa）でpeakとなるが、total active-stress traceに独立したsecondary peakは解像されない。これはcomponent帰属に限られ、Lewalleの20% slack--restretch \(k_{tr}\) protocol、閉ループ根拠、生理学的妥当化のいずれでもなく、形態はgateに用いない。

Gerach controlled comparisonは実行可能となり、SHA-256 `59ffed9b20dc156d83230fdbb4f3128487caeb7cb15e6ea407a15ebbb72c03c4` でcanonical化する。Ca、strain、\(T_{ref}\)、wall adapterを同一に固定した0.25 ms比較では、Gerach-like pairはisometric peak active stressを16.328から4.169 kPaへ、absolute matched restretch-minus-hold componentを64.254から21.701 Paへ低下させた。一方、primary active-stress amplitudeで正規化した同componentは0.004914から0.006283へ増加する。各candidateは固有の拡張期平衡へ別々にburn-inするため、prescribed inputは同一でもcandidate間のevent前Land状態はbit-exact同一ではない。\(CaT50Ref\)と\(\beta_1\)を同時に変更し、全体のforce scaleも移動するため、絶対componentの低下と正規化componentの上昇のどちらからもlength sensitivityの変化は同定できない。両候補ともtotal-stress secondary peakは独立に解像されない。現在の低いprescribed Ca inputでabsolute forceが大きく失われることは示すが、late-stretch感受性を単独同定した結果ではなく、Gerach pairのruntime defaultへの昇格根拠でも科学的棄却根拠でもない。
...
Phase A1では、この2つの内容ハッシュ付きcandidate-prior manifestと孤立twitchソース回帰packを実装した。したがって現在のschema instanceは数値的に再現可能であるが、実験的な生理学的妥当性を確立したrelease familyではない。現行の心房二重指数カルシウム候補ではTPT 72 ms、RT50 38 msであるのに対し、Land–Niedererの調整済みモデル文脈は約82 ms、75 msを報告する。現行カルシウム時相はproject-syntheticであり、同研究がdigitizeしたヒト心房transientを再構成したものではない。この不一致は隠したりPVループfitで修復したりせず、監査結果として保持する。独立カルシウムおよびprotocol対応mechanicsの受入証拠を通過するまでは、project-synthetic／source-regression evidenceとして扱い、患者・生理学的releaseの阻害要因を維持する。
...
LA/RAは \(r_A,\tau_A\) を共有し、LVFW/SEP/RVFWは \(r_V,\tau_V\) を共有する。v1遷移の下では \(K_{\infty,w}(e_\ast)=d\tau_{\infty,w}/de|_{e_\ast}=K_{0,w}\) であり、各壁の平衡スケールを保持する。壁ごとに自由な \(E_v,\tau_v\) を持たせることはv1では禁止する。SLS-onは構造的なfull-model候補、SLS-offはその必須因果アブレーションであり、いずれのラベルも生理学的受理や心房ループのトポロジーをあらかじめ決めない。SLS-offは枝と5状態を除去して54状態のアブレーションモデルとするのであり、\(1/E_v\) を含む式に \(E_v=0\) を代入するのではない。
...
SLS-onとSLS-offは、同じリザーバー／導管／ポンプ位相ラベル、同一容積圧比較、トポロジー分類器、lobe面積、充満指標、および静脈波振幅で報告しなければならない。SLS-offでのトポロジー保持は合否条件としない。8の字の生成・保持・消失は帰属結果であり、それ単独ではSLSが生理学的である根拠にならない。SLSの材料寄与を主張できるのは、独立した複数速度・複数負荷の根拠がある場合に限り、ループを生成または消失させる目的でSLSパラメータを調整してはならない（**MUST NOT**）。
...
2. full SLS-on候補とSLS-off因果アブレーションを、同じ位相・同一容積・トポロジー解析で比較し、いずれの結果も必須としない。
...
公表Taylor近似は、検証済みの壁厚–曲率範囲内でのみ用いる。Lumensらは、生理的作動条件における線維ひずみ近似誤差が1%未満、張力誤差が2%未満であると報告し、Appendix Cでは調整済みひずみfitについて \(|z|<0.8\) を論じている。しかし、これらは \(|z|<0.8\) 全域の張力誤差2%保証ではない。Appendix A4の係数 \(\operatorname{atanh}(z)/z\) と \(1+z^2/3+z^4/5\) を直接比較すると、張力相対誤差2%の境界は約 \(|z|=0.68872\) である。したがってV1では、\(|z|<0.8\) を診断継続域としてのみ保持し、公表張力参照の適格性には直接計算した張力誤差が \(\le2\%\) であることを別途要求する。厳密な壁内積分または新たに検証された近似がない限り、\(|z|\ge0.8\) は公表参照オラクルの診断域外とする。
...
canonicalな安定化Appendix A/Cコンポーネント参照は、次式とする。
...
規格化はAppendix C5a-C6が述べる／含意する二段階に従う。まず、原著がconstant strain offsetと明記するC5aの先頭2項を除去する。次に、残る両方の \(G\) 寄与を \(G(0)=2\) に対して中心化し、残余の \(+1/3\) を除去する。これによりC6の平坦壁規格化を再現し、曲率ゼロで \(e_f^{AC}=\tfrac12\ln(A_m/A_{m,ref})\) となる。ここで \(A_{m,ref}\) は、この規格化における壁固有の **平坦壁** ゼロひずみ参照面積scaleであり、非ゼロ曲率では \(A_m=A_{m,ref}\) でも一般にゼロひずみとはならない。これは心腔圧gainではなく、単一closed-loop形状だけに合わせて調整しては **MUST NOT（禁止）** とする。解析domainは \(|x|<1\) かつ \(|z|<1\) である。

\(|u|\le1/8\) では、binary64評価に次の偶数べき級数を用いる。
...
級数には次項以降の明示的剰余上界を持たせ、switch外では `log1p` 形式を用いる。switch重複域、符号、ゼロ極限、domain端は、180桁で評価し、240桁で再評価して全80桁serialize値の一致を確認するDecimal直接対数reference packに対して固定する。Appendix A6の仕事共役は独立した式経路で検査する。すなわち、C5aの面積微分は固定 \(C_m,V_w\) で直接 \(G'(z)\) 式から、張力はAppendix A4の直接 \(\operatorname{atanh}(z)/z\) 式から求める。A1 verifierはpayloadを再生成し、checked packとのbyte-for-byte一致を必須とする。そのcanonical payload SHA-256は `c2ff9b98ebfc2015a13ccab399f8240790859515f90af061110a0aa69b4c7cee` である。この昇格が解消するのは解析参照コンポーネントblockerだけであり、TriSeg根の収束、生理学的validation、またはclosed-loop release適格性を意味しない。
...
条件数または \(\det\widetilde J_T\) の符号だけを安定性と呼んではならない（**MUST NOT**）。公表Taylor局所component regressionでは
...
で写像する。各行の単位はPaとNである。根では \(g_T\) に比例する積の微分項が消える。ただし実装は、完全な \(r_T^{wc}\) 写像を独立に微分し、完全な積の微分による構成と比較する。内容ハッシュ付きの座標・一般化力scaleを用いて
...
を定義する。この対称部分は **static-restoring classifier（静的復元性分類器）** であり、energy Hessianではない。14.5節に記載した公表Taylorのひずみ経路と張力経路の仕事不整合が残るためである。独立な微分step \(h\) と \(h/4\) に対し、局所regressionは
...
を用いる。固有値は \([-\delta_K,\delta_K]\) の外にある場合だけ正または負とする。2つの正固有値と \(10^{-3}\) 以上のrobust最小marginを持つ根を robust-restoring、一正一負を robust-saddle、2つの負固有値を robust-antirestoring-maximum と分類し、未解決の場合はすべて indeterminate として受理を停止する。局所根はさらに \(\sigma_{\min}(\widetilde J_T)\ge10^{-3}\)、\(\kappa_2(\widetilde J_T)\le10^3\)、scaled residual/update \(10^{-8}\) 以下を保たなければならない。これらはproject-syntheticな実装regression閾値であって、集団生理の閾値ではない。

SLS-onおよびSLS-offのadversarial anchor regressionは、固定有限列挙のもとで上側の robust-restoring 根と下側の robust-saddle 根をともに発見する。局所gateのcanonical payload SHA-256は a253d1d98127633d68e9062102de9f1593fadccf770b581b74d7f7a81657bfa0 である。これはenergetic／thermodynamic stabilityも、未発見根が大域的に存在しないことも、supported envelope、Land/B1妥当性、生理学的妥当性、release適格性も確立しない。

局所実装regressionでは、宣言した各直線負荷pathを \((V_{LV}/V_{LV,0},V_{RV}/V_{RV,0})\) の累積弧長でparameterizeする。各受理点で、path内部の前進5点負荷微分を \(h\) と \(h/2\) で評価し、
...
を解く。その接線はpredictorにだけ使い、node correctorには根計算と同一のscaled damped-Newton solverを用いる。独立に初期化した1、2、4、8分割solveの共通endpointは、scaled-coordinate infinity normで \(10^{-6}\) 以内に一致しなければならない。これは **異なるpredictor初期化によるendpoint一致監査であり、grid convergenceの証拠ではない**。8分割pathは、同じnormと許容差で逆方向走査を閉じなければならない。順方向・逆方向の全sample根は、公表Taylorの2%張力誤差domain内に留まり、root regularity、一般化力変換監査、局所 robust-restoring classifierを通過しなければならない。Predictor失敗は構造化された監査証拠として返し、branch identityは完全な数値pathが通過した場合だけ「宣言anchorから数値的に追跡した」と記録する。

これらの局所path検査だけが確立するのは、project-synthetic anchor近傍のsampled numerical regressionである。sample点間の正則性、一意な連続分枝、連続なsupported envelopeを証明しない。受理するindex-1分枝は、最終的に別途宣言したenvelope全体で一様な \(\sigma_{\min}\) 下限を保たなければならない。Pseudo-arclengthは、correctorの持続的失敗、特異化への接近、接線反転、または拒否した代替根のfoldに対するtrigger付き診断に限定する。Pseudo-arclengthでfoldを通過できても、そのfoldを受理可能なindex-1分枝として救済してはならない（**MUST NOT**）。

**Phase B0の公表Taylor静的TriSeg有限容積envelope状況（現在の狭い子証拠）。** 別の内容ハッシュ付きtest-only子layerとして、時刻0における9個のnode \((u_{LV},u_{RV})\in\{0.99,1,1.01\}^2\) を、物理的なSLS-on／SLS-off両topologyについて固定した。LVとRVの容積増分はそれぞれPVとSVで厳密に補完し、LA／RA／SA／PAを不変に保ち、全sampleのledgerを正とし、総血液量を保存する。graphは8本のcenter spokeと8本のperimeter edgeからなり、各edgeを両方向へ走査する。各走査は事前宣言した細分係数2、4、8、16、32だけを試し、全attemptを記録し、hard gateを通過したうえで全上限閾値に50%の余裕、全下限閾値に2倍の余裕を保つ最初のattemptだけを受理する。projection、clipping、root ranking、pseudo-arclength rescue、隠れた細分化は行わない。全受理sampleは公表Taylorの2%張力誤差domain内に留まり、仕事共役static-restoring gateとroot-regularity gateを通過する。非anchorの全corrected sampleはpredictor seedから独立なalgorithmic-Jacobian stepで再solveし、anchor recordは実際に宣言したseedを保持する。forward endpointをseedとするreverse走査、8個のcenter triangle、両方向の全perimeter cycleは、宣言したscaled-coordinate許容差内で閉じる。9個すべてのcanonical nodeで事前宣言した3-seed censusと逆seed順を実行し、発見した有限clusterをすべて報告するが、大域的な網羅性や一意性は主張しない。弁圧／流量平滑化の3×3 sweepが確立するのは静的な構造的非依存性だけであり、係数2でのstructured exhaustion probeが両方向のproduction rollbackを確認する。canonical nodeと全path sampleを通じた \(\sigma_{\min}\) 最小値、\(\kappa_2\) 最大値、static-restoring margin最小値は、SLS-onでそれぞれ0.02589、11.13、0.02361、SLS-offで0.01871、15.36、0.01706である。このlayerがtrueにするのは `phaseB0PublishedTaylorStaticTriSegFiniteVolumeEnvelopePass` だけである。連続rectangle内部、大域的root一意性、energetic／thermodynamic stability、Phase B0全体受入れ、Land結合Phase B1 envelope、全拍、生理、ModelCore／browser採用、release到達性は確立しない。evidence manifest／numerical evidence／readinessのSHA-256は `701e9706965fcffd6604d2bde079ec40d5dec39781429ea074b8ab6edb9684a3`、`1832e54d6b2d8e91707dff7af71553620950c942c970e00968a39219eedbf9c1`、`3319c2bcac060156d7a8b1e1b13f6440b2faf022c6359cf9467c7366f0bd9a52` であり、verifierは `npm run verify:four-chamber-triseg-land-b0-finite-supported-envelope` である。
...
イベントを含まない陰的DAE段階では、各パッチの規定カルシウム状態を段階時刻へ厳密に伝播し、既知forcingとして供給する。カルシウム状態は非線形段階未知数ではない。パッチ面積平衡とLand／SLS残差をモノリシックに解く。condensed実装は、厳密な整合接線を返す場合に限り、Land／SLS材料状態を段階ひずみの関数として局所消去してよい。面積solve中に前ステップのLand状態またはSLS状態を固定することは禁止する。
...
代数Jacobian \(\partial g/\partial z\) は、証拠classを明示した宣言済みsupported envelope内で受理する各解分枝に沿って非特異でなければならない。1つの根における非特異性が陰関数定理によって保証するのは、局所的なindex-1正則性だけであり、大域的一意性を証明せず、互いに分離した根のどれを選ぶかも決めない。受理には、事前宣言したanchor連結static-restoring分枝契約、そのanchorからのcontinuation、および固定有限列挙で見つかったすべての代替根の明示的報告と分類を別途必要とする。

そこで得られる主張は、その宣言envelopeと列挙のもとで1つの受理可能なanchor連結分枝があることに限られ、未発見の数理根が存在しないことの証明ではない。前時刻の根は初期推定またはcontinuation predictorとしてのみ用い、最小残差、最近傍根、最大接合半径、またはhard-codeした幾何閾値で受理分枝を選んではならない（**MUST NOT**）。局所正則性の喪失への接近、受理分枝の連続性喪失、indeterminateな復元性分類、または受理可能な分枝を1つに定められないことは、モデル健全性の失敗とする。

### 14.2 参照ソルバー

立ち上げ時の参照は、モノリシック後退Euler法である。規定カルシウム駆動イベントごとに、まず候補ステップを分割する。分割後のイベントを含まない各サブステップでは、10個の規定カルシウム状態を右連続な左端からイベント直前の端点 \(t_{n+1}^{-}\) へ厳密に伝播し、既知の端点forcingとしてLandへ供給する。これらは保存される数理状態ではあるが、Newton未知数ではない。境界にイベントがある場合、まずjump前カルシウムを用いた結合BE solveで \(t_{n+1}^{-}\) に到達し、その後にのみカルシウムjumpを厳密に適用して \(t_{n+1}^{+}\) を作り、必要なら代数変数を再初期化する。jump後の値を直前のサブステップへ遡及的に作用させてはならない。

\(x=(x_{Ca},x_d)\) とすると、BE非線形残差は
...
正準SLS-onモデルでは \(\dim x_{Ca}=10\)、\(\dim x_d=49\) であり、独立な2つのTriSeg座標を加えた非線形未知数は \(49+2=51\) 個である。保存する端点は引き続き59の数理状態すべてを含む。SLS-offアブレーションは5つのSLS状態を除去するため、\(\dim x_d=44\)、非線形solveは \(44+2=46\) 未知数、保存する端点は54状態となる。
...
- 6つの流量運動量収支。
- 速度を含まないLand状態。
- SLS-on topologyにおけるSLS粘性ひずみ状態。
- TriSeg平衡。
- 同一時間レベルの心腔圧、心膜、弁損失係数。

厳密伝播したイベント直前のカルシウム端点も同一時間レベルで評価し、既知の生物学的forcingとしてのみこれらの方程式へ入る。BEカルシウム残差で重複してはならない（**MUST NOT**）。
...
**Phase B1コンポーネント基盤の凍結時状況（履歴境界）。** 速度を含まない完全な6状態LandのRHS／BE残差と、\(\lambda_L\) 固定下の解析的 \(6\times6\) BE状態Jacobianを、candidate-prior組織familyの双方について凍結し、厳密なアフィン \(\zeta\leftrightarrow\xi\) 変換のもとでPhase A1ソース座標BE標的caseと意図的に解から外したcaseの双方に対して回帰した。その凍結時点ではコンポーネント限定の証拠であり、51/46未知数の心臓全体solve、端点イベントtransaction、微分したLand／TriSeg cross-column、strict-simplex Newton制御、分枝provenance、心臓全体Land energy auditは未実装だった。本段落は変更しないfoundation境界の記録であり、現在の実装状況を示す段落ではない。foundation manifestのSHA-256は引き続き `fa588cd7d59169795429679eb5c4c824a8d4f68a20c8868a36ea2d950d6dfd2d`、コンポーネントverifierは `npm run verify:four-chamber-triseg-land-b1-foundation` である。

**Phase B1モノリシックvertical sliceの状況（現在の実装証拠）。** 別の内容ハッシュ付きtest-only sidecarとして、project-syntheticな0.25 ms後退Euler 1サブステップを、SLS-onの51未知数とSLS-offの46未知数という両物理topologyで実装した。5壁すべての6状態Land系をglobal Newtonに含め、厳密伝播する10カルシウム状態はNewtonから除外する。さらに、strict Land-simplex fraction-to-boundary、canonicalな5壁材料binding、両topologyの非空端点イベントtransaction（jump前forcing、aggregate commit 1回、イベント後TriSeg再初期化0または1回、rollback／preflight、次区間因果性）、受理端点におけるfrozen-active-set semismooth方向整合性と必須cross-block監査、30個すべてのLand状態速度を順序付き微分制約入力へ組み立てつつ、TriSeg平衡行に対する心房Land columnの構造的ゼロを明示保持する微分stage kinematics、心臓全体SLS BE恒等式、およびLand-awareな心臓全体mechanical endpoint-stage ledgerを実装した。Jacobian監査は同じ凍結semismooth mapを2つのstencilで比較するため、constitutive partialを独立検証するものではない。mechanical ledgerは公表Taylor geometry power defectを明示的に差し引いて報告するが、TriSegの仕事共役性を受理せず、心臓全体BE未分解増分もenergy acceptance gateではなく診断専用である。このvertical sliceは、受理済みcold initialization／material-homotopy分枝、短時間積分、刻み収束、全拍attractor、supported envelope、生理学的または実験的Land妥当性、Phase B1受入れ、ModelCore／browser採用、release runtime到達性を一切確立しない。canonical descriptor／manifest／readinessのSHA-256は順に `d2fd8e854e6c520d1ea73be0b3f32005e50e2be282fee94484d2ce6e91ab1d99`、`973351efd96000e1a69041eb44b41db44d2e4ecc3556005763f3d1ce672e9d69`、`379fb468c1feba725db39465ea7c7bee24476e60e36265a6dce77bc673a82324` であり、verifierは `npm run verify:four-chamber-triseg-land-b1-vertical-slice` である。

**Phase B1 project-synthetic短時間積分の状況（現在の子証拠）。** 別の内容ハッシュ付き子layerとして、同じwarm start modelをSLS-on/offの双方で1 ms進め、固定0.5、0.25、0.125 ms gridすべてに同一のLV free-calcium drive eventを0.5 ms端点で厳密に整列させた。収束normには59/54個の全保存微分状態と2個のTriSeg代数座標を、run前に固定したscale vectorで含める。観測した後退Euler状態次数はSLS-onで0.99215、offで0.99213であり、事前宣言したproject-synthetic回帰閾値0.8を上回る。全28受理substepで、初期値基準の血液量残差最大値は (4.34\times10^{-19}\,\mathrm{m^3})、strict Land-simplex marginは正、すべてのfrozen-semismooth Jacobian／微分kinematics監査は通過し、SLS恒等式は閉じ、Taylor defect差引後corrected-stage残差最大値は (8.93\times10^{-10}) である。心臓全体の未分解後退Euler energy incrementは局所的にほぼ2次で減少するが、引き続き診断専用である。この子証拠は、受理済み初期化／分枝provenance、一般的な短時間積分または刻み収束envelope、TriSeg仕事共役性、心臓全体BE energy受入れ、全拍、生理、Phase B1受入れ、runtime採用を確立しない。manifest／readinessのSHA-256は `f6c71697bcdcefe555dabe69ad9a493d82eebd07c37acd3bf9183ad3bc2fa06c` と `dd4c2b2e45d4002c1b33be742005213349ef928989d957cb46b87ac5877d1259` であり、verifierは `npm run verify:four-chamber-triseg-land-b1-short-horizon` である。

**Phase B1 project-synthetic結合cold initializationの状況（現在の独立子証拠）。** 別の内容ハッシュ付き子layerとして、synthetic-geometry warm startや短時間runnerをimportも消費もせず、固定した心腔容積・流量・カルシウムのもとで5壁材料平衡とpublished TriSeg平衡を同時に解く。物理的なSLS-on topologyは37未知数（Land座標30、独立SLS座標5、TriSeg座標2）であり、SLS-offでは5個のSLS座標と方程式を物理的に除去して32未知数とする。宣言した構成則continuationではpassive stressと、有効時のSLS stressを保持し、active stressだけを \(\tau_{\mathrm{active}}(\eta)=(1-\eta)\tau_{\mathrm{anchor}}+\eta\tau_{\mathrm{Land}}\) で置換し、\(\eta=0\) のscaffoldの対称な目標壁内外圧差1000 Paに対応する壁別project-synthetic active anchor stressから、\(\eta=1\) のcanonical Land材料へ移行する。この数値continuationは生物学的loading pathではない。目標圧と導出した5壁すべてのanchor stressはprotocol manifestに固定する。固定1、2、4、8分割のtangent-predictor／Newton-corrector経路は互いに一致する端点へ到達し、8分割の逆経路も閉じる。adaptiveな隠れ細分化、projection、clipping、pseudo-arclength rescue、root rankingは行わない。各経路のentryから最初のnodeまでの直接closureもgateに含め、両topologyで保存精度上ゼロである。両topologyを通したscaled residual最大値は \(5.25\times10^{-11}\)、strict Land-simplex margin最小値は \(9.78\times10^{-6}\)、effective TriSeg特異値最小値は \(2.55\times10^{-3}\)、condition number最大値は16.61、材料Schur tangentと独立な材料完全再平衡tangentとの差の最大値は \(1.04\times10^{-7}\) である。第2 verifierが下位kernelから残差と37/32次元の全Jacobianを再構築し、さらにAST import-graph gateがそのverifierからcold core、evidence、readiness layerへの推移的到達を拒否する。残差vector差最大値は0、tangent差最大値は \(2.07\times10^{-7}\)、材料blockのrelative pivot最小値は \(2.38\times10^{-4}\) である。事前宣言した有限3 seedの端点censusは全seedが収束し、\(\eta=0\) でrobust saddleとrobust restoringの両clusterを、\(\eta=1\) でrobust restoring clusterを1つ検出するが、globalなroot網羅性や一意性は明示的に主張しない。forward／reverseのstructured failure probeは、path entryと最初のaccepted node後の両方で失敗を注入し、隠れたrescueなしで実際の方向別path entryへrollbackすることを確認する。readinessは静的protocolだけから推論せず、これらの数値run summaryの内容hashに結合する。このlayerが確立するのは、再現可能なproject-synthetic cold residualとmaterial-branch provenanceだけである。生理的cold initialization、定常循環、連続区間regularity、energetic stability、Phase B0 supported envelope、受理済みPhase B1 reference branch、全拍または生理学的妥当性、ModelCore／browser採用、release runtime到達性は確立しない。manifest／numerical-evidence／readinessのSHA-256は `eacff4e5019d4b7650cd7bba7c3999994ebee674f37093177e572bfef7fa33b7`、`1f3066c237c5fb5661a66824968d6f924f243803a2b5f570d2832fc2d28a01d9`、`401b5e5b29b8ed33794d693074f199773e45b142d44a6191ebf3ab7fd956f994` であり、verifierは `npm run verify:four-chamber-triseg-land-b1-cold-initialization` である。

**Phase B0有限envelope中心からPhase B1 cold \(\eta=0\)へのbridge状況（現在の狭いphase間証拠）。** 別の内容ハッシュ付きtest-only bridgeとして、Phase B0の公表Taylor有限envelopeから、物理的SLS-on／off各topologyのcanonicalな受理済み中心node Cを取り込み、固定したPhase B0 numerical-evidence digestとの一致を必須にした。転送するのはTriSeg代数座標 \((V_{m,S},y_m)=(0,0.03\,\mathrm{m})\) の2値だけであり、Land、SLS、カルシウム、容積、流量、そのほかのPhase B0状態は転送しない。既存のPhase B1 cold方程式内部で、adapterは30個のLand座標と固定カルシウム状態を独立に再構成し、SLS-onでは物理的なrate-free SLS座標・方程式を5個適用し、offではそのtopology自体を除去する。さらに材料seedを解析的に再平衡化し、厳密に \(\eta=0\) で37／32未知数の結合cold nodeを解く。coreでは固定digestとの一致と現行canonical Phase B0 mode resultの再生成・完全一致を別に必須とし、evidence層では親manifestと親numerical certificateを再生成して固定hashを照合し、そのcanonical bundleから直接source modeを取得する。受理解では転送したTriSeg座標は不変で、scaled cold residual最大値は \(1.56\times10^{-13}\) であり、時刻、血液量、保存／計算流量、strain、基準容積、passive材料、TriSeg geometry／oracle、心膜、血管、組織binding、Newton registry、destination provenance、共有parameterの各監査を通過する。両topologyを通した差の最大値は、実際に組み立てられるanchor-active応力と総壁応力が \(1.82\times10^{-12}\,\mathrm{Pa}\)、SLS overstressが \(3.51\times10^{-14}\,\mathrm{Pa}\)、圧力が \(1.14\times10^{-13}\,\mathrm{Pa}\)、慣性流量加速度が \(5.69\times10^{-18}\,\mathrm{m^3\,s^{-2}}\) である。raw Land active-stress fieldには最大 \(1.95\times10^4\,\mathrm{Pa}\) の差があり、\(\eta=0\) では意図的に診断専用である。parity gateが比較するのはLandのraw値ではなく、anchor continuationが実際に組み立てるactive stressである。このlayerがtrueにするのは `phaseB0CenterToPhaseB1EtaZeroBridgePass` だけである。Phase B0全体受入れ、受理済みPhase B1 reference branch、Land結合volume envelope、連続区間regularity、大域的root一意性、energetic stability、生理的初期化、循環定常性、全拍、生理学的妥当性、ModelCore／browser採用、release到達性は確立しない。evidence manifest／numerical evidence／readinessのSHA-256は `a74e6294986d2144573990df4fca5e1d17a58c4642a7b0737f3b956db525528c`、`183f938b1a87c619dee2fd60344ac4f1baed2aa94eac9a0bea8e40552111f60c`、`84ccdf648a18e0f6a5eea1278c744fae2ac02822375075e1862f84376fd8ac47` であり、verifierは `npm run verify:four-chamber-triseg-land-b0-to-b1-eta-zero-bridge` である。

**Phase B0有限envelope中心からPhase B1 \(\eta=1\) material branchまでのstitch状況（現在の狭いphase間証拠）。** 別の内容ハッシュ付き・再帰的freeze済み・test-only evidence bundleとして、evidence層でeta-zero bridgeとproject-synthetic cold branchのmanifest、numerical、readiness各artifactを再生成して固定hashを照合する一方、純粋なstitch core自体は親evidenceを認証したとは主張しない。物理的SLS-on／off各topologyで、bridgeの解析的に再平衡化した \(\eta=0\) endpointとcold anchorのdeep-exact一致、bridge solved nodeとcanonical forward pathの \(\eta=0\) nodeのdeep-exact一致、unknown layoutの厳密一致、scaled unknown差の最大値0を要求し、この境界を再初期化も再求解もしない。続いて、独立初期化した1／2／4／8 segmentのforward continuation、canonicalな8 segment grid \(\eta=0,1/8,\ldots,1\)、およびその8 segment reverse traversalを、時刻、全血液量、全慣性流量、全壁カルシウム状態を固定したままbindingする。両topologyを通した実測最大値は、forward／reverse pathのsampled scaled residualが \(7.094949627806102\times10^{-11}\)、共通endpoint間の不一致が \(3.1290074697931658\times10^{-15}\)、deep-exactなbridge \(\eta=0\) nodeへのreverse closureが \(9.107021061793171\times10^{-11}\) である。\(\eta=1\) では、totalからequilibrium-passiveとSLS-overstressを引いて組み立てたwall-wise active stressがraw Land active stressに対しSLS-onで厳密に差0、SLS-offで最大 \(4.440892098500626\times10^{-16}\,\mathrm{Pa}\) まで一致する。一方、\(\eta=0\) のraw値とassembled値の差は最大 \(19547.179601581924\,\mathrm{Pa}\) であり、このendpoint identityは全pathで自明に成立する同一性ではない。\(\eta=0\) と1で事前宣言した各3 seedの有限censusは両topologyですべて収束し、sampleされた全cluster（\(\eta=0\) ではrestoring 1個と別のsaddle 1個、\(\eta=1\) ではrestoring 1個）を報告する。census結果を受理したtracked pathの選択入力には使わず、大域的exhaustivenessも主張しない。このlayerがtrueにするのは `phaseB0CenterConnectedPhaseB1EtaOneMaterialBranchStitchPass` だけであり、test-onlyのままである。Phase B0全体受入れ、Phase B1受入れまたは受理済みreference branch、supportedなLand結合有限volume graphまたはvolume envelope、連続eta／volume regularity、大域的root一意性または網羅性、energetic stability、生理的初期化、循環定常性、全拍受入れ、生理学的妥当性、ModelCore／browser採用、release-runtime受入れまたは到達性はすべてfalseのままである。evidence manifest／numerical evidence／readinessのSHA-256は `1d1459076227bfd16f924bb19d8f3fa9e5a59408480fb7d644369858d5ef8edd`、`6759092ba01aa58cda41ef5f529f67bdeb7ebc3ee6cdd7afeba61c519af5adef`、`b9dfa93528f0431659d53b8c5998e490093a6236c636570cb21f4e729b90336a` であり、strict verifierは `npm run verify:four-chamber-triseg-land-b0-center-to-b1-eta-one-material-branch-stitch` である。

**Phase B1 \(\eta=1\) Land結合有限sampled LV/RV volume graphの状況（現在の狭い子証拠）。** このproject-syntheticなtest-only layerは、LVとRVの容積倍率0.99、1、1.01の直積にある \(3\times3\) node C/SW/W/NW/N/NE/E/SE/Sをsampleする。厳密なLV\(\leftrightarrow\)PVおよびRV\(\leftrightarrow\)SV closed static ledgerによって総血液量を保存し、LA/RA/SA/PA容積、全flow、時刻、全calcium状態は厳密に固定する。物理的SLS各modeで、8本のcenter spokeと8本のperimeter edgeを、canonical sourceからの有向path 32本およびforward endpointからのround trip 16本として監査する。各pathはrefinement factor 2、4、8ごとに厳密なcanonical sourceから独立にrestartし、hard gateと事前宣言した50% guardを満たす最初のfactorを受理する。中心Cは \(\eta=1\) material-branch stitchとの `Object.is` identityを保持し、宣言方向とreverse方向のfinite censusはいずれもcanonical 9 nodeすべてを覆い、structured failure probeはattempt 0とattempt 1で失敗を注入する。選択factorのhistogramはfactor 2/4/8の順にSLS-onで48/0/0、SLS-offで38/9/1である。両modeのcanonical/pathで記録された全nodeを通したscaled residual最大値は \(8.986318633663615\times10^{-11}\)、Land simplex margin最小値は \(9.7838288212039\times10^{-6}\)、effective TriSeg singular value最小値とcondition number最大値は \(2.513457454296644\times10^{-3}\) と7.795109213039418、static-restoring margin最小値は \(2.5124574542966455\times10^{-3}\) である。material-Schurと完全再平衡化の差の最大値は \(2.162883998185507\times10^{-13}\)、総血液量ledgerの相対誤差最大値は \(1.8269204818159983\times10^{-16}\)、destination不一致とround-trip closureの最大値は \(5.928406853969448\times10^{-11}\) と \(6.680159246943984\times10^{-11}\)、assembled active stressとraw Land active stressの差の最大値は \(8.881784197001252\times10^{-16}\,\mathrm{Pa}\) である。このfinite sampled graphはfinite-volume PDEでも連続volume envelopeでもなく、連鎖したtriangle routeまたはperimeter routeの証拠を与えない。trueになるのは `phaseB1LandCoupledFiniteVolumeGraphPass` だけであり、volume-envelope support、連続eta／volume regularity、大域的root一意性または網羅性、energetic stability、生理的初期化または妥当性、循環定常性、全拍またはPhase B1受入れ、ModelCore／browser採用、release-runtime受入れまたは到達性はすべてfalseのままである。evidence manifest／numerical evidence／readinessのSHA-256は `bcf9c3c628a9a8bed02e783f503f266efd5a44dece1e636a05e18d6bcdbfe89c`、`3a304894732eea7ed0a0ffd55ea5a18b1c6554c874fea60e288adad15151d4d8`、`54a60c3d446a26520aeabecada96694d36e9c3a577675b162ff7ff3d4746dd7f` であり、strict verifierは `npm run verify:four-chamber-triseg-land-b1-eta-one-finite-volume-graph` である。

**Phase B1 project-synthetic quiescent-reference inverseおよびevent-free identity holdの状況（現在の狭い子証拠）。** 別の内容hash付き・再帰的freeze済み・test-only layerとして、\(\eta=1\) finite volume graphのlive canonical center Cだけをsourceにし、4個のlog基準構成、すなわちLA／RAの基準心腔容積倍率と、LVFW／RVFW基準面積のeven／odd modeを射影する。4個の圧力datumでは5番目の基準座標を同定できないため、SEP基準面積はanchor値に厳密固定する。\(\rho\in[0,1]\) のpositive geometric target-pressure scheduleは各source心腔圧を対応する共通血管圧へ補間するが、この数値continuationは生物学的loading pathではない。各trialではaccepted edge-entry cold seedから結合cold mechanicsを完全再平衡化する。SLS-on／offで41／36次元のdirect full augmented five-point Jacobianを \(4\times4\) pressure/log Schur complementへblock eliminationし、primaryとindependentの監査ではconfigured scaled step \(2^{-19}\) と \(2^{-17}\) を使う。全dense columnで実際のeffective stepとstencilをbindingし、対応する各independent effective stepがprimaryと異なり、かつprimaryより大きいことを要求する。両監査はfull rank、singular-value／condition-number gate、cold-block partial-pivot LU診断、収束済みJacobi singular-value監査、coarse/fine Schur一致を必須にする。固定factor 2、4、8は厳密なsourceから独立にrestartし、hard gateと事前宣言した50% guardを初めてともに通過するfactorは、物理的SLS両modeで8である。factor 2と4の全試行も記録し、両reverse pathが閉じ、attempt 0／1のfailure probeがprojection、clipping、prior、regularization、fallback reseed、隠れ細分化、pseudo-arclength rescue、root rankingなしのrollbackを確認する。両modeを通したsource Schur singular value最小値とcondition number最大値は \(1.091307\times10^{-5}\) と30.5741、cold-block relative pivot最小値は \(5.631634\times10^{-7}\)、記録された全nodeにおけるprimary／independent Schur差最大値は \(4.13755\times10^{-8}\) である。共通endpoint不一致最大値は \(1.6213\times10^{-10}\)、factor-eight reverse closure最大値は \(4.5419\times10^{-9}\)、endpointの絶対圧力不一致最大値は \(6.3597\times10^{-10}\,\mathrm{Pa}\)、慣性flow加速度最大値は \(3.1799\times10^{-14}\,\mathrm{m^3\,s^{-2}}\)、sampled fiber log strain絶対値最大は0.404350である。このendpointを受理した後にのみdestination-centered Newton scale registryを厳密に1回compileし、source registryからのreverse engineeringもadaptive rescalingも行わない。続いて0.125、0.25、0.5 msの3個のevent-free後退Euler probeを同じendpointからそれぞれ独立に開始し、単一registryを再利用する。時刻以外の完全な51／46 scalar Newton topologyと10個のcalcium scalarをbit-exactに保持し、全closed-loop flowと総血液量変化は厳密に0で、accepted Newton step数も0である。final scaled residual／update normの最大値は \(4.782914\times10^{-13}\) と \(1.549747\times10^{-13}\) である。trueにするのは `projectSyntheticQuiescentPressureReferenceInverse`、`instantaneousHydraulicQuiescence`、`projectSyntheticQuiescentReferenceEventFreeCase`、`projectSyntheticToleranceCertifiedIdentityHold` の4項目だけである。解析的な厳密平衡、closed-loop stationarity、生理的referenceまたはvalidation、supported reference envelope、event-integratedまたはfull-beat受入れ、Phase B1受入れ、ModelCore／browser採用、release-runtime到達性はすべてfalseのままである。evidence manifest／numerical evidence／readinessのSHA-256は `218d5abd7b53dd0e4db594bac1ac1848d3ac49acc3efbdd02b843ee27338bf24`、`7dd2cc61e9f5da24d9495a89b20369bfea143eb03e511fa312ee0c55a3acc9ec`、`0fd4b9a8b8c624896180303897b73ac72ec5d9f269c96f96ba4490164669b28f` であり、strict verifierは `npm run verify:four-chamber-triseg-land-b1-quiescent-reference` である。
...
と分割する。\(x_{Ca}\) は5つの \((r_p,d_p)\) 対を含み、\(x_d\) は循環、Land、およびSLSの全状態を含む。候補ステップ内に規定カルシウム駆動イベントがある場合、段階を構成する前にステップを分割する。左サブステップとその全段階は、イベント直前極限 \(t_e^-\) まで伝播したカルシウムのみを用いる。そのサブステップの受理後にjumpを \(t_e\) で厳密に適用し、必要なら代数変数を再初期化して、次のサブステップを右連続なjump後状態 \(t_e^+\) から開始する。
...
凍結したproject-synthetic正常洞調律・後退Euler V1プロトコルでは、規定数を **連続3個の1拍supercycle**、hard capを **100 supercycle** とする。候補となる3周期streakの各周期は、隣接周期間の完全状態、Land状態、および物理的に存在する場合のSLS状態について、許容差 \(10^{-6}\) を独立に通過しなければならない。さらに、streak開始点から現在の終点までの完全状態・組織状態gateも通過させるため、個々の変化が小さくても3周期累積変位が過大なら受理しない。各周期では、閉ループ8本すべての符号付き平均流量を、ゼロでない体循環符号付き正味流量 \(Q_{sys}\) に対して相対許容差 \(10^{-3}\) で比較し、符号付き容積が順行容積−逆流容積へ \(512\,\epsilon_{mach}\) 以内で分解され、周期ごとおよびrun開始からの累積TBV相対driftがそれぞれ \(10^{-10}\) 未満であり、上流のschedule、commit済み区間provenance、構造・数値ledger、projection／clipping／fallback不使用の全gateが通過することを要する。連続3周期を満たさず100周期へ達した場合は、収束ではなく `maximum-supercycles-exhausted` とする。checkpoint hash chainには、厳密な定義、schedule、source endpoint、材料binding、Newton scale registry、時間刻み、周期index、両endpoint、診断量を記録する。このプロトコルを変更すれば新しいcontent hashを発行する。通過が確立するのはsingle-startの数値的period-1証拠だけであり、それ単独ではmulti-start受入れ、生理学的妥当性、Phase B1受入れ、runtime採用を確立しない。

対応する3-grid数値programを実行する前に、V1は物理的SLS各topologyについて、後退Euler公称刻み1、0.5、0.25 msの別個のpre-result比較定義を凍結する。各modeではdestination中心のNewton scale registryをいずれのrunより前に1回だけcompileし、その同じregistry objectと内容hashを3 gridすべてで再利用しなければならない。adaptive rescalingと結果確認後のoutput選択は禁止する。閉じたinventoryは厳密に100個のmajor scalarからなる。内訳は、8血液区画すべての容積・絶対圧極値、8 closed-loop edgeすべての符号付き平均流量・順行容積・逆流容積、4弁すべての順行・逆行peak flow、LV/RVのEDV・ESV・SV・EF、5壁のfiber strain・active Kirchhoff stress極値、septum曲率・\(V_{m,S}\)・\(y_m\) の極値、心膜超過圧極値である。極値には認証済みcycle開始endpointと全requested segment exitを用い、commit済みeventと代数再初期化後の右連続値として評価する。受理されたretry child endpointを極値集合へ入れてはならない。公称index 0から \(N\) は各1回だけ現れ、event-onlyの追加exitは明示分類するが公称indexを所有しない。24個のedge積分scalarはすべて認証済みcommitted-interval ledgerだけから取得し、canonicalなcommit済みtransactionをactual durationと後退Eulerのnext-left-limit flowで各1回だけ積分する。失敗／superseded attemptとrequested endpointの台形積分は除外する。medium/fine対にはregistry正規化したstrict 1% scalar boundを課し、ほぼゼロの弁逆流容積だけは別にstrict 0.05 mL絶対差規則を用いる。創発的な弁開閉時刻はmain positive-flow lobeのchord crossingで定義し、2 ms以内で一致させる。各requested segment・各弁では、時系列順の受理済みretry-child endpointをrequested endpointに対するpositive-flow sign topologyの比較だけに用いる。隠れた符号反転または追加positive lobeがあれば、報告時刻を細かく補間せずtraceを無効とする。各SLS modeを独立に、全gridのstage正規化mechanical-energy residualを \(10^{-5}\) 未満、0.25 msのcycle residualを \(10^{-3}\) 未満とし、fine residualがstrictに \(10^{-6}\) 未満でない限り、隣接2対の観測次数をともに0.8以上とする。canonical pre-result manifestのSHA-256は `e067078d52e4ed5d289996cee0ace2b1ff8f26ef90e94e8c4c5b5783c7af7de6` である。このhashが固定するのは比較契約だけであり、terminal observation、timestep収束結果、cycle-energy受入れ結果、生理学的証拠ではない。

この契約に基づく最初のretained terminal-waveform実行を、物理的SLS-on topology・後退Euler 1 msについて記録した。完了supercycle 14で規定の連続3周期通過に達し、認証済み終末拍から右連続の801 sampleを構成した。公称sample inventory、100-scalar inventoryの完全性／順序gate、解決済み8個の弁時相scalar、隠れretry符号監査がすべて通過し、terminal observation gateはpassである。stage正規化mechanical-energy residualの最大値は \(2.64\times10^{-9}\) だったが、1 msでのcycle正規化residualは \(3.254\times10^{-3}\) であるため、このrunは0.25 ms energy gateの対象でもenergy受入れ結果でもない。serialized visual-QA artifactは `docs/myocardium/verification/four-chamber-triseg-land-phase-b1-sls-on-live-periodic-terminal-waveform-1ms.json`、canonical payload SHA-256は `06f4cbf5a6a9f67a8a4c19515872043e6ed5b5aafce2eaa5d32dd5268d477dd7` である。builder-issued認証はserialization前の同一process内で確立され、serialization後には存続しない。これはsingle-start・single-grid・project-syntheticの証拠に限られ、3-grid収束、生理、Phase B1受入れ、runtime採用を確立しない。
...
- 急速伸長／解放。
- 閉ループPV結果から独立にカルシウム履歴と長さ履歴を固定した、短縮–減速–再伸長protocol。二次的な能動応力spikeと短縮後弛緩を報告する。
- 定速短縮と力–速度挙動。
...
- 心室・心房別の標的パック。
- ヒトLAの定常力–カルシウム、長さ依存活性化、\(k_{tr}\)、および急速再伸長の観測量を、protocolを対応させた実験参照として保持する。太いフィラメントの追加OFF状態を持つLand派生モデルのパラメータを、v1の6状態Land方程式へ移植してはならない（**MUST NOT**）。
...
- 固定事前SLS-onを構造的なfull 59状態候補とする。
- SLS-offは必須の54状態因果アブレーションとし、同一の位相・同一容積・トポロジー・ループ面積・波形解析で報告する。8の字保持は合否条件としない。
...
- \(|z|<0.8\) 内でもAppendix A4に対する張力相対誤差を直接計算し、2%を超える場合は **within_0p8_diagnostic_envelope_but_exceeds_2pct_tension_error** と分類して、公表張力参照gateには合格させない。
...
- 受理するBE SLS恒等式gateは、壁間相殺を許さず、7.2節の4項エネルギー恒等式と代数的に同値な次の数値的に安定な残差を壁ごとに評価する：
  \[
  ...
  \right].
  \]
  \(|R_{w,\mathrm{stable}}^{BE}|\) を、その壁のジュール値4項それぞれの絶対値和に \(\mathcal P_{floor}\Delta t\) の5分の1を加えた値で正規化し、5壁での最大値を \(<10^{-10}\) とする。直接展開した4項エネルギー残差は壁ごとに保持し、その壁別最大値と全壁の符号付き総和を診断専用として報告するが、壁別の安定形gateを置き換えたり、壁間相殺を許したりしてはならない。さらに、受理された \(\alpha_v\) と独立な厳密BE更新との差の最大絶対値も \(<10^{-10}\) とする。
- SDIRK2 SLS周期仕事
  \[
  W_{SLS}
  =\sum_wV_{w0,w}
...
- SLS-offでリザーバー／導管／ポンプの順序と中核的な8の字トポロジーが保持・生成・消失のいずれになるかを、同一容積圧較差の帰属とともに報告する。この3つのトポロジー結果のいずれも事前の合否条件とはしない。
...
3. 規定カルシウム駆動イベントでBEステップを分割する。左側の端点solveでは厳密伝播したjump前カルシウムを既知Land forcingとして用い、その後にjumpを適用し、右側サブステップの前に代数変数を再初期化する。カルシウムをNewton未知数には決して含めない。
4. LA/RA one-fiberと公表TriSegリリース参照を、SLS-onでは51未知数、SLS-offでは46未知数のモノリシックBE solveで結合する。
5. 周期、multi-start、摂動、保存、およびエネルギーゲートを通過する。

凍結済み14.2節コンポーネント基盤だけでは、項目2–5の心臓全体Phase B1ゲートは何も完了しない。その後のtest-onlyモノリシックvertical sliceは、上記の単一サブステップ／端点イベントという狭い証拠境界に限って項目1–4を実装した。さらに後続のproject-synthetic子layer群が、項目5に向けた1 ms・3 grid数値回帰と、固定容積・流量・カルシウムにおける独立な結合cold residualおよびactive-material分枝provenanceを追加した。どちらの子layerも、生理的または定常循環の初期化、受理済みreference branch、受理済み項目5 program、Phase B1受入れを確立しない。

### Phase C — 本番数値解法

1. 各SDIRK段階時刻への厳密カルシウム伝播と、厳密SLS段階消去。
