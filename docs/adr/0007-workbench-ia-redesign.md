...
- **P3:** publish flow split into P3a domain/validator, P3b dialog/review-as-reader, and P3c cloud/public-route polish. P3a adds top-level `CaseDocument.defaultEntry?: "read" | "explore"`, pure `validatePublishableCase` referential-integrity checks, and a conservative `active-scenario-hidden` warning based on `isVisible === false`. P3b ships the owner status badge, publish settings dialog, validation checks, and Review-as-reader on an isolated runtime cloned from the draft-applied document. P3c wires Publish/Update through `saveCase({ publish: true })`, preserves publication state on normal Save, adds Unpublish, visible copy-link sharing, and fixes cloud-owned non-owner route gating so reader mode remains the derived Read/Explore experience. No parallel runtime snapshot envelope.
