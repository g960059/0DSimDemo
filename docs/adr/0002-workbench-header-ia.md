...
```
- **Learner** (official case/lesson): `← Cases/Lesson · <title>(read) ⓘ · Compare▾(branch color-dots) · ▶ 1×▾ · **Save a copy** · ⋯`. Hide Export/Load/+Pane/Publish/Save-as-lesson/layout-edit. Dirty state folds into the **Save a copy** CTA. Internally this is a **copy/fork** op.
- **Author** (owned/remixed): `← Cases · ✎<title>•dirty · Branches · Edit layout · +Pane · ▶ 1×▾ · **Share** · ⚙`. Title ✎ → title/description/modelLimitations modal (AI-Studio "Rename app" pattern). Save = quiet auto local-first draft.
- **Sandbox** (blank, default Workbench): `← Home · ✎Untitled scene · +Pane · ▶ 1×▾ · **Save case** · ⚙`.
...
- Mode is inferred from provenance (`meta.author` etc.); ambiguous → default to **Learner** (safer = less chrome). Learner→Author promotion (Save a copy / Remix) is **additive** (never removes controls) + a one-time toast «コピーを編集中です».
