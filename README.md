...
Phase 4C-A では、`passive-exponential-energy-v1` を passive energy readiness candidate として監査します。これは `verify:myocardium-passive-energy-readiness` で derivative/tangent、convexity、smooth hinge、compression behavior、viscous dissipation sign、legacy passive parameter exclusion、runtime/case non-wiring を確認する gate です。Decision 5、Decision 6、Decision 8 は `PENDING OWNER` のままで、production passive law、official morphology outcome、live runtime replacement、multi-coordinate generalized-force mapper は claim しません。RV pressure overload、septal bowing、ventricular interdependence、right-heart failure はこの gate では covered ではなく、future TriSeg path として `triseg-lite-compatible`、`full-triseg-compatible`、full TriSeg escalation を保持します。

Phase 4C-B では、`virtual-power-generalized-force-v1` を generalized-force mapper readiness artifact として監査します。これは `verify:myocardium-generalized-force-mapper-readiness` で active/passive/viscous contribution、multi-coordinate virtual-power closure、m3 coordinate だけの pressure map、zero-derivative coordinate、Phase 3B scalar reduction、runtime/case/workbench non-wiring を確認する gate です。Decisions 4、5、6、8 は `PENDING OWNER`、Decision 19 accepted owner selection は read-only reuse、`ownerAcceptanceStatus=not-owner-acceptance`、`evidenceStatus=synthetic-multi-coordinate-virtual-power-closure-only` のままで、production mechanics、production passive law、official morphology outcome、atrial bridge selection、live runtime replacement、official-case wiring は claim しません。Official morphology、atrial bridge selection、RV pressure overload、septal bowing、ventricular interdependence、right-heart failure はこの gate では covered ではなく、future TriSeg path として `triseg-lite-compatible`、`full-triseg-compatible`、full TriSeg escalation を保持します。

Phase 4D では、Decision 19 で選定された `thick-sphere-v2-explicit-external-septal-coupling` / `thick-sphere-v2` を selected mechanics calibration readiness candidate として監査します。これは `verify:myocardium-selected-mechanics-calibration-readiness` で LV/RV の v2-owned parameter set ids と stable hashes、finite-strain geometry closure、analytic derivative vs finite difference、fixed Land source Tref、no free homogenization/pressure/geometry gain、target-pack measurement hooks、Phase 4C-A passive candidate output、Phase 4C-B generalized-force mapper composition smoke、m3 pressure map finite、runtime/case/workbench non-wiring を確認する `selected-mechanics-calibration-readiness-only` gate です。Phase 4B-A の executable path が `thick-sphere-spike-v1` だった evidence は read-only に保持しますが、現在の selected v2 calibration candidate executable は `selected-thick-sphere-v2-calibration-candidate-executable` であり、production completion や runtime wiring は claim しません。Decisions 4、5、6、8 は `PENDING OWNER`、Decision 19 と Phase 4A dossier は read-only reuse のままです。Official morphology pass、robust calcium-cycling no-alternans validation、septal coordinate/coupling implementation、RV pressure overload、septal bowing、ventricular interdependence、right-heart failure はこの gate では covered ではなく、future TriSeg path として `triseg-lite-compatible`、`full-triseg-compatible`、full TriSeg escalation を保持します。

Phase 5A では、`local-monolithic-be-v1` を pure local monolithic BE reference として監査します。これは `verify:myocardium-local-monolithic-coupling-readiness` で selected v2 LV/RV samples、prescribed calcium、Land BE residual convergence for the six Land states、identity homogenization、passive exponential energy、generalized-force pressure map、7-unknown Newton iteration/finite-difference Jacobian/line-search evidence、local force-balance residual、finite state health、Land strain-rate derivation consistency、deterministic hash を確認する gate です。Phase 5A is not Phase 5 completion. SDIRK2 reference completion is Phase 5B work because current `deriveLand2017StepKinematics()` throws for SDIRK2. Production solver comparison is not claimed, performance acceptance is not claimed, active-stiffness production coupling is not claimed, active-stiffness partitioned production coupling is not claimed, runtime replacement/ModelCore wiring/chamber wiring/case wiring/official-case wiring/workbench wiring are not claimed, official morphology pass and robust no-alternans are not claimed, septal coordinate/coupling implementation, RV pressure overload, septal bowing, ventricular interdependence, and right-heart failure coverage are not claimed, and TriSeg adoption is not claimed. The future TriSeg path preserves `triseg-lite-compatible`, `full-triseg-compatible`, and full TriSeg escalation.

Phase 5B では、`local-monolithic-sdirk2-v1` を pure local monolithic SDIRK2 reference として監査します。これは `verify:myocardium-local-monolithic-sdirk2-readiness` で fixed gamma/tableau、stage0/stage1 の sequential 7-unknown solves、stage history 由来の Land strain rate と cavity coordinate rate、prescribed calcium の sequential stage-increment `dtSec` semantics、Land SDIRK2 residual convergence、local force-balance residual、Newton/finite-difference Jacobian/line-search evidence、finite state health、BE-vs-SDIRK2 finite bounded discrepancy、deterministic hash を確認する reference-only gate です。Phase 5B の SDIRK2 reference completion は local monolithic synthetic reference に限定されます。Phase 5 completion、production solver comparison、performance acceptance、runtime replacement/ModelCore wiring/chamber wiring/case wiring/official-case wiring/workbench wiring、official morphology pass、robust no-alternans、calcium-cycling alternans acceptance、septal coordinate/coupling implementation、RV pressure overload、septal bowing、ventricular interdependence、right-heart failure coverage、TriSeg adoption は claim しません。

Phase 5C-A では、`verify:myocardium-land-shadow-alternans-comparator-readiness` で Land shadow alternans comparator readiness を監査します。これは legacy fixed low-preload period-2 VLV trajectory を selected thick-sphere-v2 LV + prescribed calcium + Land source stress に feedforward shadow replay する artifact gate であり、Phase 4B-A synthetic prescribed protocol とは異なります。Land の beat-to-beat delta は prescribed alternating legacy volume から inherited したものとしてのみ解釈し、Land や calcium cycling が生成した alternans とは解釈しません。現在の validation PASS / artifactGate PASS は、Phase 5C-B の selected v2 LV 30 mL lower-domain extension により fixed legacy VLV samples が selected LV calibration domain 内になったことを安定に報告できるという意味です。no runtime replacement, no official morphology pass, no robust no-alternans, RV pressure overload not covered, ventricular interdependence not covered, right-heart failure not covered, and TriSeg future not covered by this gate.
